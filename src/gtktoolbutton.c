...
gtk_tool_button_construct_contents (GtkToolItem *tool_item)
{
  ...
  if (gtk_tool_button_get_props (button)->priv->icon_widget && gtk_widget_get_props (gtk_tool_button_get_props (button)->priv->icon_widget)->parent)
    {
      __gtk_container_remove (GTK_CONTAINER (gtk_widget_get_props (gtk_tool_button_get_props (button)->priv->icon_widget)->parent),
			    gtk_tool_button_get_props (button)->priv->icon_widget);
    }

  if (gtk_tool_button_get_props (button)->priv->label_widget && gtk_widget_get_props (gtk_tool_button_get_props (button)->priv->label_widget)->parent)
    {
      __gtk_container_remove (GTK_CONTAINER (gtk_widget_get_props (gtk_tool_button_get_props (button)->priv->label_widget)->parent),
  ...
  if (style == GTK_TOOLBAR_BOTH_HORIZ &&
      (__gtk_tool_item_get_is_important (GTK_TOOL_ITEM (button)) ||
       __gtk_tool_item_get_orientation (GTK_TOOL_ITEM (button)) == GTK_ORIENTATION_VERTICAL ||
       __gtk_tool_item_get_text_orientation (GTK_TOOL_ITEM (button)) == GTK_ORIENTATION_VERTICAL))
    {
      need_label = TRUE;
    }
  ...
  if (need_label)
      ...
      else
	{
	  ...
	  if (elide)
	    label_text = ___gtk_toolbar_elide_underscores (label_text);
	  else
	    label_text = g_strdup (label_text);

	  label = __gtk_label_new (label_text);
      ...
      if (GTK_IS_LABEL (label))
        {
          __gtk_label_set_ellipsize (GTK_LABEL (label),
			           __gtk_tool_item_get_ellipsize_mode (GTK_TOOL_ITEM (button)));
          text_orientation = __gtk_tool_item_get_text_orientation (GTK_TOOL_ITEM (button));
          if (text_orientation == GTK_ORIENTATION_HORIZONTAL)
	    {
              __gtk_label_set_angle (GTK_LABEL (label), 0);
              __gtk_misc_set_alignment (GTK_MISC (label),
                                      __gtk_tool_item_get_text_alignment (GTK_TOOL_ITEM (button)),
                                      0.5);
            }
          else
            {
              __gtk_label_set_ellipsize (GTK_LABEL (label), PANGO_ELLIPSIZE_NONE);
	      if (__gtk_widget_get_direction (GTK_WIDGET (tool_item)) == GTK_TEXT_DIR_RTL)
	        __gtk_label_set_angle (GTK_LABEL (label), -90);
	      else
	        __gtk_label_set_angle (GTK_LABEL (label), 90);
              __gtk_misc_set_alignment (GTK_MISC (label),
                                      0.5,
                                      1 - __gtk_tool_item_get_text_alignment (GTK_TOOL_ITEM (button)));
            }
        }
    }
  ...
  if (need_icon)
	...
	__gtk_misc_set_alignment (GTK_MISC (icon),
				1.0 - __gtk_tool_item_get_text_alignment (GTK_TOOL_ITEM (button)),
				0.5);
	...
	__gtk_misc_set_alignment (GTK_MISC (icon),
      ...
      if (icon)
	{
	  size_group = __gtk_tool_item_get_text_size_group (GTK_TOOL_ITEM (button));
	  if (size_group != NULL)
	    __gtk_size_group_add_widget (size_group, icon);
	}
    }
  ...
  switch (style)
    ...
    case GTK_TOOLBAR_ICONS:
      if (icon)
	__gtk_container_add (GTK_CONTAINER (gtk_tool_button_get_props (gtk_tool_button_get_props (button)->priv->button)), icon);
      break;

    case GTK_TOOLBAR_BOTH:
      if (text_orientation == GTK_ORIENTATION_HORIZONTAL)
	box = __gtk_vbox_new (FALSE, icon_spacing);
      else
	box = __gtk_hbox_new (FALSE, icon_spacing);
      if (icon)
	__gtk_box_pack_start (GTK_BOX (box), icon, TRUE, TRUE, 0);
      __gtk_box_pack_end (GTK_BOX (box), label, FALSE, TRUE, 0);
      __gtk_container_add (GTK_CONTAINER (gtk_tool_button_get_props (gtk_tool_button_get_props (button)->priv->button)), box);
      break;
      ...
      if (text_orientation == GTK_ORIENTATION_HORIZONTAL)
	{
	  box = __gtk_hbox_new (FALSE, icon_spacing);
	  if (icon)
	    __gtk_box_pack_start (GTK_BOX (box), icon, label? FALSE : TRUE, TRUE, 0);
	  if (label)
	    __gtk_box_pack_end (GTK_BOX (box), label, TRUE, TRUE, 0);
	}
      else
	{
	  box = __gtk_vbox_new (FALSE, icon_spacing);
	  if (icon)
	    __gtk_box_pack_end (GTK_BOX (box), icon, label ? FALSE : TRUE, TRUE, 0);
	  if (label)
    ...
    case GTK_TOOLBAR_TEXT:
      __gtk_container_add (GTK_CONTAINER (gtk_tool_button_get_props (gtk_tool_button_get_props (button)->priv->button)), label);
      break;
    }
  ...
  __gtk_button_set_relief (GTK_BUTTON (gtk_tool_button_get_props (gtk_tool_button_get_props (button)->priv->button)),
			 __gtk_tool_item_get_relief_style (GTK_TOOL_ITEM (button)));

  __gtk_tool_item_rebuild_menu (tool_item);
  
  __gtk_widget_queue_resize (GTK_WIDGET (button));
}
			      ...
			      GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_LABEL:
      __gtk_tool_button_set_label (button, g_value_get_string (value));
      break;
    case PROP_USE_UNDERLINE:
      __gtk_tool_button_set_use_underline (button, g_value_get_boolean (value));
      break;
    case PROP_LABEL_WIDGET:
      __gtk_tool_button_set_label_widget (button, g_value_get_object (value));
      break;
    case PROP_STOCK_ID:
      __gtk_tool_button_set_stock_id (button, g_value_get_string (value));
      break;
    case PROP_ICON_NAME:
      __gtk_tool_button_set_icon_name (button, g_value_get_string (value));
      break;
    case PROP_ICON_WIDGET:
      __gtk_tool_button_set_icon_widget (button, g_value_get_object (value));
      break;
    default:
			      ...
			      GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_LABEL:
      g_value_set_string (value, __gtk_tool_button_get_label (button));
      break;
    case PROP_LABEL_WIDGET:
      g_value_set_object (value, __gtk_tool_button_get_label_widget (button));
      break;
    case PROP_USE_UNDERLINE:
...
{
  GtkImageType storage_type = __gtk_image_get_storage_type (image);

  if (storage_type == GTK_IMAGE_STOCK)
    {
      gchar *stock_id;
      __gtk_image_get_stock (image, &stock_id, NULL);
      return __gtk_image_new_from_stock (stock_id, GTK_ICON_SIZE_MENU);
    }
  else if (storage_type == GTK_IMAGE_ICON_NAME)
    {
      const gchar *icon_name;
      __gtk_image_get_icon_name (image, &icon_name, NULL);
      return __gtk_image_new_from_icon_name (icon_name, GTK_ICON_SIZE_MENU);
    }
  else if (storage_type == GTK_IMAGE_ICON_SET)
    {
      GtkIconSet *icon_set;
      __gtk_image_get_icon_set (image, &icon_set, NULL);
      return __gtk_image_new_from_icon_set (icon_set, GTK_ICON_SIZE_MENU);
    }
  else if (storage_type == GTK_IMAGE_GICON)
    {
      GIcon *icon;
      __gtk_image_get_gicon (image, &icon, NULL);
      return __gtk_image_new_from_gicon (icon, GTK_ICON_SIZE_MENU);
    }
					     ...
					     &width, &height))
	{
	  ...
	  src_pixbuf = __gtk_image_get_pixbuf (image);
	  dest_pixbuf = gdk_pixbuf_scale_simple (src_pixbuf, width, height,
						 GDK_INTERP_BILINEAR);

	  cloned_image = __gtk_image_new_from_pixbuf (dest_pixbuf);
...
gtk_tool_button_create_menu_proxy (GtkToolItem *item)
{
  ...
  if (use_mnemonic)
    menu_item = __gtk_image_menu_item_new_with_mnemonic (label);
  else
  ...
  if (menu_image)
    __gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (menu_item), menu_image);
		...
		GtkToolButton *button)
{
  ...
  action = __gtk_activatable_get_related_action (GTK_ACTIVATABLE (button));
  ...
  if (GTK_IS_BOX (box))
    {
      __gtk_widget_style_get (GTK_WIDGET (button), 
			    ...
			    NULL);
      __gtk_box_set_spacing (GTK_BOX (box), spacing);      
    }
}
			...
			const gchar    *property_name)
{
  ...
  if (!__gtk_activatable_get_use_action_appearance (activatable))
    return;

  button = GTK_TOOL_BUTTON (activatable);
  
  if (strcmp (property_name, "short-label") == 0)
    __gtk_tool_button_set_label (button, __gtk_action_get_short_label (action));
  else if (strcmp (property_name, "stock-id") == 0)
    __gtk_tool_button_set_stock_id (button, __gtk_action_get_stock_id (action));
  else if (strcmp (property_name, "gicon") == 0)
    {
      const gchar *stock_id = __gtk_action_get_stock_id (action);
      GIcon *icon = __gtk_action_get_gicon (action);
      GtkIconSize icon_size = GTK_ICON_SIZE_BUTTON;

      if ((stock_id && __gtk_icon_factory_lookup_default (stock_id)) || !icon)
	image = NULL;
      else 
	{   
	  image = __gtk_tool_button_get_icon_widget (button);
	  icon_size = __gtk_tool_item_get_icon_size (GTK_TOOL_ITEM (button));

	  if (!image)
	    image = __gtk_image_new ();
	}

      __gtk_tool_button_set_icon_widget (button, image);
      __gtk_image_set_from_gicon (GTK_IMAGE (image), icon, icon_size);

    }
  else if (strcmp (property_name, "icon-name") == 0)
    __gtk_tool_button_set_icon_name (button, __gtk_action_get_icon_name (action));
}
				        ...
				        GtkAction      *action)
{
  ...
  button = GTK_TOOL_BUTTON (activatable);
  stock_id = __gtk_action_get_stock_id (action);

  __gtk_tool_button_set_label (button, __gtk_action_get_short_label (action));
  __gtk_tool_button_set_use_underline (button, TRUE);
  __gtk_tool_button_set_stock_id (button, stock_id);
  __gtk_tool_button_set_icon_name (button, __gtk_action_get_icon_name (action));

  if (stock_id && __gtk_icon_factory_lookup_default (stock_id))
      __gtk_tool_button_set_icon_widget (button, NULL);
  else if ((icon = __gtk_action_get_gicon (action)) != NULL)
    {
      GtkIconSize icon_size = __gtk_tool_item_get_icon_size (GTK_TOOL_ITEM (button));
      GtkWidget  *image = __gtk_tool_button_get_icon_widget (button);
  ...
  else if (__gtk_action_get_icon_name (action))
    __gtk_tool_button_set_icon_name (button, __gtk_action_get_icon_name (action));
  else
    __gtk_tool_button_set_label (button, __gtk_action_get_short_label (action));
}
...
/**
 * __gtk_tool_button_new_from_stock:
 ...
 **/
GtkToolItem *
__gtk_tool_button_new_from_stock (const gchar *stock_id)
{
  GtkToolButton *button;
 ...
 **/
GtkToolItem *
__gtk_tool_button_new (GtkWidget	 *icon_widget,
		     const gchar *label)
{
  GtkToolButton *button;
 ...
 **/
void
__gtk_tool_button_set_label (GtkToolButton *button,
			   const gchar   *label)
{
  gchar *old_label;
  gchar *elided_label;
  ...
  if (label)
    {
      ...
      accessible = __gtk_widget_get_accessible (GTK_WIDGET (gtk_tool_button_get_props (gtk_tool_button_get_props (button)->priv->button)));
 ...
 **/
const gchar *
__gtk_tool_button_get_label (GtkToolButton *button)
{
...
/**
 * __gtk_tool_button_set_use_underline:
 * @button: a #GtkToolButton
 ...
 **/
void
__gtk_tool_button_set_use_underline (GtkToolButton *button,
...
/**
 * __gtk_tool_button_get_use_underline:
 * @button: a #GtkToolButton
 * 
 * Returns whether underscores in the label property are used as mnemonics
 * on menu items on the overflow menu. See __gtk_tool_button_set_use_underline().
 * 
 ...
 **/
gboolean
__gtk_tool_button_get_use_underline (GtkToolButton *button)
{
...
/**
 * __gtk_tool_button_set_stock_id:
 * @button: a #GtkToolButton
 * @stock_id: (allow-none): a name of a stock item, or %NULL
 *
 * Sets the name of the stock item. See __gtk_tool_button_new_from_stock().
 ...
 **/
void
__gtk_tool_button_set_stock_id (GtkToolButton *button,
			      const gchar   *stock_id)
{
  gchar *old_stock_id;
...
/**
 * __gtk_tool_button_get_stock_id:
 * @button: a #GtkToolButton
 * 
 * Returns the name of the stock item. See __gtk_tool_button_set_stock_id().
 ...
 **/
const gchar *
__gtk_tool_button_get_stock_id (GtkToolButton *button)
{
  g_return_val_if_fail (GTK_IS_TOOL_BUTTON (button), NULL);
...
/**
 * __gtk_tool_button_set_icon_name
 * @button: a #GtkToolButton
 ...
 **/
void
__gtk_tool_button_set_icon_name (GtkToolButton *button,
			       const gchar   *icon_name)
{
  gchar *old_icon_name;
...
/**
 * __gtk_tool_button_get_icon_name
 * @button: a #GtkToolButton
 * 
 * Returns the name of the themed icon for the tool button,
 * see __gtk_tool_button_set_icon_name().
 *
 ...
 **/
const gchar*
__gtk_tool_button_get_icon_name (GtkToolButton *button)
{
  g_return_val_if_fail (GTK_IS_TOOL_BUTTON (button), NULL);
...
/**
 * __gtk_tool_button_set_icon_widget:
 * @button: a #GtkToolButton
 ...
 **/
void
__gtk_tool_button_set_icon_widget (GtkToolButton *button,
				 GtkWidget     *icon_widget)
{
  g_return_if_fail (GTK_IS_TOOL_BUTTON (button));
  ...
  if (icon_widget != gtk_tool_button_get_props (button)->priv->icon_widget)
    {
      if (gtk_tool_button_get_props (button)->priv->icon_widget)
	{
	  if (gtk_widget_get_props (gtk_tool_button_get_props (button)->priv->icon_widget)->parent)
	    __gtk_container_remove (GTK_CONTAINER (gtk_widget_get_props (gtk_tool_button_get_props (button)->priv->icon_widget)->parent),
				    gtk_tool_button_get_props (button)->priv->icon_widget);
...
/**
 * __gtk_tool_button_set_label_widget:
 * @button: a #GtkToolButton
 ...
 **/
void
__gtk_tool_button_set_label_widget (GtkToolButton *button,
				  GtkWidget     *label_widget)
{
  g_return_if_fail (GTK_IS_TOOL_BUTTON (button));
  ...
  if (label_widget != gtk_tool_button_get_props (button)->priv->label_widget)
    {
      if (gtk_tool_button_get_props (button)->priv->label_widget)
	{
	  if (gtk_widget_get_props (gtk_tool_button_get_props (button)->priv->label_widget)->parent)
	    __gtk_container_remove (GTK_CONTAINER (gtk_widget_get_props (gtk_tool_button_get_props (button)->priv->label_widget)->parent),
		    	          gtk_tool_button_get_props (button)->priv->label_widget);
...
/**
 * __gtk_tool_button_get_label_widget:
 * @button: a #GtkToolButton
 *
 * Returns the widget used as label on @button.
 * See __gtk_tool_button_set_label_widget().
 *
 ...
 **/
GtkWidget *
__gtk_tool_button_get_label_widget (GtkToolButton *button)
{
  g_return_val_if_fail (GTK_IS_TOOL_BUTTON (button), NULL);
...
/**
 * __gtk_tool_button_get_icon_widget:
 * @button: a #GtkToolButton
 *
 * Return the widget used as icon widget on @button.
 * See __gtk_tool_button_set_icon_widget().
 *
 ...
 **/
GtkWidget *
__gtk_tool_button_get_icon_widget (GtkToolButton *button)
{
  g_return_val_if_fail (GTK_IS_TOOL_BUTTON (button), NULL);
...
GtkWidget *
___gtk_tool_button_get_button (GtkToolButton *button)
{
  g_return_val_if_fail (GTK_IS_TOOL_BUTTON (button), NULL);

  return gtk_tool_button_get_props (gtk_tool_button_get_props (button)->priv->button);
}
