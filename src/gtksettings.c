...
gtk_settings_class_init (GtkSettingsClass *class)
{
   ...
   * sizes with __gtk_icon_size_register().
   */
  result = settings_install_property_parser (class,
   ...
   * __gdk_window_beep(), the windowing system may offer ways to
 ...
 */
GtkSettings*
__gtk_settings_get_for_screen (GdkScreen *screen)
{
...
/**
 * __gtk_settings_get_default:
 * 
 * Gets the #GtkSettings object for the default GDK screen, creating
 * it if necessary. See __gtk_settings_get_for_screen().
 *
 * Return value: (transfer none): a #GtkSettings object. If there is no default
 *  screen, then returns %NULL.
 **/
GtkSettings*
__gtk_settings_get_default (void)
{
  GdkScreen *screen = __gdk_screen_get_default ();

  if (screen)
    return __gtk_settings_get_for_screen (screen);
  else
    return NULL;
}
  ...
  else
    {
	  ...
	  !__gdk_screen_get_setting (gtk_settings_get_props (settings)->screen, pspec->name, &val))
      ...
      else
        {
          ...
          if (parser && ___gtk_settings_parse_convert (parser, &gstring_value,
                                                     pspec, &tmp_value))
            {
		     ...
		     GParamSpec *pspec)
{
  ...
  switch (property_id)
    ...
    case PROP_XFT_DPI:
      settings_update_resolution (settings);
      /* This is a hack because with __gtk_rc_reset_styles() doesn't get
       * widgets with gtk_widget_style_set(), and also causes more
       * recomputation than necessary.
       */
      __gtk_rc_reset_styles (GTK_SETTINGS (object));
      break;
...
gboolean
___gtk_settings_parse_convert (GtkRcPropertyParser parser,
		      ...
		      GtkSettingsValuePrivate *qvalue)
{
  ...
  g_value_init (&tmp_value, G_PARAM_SPEC_VALUE_TYPE (pspec));
  if (___gtk_settings_parse_convert (parser, &qvalue->public.value,
				   pspec, &tmp_value))
    {
      if (pspec->param_id == PROP_COLOR_SCHEME) 
...
GtkRcPropertyParser
___gtk_rc_property_parser_from_type (GType type)
{
  if (type == GDK_TYPE_COLOR)
    return __gtk_rc_property_parse_color;
  else if (type == GTK_TYPE_REQUISITION)
    return __gtk_rc_property_parse_requisition;
  else if (type == GTK_TYPE_BORDER)
    return __gtk_rc_property_parse_border;
  else if (G_TYPE_FUNDAMENTAL (type) == G_TYPE_ENUM && G_TYPE_IS_DERIVED (type))
    return __gtk_rc_property_parse_enum;
  else if (G_TYPE_FUNDAMENTAL (type) == G_TYPE_FLAGS && G_TYPE_IS_DERIVED (type))
    return __gtk_rc_property_parse_flags;
  else
    return NULL;
}

void
__gtk_settings_install_property (GParamSpec *pspec)
{
...
void
__gtk_settings_install_property_parser (GParamSpec          *pspec,
				      GtkRcPropertyParser  parser)
{
  static GtkSettingsClass *klass = NULL;
...
static void
__gtk_settings_set_property_value_internal (GtkSettings            *settings,
...
void
__gtk_settings_set_property_value (GtkSettings            *settings,
				 const gchar            *prop_name,
				 const GtkSettingsValue *new_value)
{
  ...
  __gtk_settings_set_property_value_internal (settings, prop_name, new_value,
					    GTK_SETTINGS_SOURCE_APPLICATION);
}

void
___gtk_settings_set_property_value_from_rc (GtkSettings            *settings,
					  const gchar            *prop_name,
					  const GtkSettingsValue *new_value)
{
  g_return_if_fail (GTK_SETTINGS (settings));
...
void
__gtk_settings_set_string_property (GtkSettings *settings,
				  const gchar *name,
				  ...
				  const gchar *origin)
{
  ...
  __gtk_settings_set_property_value (settings, name, &svalue);
  g_value_unset (&svalue.value);
}

void
__gtk_settings_set_long_property (GtkSettings *settings,
				const gchar *name,
				glong	     v_long,
				const gchar *origin)
{
...
void
__gtk_settings_set_double_property (GtkSettings *settings,
				  const gchar *name,
				  gdouble      v_double,
				  const gchar *origin)
{
...
/**
 * __gtk_rc_property_parse_color:
 * @pspec: a #GParamSpec
 * @gstring: the #GString to be parsed
 * @property_value: a #GValue which must hold #GdkColor values.
 * 
 * A #GtkRcPropertyParser for use with __gtk_settings_install_property_parser()
 * or __gtk_widget_class_install_style_property_parser() which parses a
 ...
 **/
gboolean
__gtk_rc_property_parse_color (const GParamSpec *pspec,
			     ...
			     GValue           *property_value)
{
  ...
  if (__gtk_rc_parse_color (scanner, &color) == G_TOKEN_NONE &&
 ...
 * 
 * A #GtkRcPropertyParser for use with __gtk_settings_install_property_parser()
 * or __gtk_widget_class_install_style_property_parser() which parses a single
 * enumeration value.
 *
 ...
 **/
gboolean
__gtk_rc_property_parse_enum (const GParamSpec *pspec,
			    const GString    *gstring,
			    GValue           *property_value)
{
...
/**
 * __gtk_rc_property_parse_flags:
 * @pspec: a #GParamSpec
 * @gstring: the #GString to be parsed
 * @property_value: a #GValue which must hold flags values.
 * 
 * A #GtkRcPropertyParser for use with __gtk_settings_install_property_parser()
 * or __gtk_widget_class_install_style_property_parser() which parses flags. 
 * 
 ...
 **/
gboolean
__gtk_rc_property_parse_flags (const GParamSpec *pspec,
			     const GString    *gstring,
			     GValue           *property_value)
{
  GFlagsClass *class;
...
/**
 * __gtk_rc_property_parse_requisition:
 * @pspec: a #GParamSpec
 * @gstring: the #GString to be parsed
 ...
 **/
gboolean
__gtk_rc_property_parse_requisition  (const GParamSpec *pspec,
				    const GString    *gstring,
				    GValue           *property_value)
{
...
/**
 * __gtk_rc_property_parse_border:
 * @pspec: a #GParamSpec
 * @gstring: the #GString to be parsed
 * @property_value: a #GValue which must hold boxed values.
 * 
 * A #GtkRcPropertyParser for use with __gtk_settings_install_property_parser()
 * or __gtk_widget_class_install_style_property_parser() which parses
 * borders in the form 
 ...
 **/
gboolean
__gtk_rc_property_parse_border (const GParamSpec *pspec,
			      const GString    *gstring,
			      GValue           *property_value)
{
  GtkBorder border;
...
void
___gtk_settings_handle_event (GdkEventSetting *event)
{
  GtkSettings *settings;
  GParamSpec *pspec;
  guint property_id;

  settings = __gtk_settings_get_for_screen (__gdk_window_get_screen (event->window));
...
void
___gtk_settings_reset_rc_values (GtkSettings *settings)
{
  ...
  if (__gdk_screen_get_number (gtk_settings_get_props (settings)->screen) == 0)
    {
      ...
      __gdk_display_set_double_click_time (display, double_click_time);
      __gdk_display_set_double_click_distance (display, double_click_distance);
    }
}
...
settings_update_modules (GtkSettings *settings)
{
  ...
  ___gtk_modules_settings_changed (settings, modules);
...
settings_update_cursor_theme (GtkSettings *settings)
{
  ...
  __gdk_x11_display_set_cursor_theme (display, theme, size);
		   ...
		   GHashTable *target)
{
  ...
  if (!old || !__gdk_color_equal (old, color))
    {
      g_hash_table_insert (target, g_strdup (name), __gdk_color_copy (color));
