...
STLWRT_DEFINE_FTYPE_VPARENT (GtkRecentChooserDefault, _gtk_recent_chooser_default, GTK_TYPE_VBOX,
                             G_TYPE_FLAG_NONE,
...
{
  __gtk_box_set_spacing (GTK_BOX (impl), 6);
				        ...
				        GObjectConstructParam *params)
{
  ...
  scrollw = __gtk_scrolled_window_new (NULL, NULL);
  __gtk_scrolled_window_set_shadow_type (GTK_SCROLLED_WINDOW (scrollw),
  				       GTK_SHADOW_IN);
  __gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrollw),
  				  GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
  __gtk_box_pack_start (GTK_BOX (impl), scrollw, TRUE, TRUE, 0);
  __gtk_widget_show (scrollw);
  
  impl->recent_view = __gtk_tree_view_new ();
  __gtk_tree_view_set_headers_visible (GTK_TREE_VIEW (impl->recent_view), FALSE);
  ...
  __gtk_container_add (GTK_CONTAINER (scrollw), impl->recent_view);
  __gtk_widget_show (impl->recent_view);
  
  impl->icon_column = __gtk_tree_view_column_new ();
  __gtk_tree_view_column_set_expand (impl->icon_column, FALSE);
  __gtk_tree_view_column_set_resizable (impl->icon_column, FALSE);
  
  renderer = __gtk_cell_renderer_pixbuf_new ();
  __gtk_tree_view_column_pack_start (impl->icon_column, renderer, FALSE);
  __gtk_tree_view_column_set_cell_data_func (impl->icon_column,
  					   renderer,
  					   recent_icon_data_func,
  					   impl,
  					   NULL);
  __gtk_tree_view_append_column (GTK_TREE_VIEW (impl->recent_view),
                               impl->icon_column);
  
  impl->meta_column = __gtk_tree_view_column_new ();
  __gtk_tree_view_column_set_expand (impl->meta_column, TRUE);
  __gtk_tree_view_column_set_resizable (impl->meta_column, FALSE);
  
  impl->meta_renderer = __gtk_cell_renderer_text_new ();
  g_object_set (G_OBJECT (impl->meta_renderer),
                "ellipsize", PANGO_ELLIPSIZE_END,
                NULL);
  __gtk_tree_view_column_pack_start (impl->meta_column, impl->meta_renderer, TRUE);
  __gtk_tree_view_column_set_cell_data_func (impl->meta_column,
  					   impl->meta_renderer,
  ...
  impl->selection = __gtk_tree_view_get_selection (GTK_TREE_VIEW (impl->recent_view));
  __gtk_tree_selection_set_mode (impl->selection, GTK_SELECTION_SINGLE);
  g_signal_connect (impl->selection, "changed", G_CALLBACK (selection_changed_cb), impl);

  /* drag and drop */
  __gtk_drag_source_set (impl->recent_view,
		       GDK_BUTTON1_MASK,
		       NULL, 0,
		       GDK_ACTION_COPY);
  __gtk_drag_source_add_uri_targets (impl->recent_view);

  impl->filter_combo_hbox = __gtk_hbox_new (FALSE, 12);
  
  impl->filter_combo = __gtk_combo_box_text_new ();
  __gtk_combo_box_set_focus_on_click (GTK_COMBO_BOX (impl->filter_combo), FALSE);
  g_signal_connect (impl->filter_combo, "changed",
                    G_CALLBACK (filter_combo_changed_cb), impl);
  __gtk_widget_set_tooltip_text (impl->filter_combo,
		               _("Select which type of documents are shown"));
  
  __gtk_box_pack_end (GTK_BOX (impl->filter_combo_hbox),
                    impl->filter_combo,
                    FALSE, FALSE, 0);
  __gtk_widget_show (impl->filter_combo);
  
  __gtk_box_pack_end (GTK_BOX (impl), impl->filter_combo_hbox, FALSE, FALSE, 0);
  
  __gtk_widget_pop_composite_child ();
  
  impl->recent_store = __gtk_list_store_new (N_RECENT_COLUMNS,
  					   G_TYPE_STRING,       /* uri */
  ...
  switch (prop_id)
      ...
      if (impl->recent_popup_menu_show_private_item)
	{
	  ...
	  g_signal_handlers_block_by_func (item, G_CALLBACK (show_private_toggled_cb), impl);
          __gtk_check_menu_item_set_active (item, impl->show_private);
    ...
    case GTK_RECENT_CHOOSER_PROP_SHOW_ICONS:
      ...
      __gtk_tree_view_column_set_visible (impl->icon_column, impl->show_icons);
      break;
    case GTK_RECENT_CHOOSER_PROP_SELECT_MULTIPLE:
      impl->select_multiple = g_value_get_boolean (value);
      
      if (impl->select_multiple)
        __gtk_tree_selection_set_mode (impl->selection, GTK_SELECTION_MULTIPLE);
      else
        __gtk_tree_selection_set_mode (impl->selection, GTK_SELECTION_SINGLE);
      break;
...
gtk_recent_chooser_default_dispose (GObject *object)
{
  ...
  if (impl->recent_items)
    {
      g_list_foreach (impl->recent_items, (GFunc) __gtk_recent_info_unref, NULL);
			   ...
			   const gchar *detail)
{
  ...
  dialog = __gtk_message_dialog_new (parent,
		 ...
		 gboolean                 show_busy_cursor)
{
  ...
  if (!toplevel || !__gtk_widget_get_realized (GTK_WIDGET (toplevel)))
    return;
  
  display = __gtk_widget_get_display (GTK_WIDGET (toplevel));
...
chooser_set_model (GtkRecentChooserDefault *impl)
{
  ...
  __gtk_tree_view_set_model (GTK_TREE_VIEW (impl->recent_view),
                           GTK_TREE_MODEL (impl->recent_store));
  __gtk_tree_view_columns_autosize (GTK_TREE_VIEW (impl->recent_view));
  __gtk_tree_view_set_enable_search (GTK_TREE_VIEW (impl->recent_view), TRUE);
  __gtk_tree_view_set_search_column (GTK_TREE_VIEW (impl->recent_view),
  				   RECENT_DISPLAY_NAME_COLUMN);
...
load_recent_items (gpointer user_data)
{
  ...
  if (!impl->recent_items)
    {
      impl->recent_items = __gtk_recent_chooser_get_items (GTK_RECENT_CHOOSER (impl));
      if (!impl->recent_items)
        {
	  impl->load_state = LOAD_FINISHED;
  ...
  uri = __gtk_recent_info_get_uri (info);
  name = __gtk_recent_info_get_display_name (info);
   ...
   * is actually loaded. */
  __gtk_list_store_append (impl->recent_store, &iter);
  __gtk_list_store_set (impl->recent_store, &iter,
...
reload_recent_items (GtkRecentChooserDefault *impl)
{
  ...
  __gtk_tree_view_set_model (GTK_TREE_VIEW (impl->recent_view), NULL);
  __gtk_list_store_clear (impl->recent_store);
  ...
  impl->load_state = LOAD_EMPTY;
  impl->load_id = __gdk_threads_add_idle_full (G_PRIORITY_HIGH_IDLE + 30,
                                             load_recent_items,
                                             impl,
                                             cleanup_after_load);
}
...
set_default_size (GtkRecentChooserDefault *impl)
{
  ...
  __gtk_widget_size_request (widget, &req);
  ...
  /* Set size */
  __gtk_widget_set_size_request (impl->recent_view, width, height);
}
		       ...
		       gpointer           user_data)
{
  ...
  pixbuf = __gtk_recent_info_get_icon (info, impl->icon_size);
		       ...
		       gpointer           user_data)
{
  ...
  if (!name)
    name = __gtk_recent_info_get_short_name (info);
  ...
  if (!impl->select_multiple)
    {
      ...
      if (!__gtk_tree_selection_get_selected (impl->selection, &model, &iter))
        return NULL;
      
      __gtk_tree_model_get (model, &iter, RECENT_URI_COLUMN, &uri, -1);
  ...
  if (strcmp (uri, select_data->uri) == 0)
    {
      ...
      if (select_data->do_activate)
        __gtk_tree_view_row_activated (GTK_TREE_VIEW (select_data->impl->recent_view),
        			     path,
        			     select_data->impl->meta_column);
      
      if (select_data->do_select)
        __gtk_tree_selection_select_path (select_data->impl->selection, path);
      else
        __gtk_tree_selection_unselect_path (select_data->impl->selection, path);
					    ...
					    GError           **error)
{
  ...
  __gtk_tree_model_foreach (GTK_TREE_MODEL (impl->recent_store),
  			  scan_for_uri_cb,
  			  data);
...
gtk_recent_chooser_default_select_all (GtkRecentChooser *chooser)
{
  ...
  __gtk_tree_selection_select_all (impl->selection);
}
...
gtk_recent_chooser_default_unselect_all (GtkRecentChooser *chooser)
{
  ...
  __gtk_tree_selection_unselect_all (impl->selection);
}
...
{
  if (show)
    __gtk_widget_show (impl->filter_combo_hbox);
  else
    __gtk_widget_hide (impl->filter_combo_hbox);
}
				       ...
				       GtkRecentFilter  *filter)
{
  ...
  if (g_slist_find (impl->filters, filter))
    {
      g_warning ("__gtk_recent_chooser_add_filter() called on filter already in list\n");
      return;
    }
  ...
  /* display new filter */
  name = __gtk_recent_filter_get_name (filter);
  if (!name)
    name = _("Untitled filter");

  __gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (impl->filter_combo), name);
					  ...
					  GtkRecentFilter  *filter)
{
  ...
  if (filter_idx < 0)
    {
      g_warning ("__gtk_recent_chooser_remove_filter() called on filter not in list\n");
      return;  
    }
  ...
  model = __gtk_combo_box_get_model (GTK_COMBO_BOX (impl->filter_combo));
  __gtk_tree_model_iter_nth_child (model, &iter, NULL, filter_idx);
  __gtk_list_store_remove (GTK_LIST_STORE (model), &iter);
  ...
  if (impl->current_filter != filter)
    {
      ...
      if (impl->filters)
        __gtk_combo_box_set_active (GTK_COMBO_BOX (impl->filter_combo),
                                  filter_idx);
...
get_icon_theme_for_widget (GtkWidget *widget)
{
  if (__gtk_widget_has_screen (widget))
    return __gtk_icon_theme_get_for_screen (__gtk_widget_get_screen (widget));

  return __gtk_icon_theme_get_default ();
}
			  ...
			  GtkIconSize  icon_size)
{
  ...
  if (__gtk_widget_has_screen (widget))
    settings = __gtk_settings_get_for_screen (__gtk_widget_get_screen (widget));
  else
    settings = __gtk_settings_get_default ();

  if (__gtk_icon_size_lookup_for_settings (settings, icon_size,
                                         &width, &height))
			 ...
			 gpointer     user_data)
{
  ...
  new_index = __gtk_combo_box_get_active (combo_box);
...
get_drag_pixbuf (GtkRecentChooserDefault *impl)
{
  ...
  info = __gtk_recent_chooser_get_current_item (GTK_RECENT_CHOOSER (impl));
  if (!info)
    return NULL;

  size = get_icon_size_for_widget (GTK_WIDGET (impl), GTK_ICON_SIZE_DND);

  retval = __gtk_recent_info_get_icon (info, size);
  __gtk_recent_info_unref (info);
			   ...
			   gpointer        user_data)
{
  ...
  if (pixbuf)
    {
      __gtk_drag_set_icon_pixbuf (context, pixbuf, 0, 0);
      g_object_unref (pixbuf);
    }
  else
		       ...
		       gpointer      user_data)
{
  ...
  __gtk_tree_model_filter_convert_iter_to_child_iter (GTK_TREE_MODEL_FILTER (model),
  						    &child_iter,
  						    iter);
  __gtk_tree_model_get (child_model, &child_iter,
			      ...
			      gpointer          data)
{
  ...
  n_uris = __gtk_tree_selection_count_selected_rows (impl->selection);
  if (n_uris == 0)
    return;
  ...
  __gtk_tree_selection_selected_foreach (impl->selection,
      				       append_uri_to_urilist,
      				       &drag_data);
  
  __gtk_selection_data_set_uris (selection_data, drag_data.uri_list);
                              ...
                              gpointer    user_data)
{
  ...
  __gtk_tree_view_get_tooltip_context (tree_view,
                                     &x, &y,
                                     keyboard_tip,
                                     NULL, &path, NULL);
  if (!path)
  ...
  if (!__gtk_tree_model_get_iter (GTK_TREE_MODEL (impl->recent_store), &iter, path))
    {
      __gtk_tree_path_free (path);
      return FALSE;
    }

  __gtk_tree_model_get (GTK_TREE_MODEL (impl->recent_store), &iter,
                      RECENT_INFO_COLUMN, &info,
                      -1);

  uri_display = __gtk_recent_info_get_uri_display (info);
  
  __gtk_tooltip_set_text (tooltip, uri_display);
...
remove_selected_from_list (GtkRecentChooserDefault *impl)
{
  ...
  uri = __gtk_recent_chooser_get_current_uri (GTK_RECENT_CHOOSER (impl));
  if (!uri)
    return;
  
  err = NULL;
  if (!__gtk_recent_manager_remove_item (impl->manager, uri, &err))
    {
      gchar *msg;
		   ...
		   gpointer     user_data)
{
  ...
  utf8_uri = __gtk_recent_info_get_uri_display (info);
  
  __gtk_clipboard_set_text (__gtk_widget_get_clipboard (GTK_WIDGET (impl),
			 ...
			 gpointer     user_data)
{
  ...
  __gtk_recent_manager_purge_items (impl->manager, &err);
  if (err)
    {
       gchar *msg;
			 ...
			 gpointer          user_data)
{
  ...
  g_object_set (G_OBJECT (impl),
  		"show-private", __gtk_check_menu_item_get_active (menu_item),
  		NULL);
}
...
recent_view_menu_ensure_state (GtkRecentChooserDefault *impl)
{
  ...
  if (count == 0)
    {
      __gtk_widget_set_sensitive (impl->recent_popup_menu_remove_item, FALSE);
      __gtk_widget_set_sensitive (impl->recent_popup_menu_copy_item, FALSE);
      __gtk_widget_set_sensitive (impl->recent_popup_menu_clear_item, FALSE);
      __gtk_widget_set_sensitive (impl->recent_popup_menu_show_private_item, FALSE);
    }
}
...
recent_view_menu_build (GtkRecentChooserDefault *impl)
{
  ...
  impl->recent_popup_menu = __gtk_menu_new ();
  __gtk_menu_attach_to_widget (GTK_MENU (impl->recent_popup_menu),
  			     impl->recent_view,
  			     recent_popup_menu_detach_cb);
  
  item = __gtk_image_menu_item_new_with_mnemonic (_("Copy _Location"));
  impl->recent_popup_menu_copy_item = item;
  __gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (item),
		  		 __gtk_image_new_from_stock (GTK_STOCK_COPY, GTK_ICON_SIZE_MENU));
  g_signal_connect (item, "activate",
		    G_CALLBACK (copy_activated_cb), impl);
  __gtk_widget_show (item);
  __gtk_menu_shell_append (GTK_MENU_SHELL (impl->recent_popup_menu), item);

  item = __gtk_separator_menu_item_new ();
  __gtk_widget_show (item);
  __gtk_menu_shell_append (GTK_MENU_SHELL (impl->recent_popup_menu), item);
  
  item = __gtk_image_menu_item_new_with_mnemonic (_("_Remove From List"));
  impl->recent_popup_menu_remove_item = item;
  __gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (item),
  				 __gtk_image_new_from_stock (GTK_STOCK_REMOVE, GTK_ICON_SIZE_MENU));
  g_signal_connect (item, "activate",
  		    G_CALLBACK (remove_item_activated_cb), impl);
  __gtk_widget_show (item);
  __gtk_menu_shell_append (GTK_MENU_SHELL (impl->recent_popup_menu), item);

  item = __gtk_image_menu_item_new_with_mnemonic (_("_Clear List"));
  impl->recent_popup_menu_clear_item = item;
  __gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (item),
  				 __gtk_image_new_from_stock (GTK_STOCK_CLEAR, GTK_ICON_SIZE_MENU));
  g_signal_connect (item, "activate",
  ...
  item = __gtk_check_menu_item_new_with_mnemonic (_("Show _Private Resources"));
  impl->recent_popup_menu_show_private_item = item;
  __gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (item), impl->show_private);
			...
			GdkEventButton          *event)
{
  ...
  if (event)
    __gtk_menu_popup (GTK_MENU (impl->recent_popup_menu),
    		    NULL, NULL, NULL, NULL,
  ...
  else
    {
      __gtk_menu_popup (GTK_MENU (impl->recent_popup_menu),
      		      ...
      		      0, GDK_CURRENT_TIME);
      __gtk_menu_shell_select_first (GTK_MENU_SHELL (impl->recent_popup_menu),
      				   FALSE);
    }
}
  ...
  if (_gtk_button_event_triggers_context_menu (event))
    {
      ...
      if (event->window != __gtk_tree_view_get_bin_window (GTK_TREE_VIEW (impl->recent_view)))
        return FALSE;

      res = __gtk_tree_view_get_path_at_pos (GTK_TREE_VIEW (impl->recent_view),
		      			   event->x, event->y,
					   &path,
					   NULL, NULL, NULL);
      if (!res)
      ...
      __gtk_tree_selection_select_path (impl->selection, path);
      __gtk_tree_path_free (path);
		    ...
		    GtkRecentManager        *manager)
{
  ...
  if (manager)
    ...
    impl->manager = __gtk_recent_manager_get_default ();
