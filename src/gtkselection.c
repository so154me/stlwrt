...
struct _GtkSelectionInfo
{
  ...
  GdkDisplay	*display;	/* needed in __gtk_selection_remove_all */    
};
 ...
 **/
GtkTargetList *
__gtk_target_list_new (const GtkTargetEntry *targets,
		     guint                 ntargets)
{
  ...
  if (targets)
    __gtk_target_list_add_table (result, targets, ntargets);
  
  return result;
}

/**
 * __gtk_target_list_ref:
 ...
 **/
GtkTargetList *
__gtk_target_list_ref (GtkTargetList *list)
{
 ...
 **/
void               
__gtk_target_list_unref (GtkTargetList *list)
{
 ...
 **/
void 
__gtk_target_list_add (GtkTargetList *list,
		     GdkAtom        target,
		     guint          flags,
		     guint          info)
{
...
init_atoms (void)
{
  ...
  if (!utf8_atom)
    {
      utf8_atom = __gdk_atom_intern_static_string ("UTF8_STRING");
      text_atom = __gdk_atom_intern_static_string ("TEXT");
      ctext_atom = __gdk_atom_intern_static_string ("COMPOUND_TEXT");
      text_plain_atom = __gdk_atom_intern_static_string ("text/plain");
      text_plain_utf8_atom = __gdk_atom_intern_static_string ("text/plain;charset=utf-8");
      g_get_charset (&charset);
      tmp = g_strdup_printf ("text/plain;charset=%s", charset);
      text_plain_locale_atom = __gdk_atom_intern (tmp, FALSE);
      g_free (tmp);

      text_uri_list_atom = __gdk_atom_intern_static_string ("text/uri-list");
    }
}

/**
 * __gtk_target_list_add_text_targets:
 * @list: a #GtkTargetList
 * @info: an ID that will be passed back to the application
 * 
 ...
 **/
void 
__gtk_target_list_add_text_targets (GtkTargetList *list,
				  guint          info)
{
  g_return_if_fail (list != NULL);
  ...
  /* Keep in sync with __gtk_selection_data_targets_include_text()
   */
  __gtk_target_list_add (list, utf8_atom, 0, info);  
  __gtk_target_list_add (list, ctext_atom, 0, info);  
  __gtk_target_list_add (list, text_atom, 0, info);  
  __gtk_target_list_add (list, GDK_TARGET_STRING, 0, info);  
  __gtk_target_list_add (list, text_plain_utf8_atom, 0, info);  
  if (!g_get_charset (NULL))
    __gtk_target_list_add (list, text_plain_locale_atom, 0, info);  
  __gtk_target_list_add (list, text_plain_atom, 0, info);  
}

/**
 * __gtk_target_list_add_rich_text_targets:
 * @list: a #GtkTargetList
 * @info: an ID that will be passed back to the application
 ...
 * __gtk_text_buffer_register_serialize_format() or
 * __gtk_text_buffer_register_deserialize_format() to the target list. All
 * targets are added with the same @info.
 *
 * Since: 2.10
 **/
void
__gtk_target_list_add_rich_text_targets (GtkTargetList  *list,
                                       guint           info,
                                       ...
                                       GtkTextBuffer  *buffer)
{
  ...
  if (deserializable)
    ...
    atoms = __gtk_text_buffer_get_serialize_formats (buffer, &n_atoms);

  for (i = 0; i < n_atoms; i++)
    __gtk_target_list_add (list, atoms[i], 0, info);

  g_free (atoms);
}

/**
 * __gtk_target_list_add_image_targets:
 * @list: a #GtkTargetList
 * @info: an ID that will be passed back to the application
 ...
 **/
void 
__gtk_target_list_add_image_targets (GtkTargetList *list,
				   guint          info,
				   gboolean       writable)
{
  GSList *formats, *f;
  ...
  for (f = formats; f; f = f->next)
	...
	continue;
      ...
      for (m = mimes; *m; m++)
	{
	  atom = __gdk_atom_intern (*m, FALSE);
	  __gtk_target_list_add (list, atom, 0, info);  
	}
      g_strfreev (mimes);
    }
...
/**
 * __gtk_target_list_add_uri_targets:
 * @list: a #GtkTargetList
 * @info: an ID that will be passed back to the application
 * 
 ...
 **/
void 
__gtk_target_list_add_uri_targets (GtkTargetList *list,
				 guint          info)
{
  g_return_if_fail (list != NULL);
  ...
  __gtk_target_list_add (list, text_uri_list_atom, 0, info);  
}

/**
 * __gtk_target_list_add_table:
 * @list: a #GtkTargetList
 ...
 **/
void               
__gtk_target_list_add_table (GtkTargetList        *list,
  ...
  for (i=ntargets-1; i >= 0; i--)
    {
      ...
      pair->target = __gdk_atom_intern (targets[i].target, FALSE);
 ...
 **/
void 
__gtk_target_list_remove (GtkTargetList *list,
			GdkAtom            target)
{
  GList *tmp_list;
 ...
 **/
gboolean
__gtk_target_list_find (GtkTargetList *list,
		      GdkAtom        target,
		      guint         *info)
{
  GList *tmp_list;
...
/**
 * __gtk_target_table_new_from_list:
 * @list: a #GtkTargetList
 * @n_targets: return location for the number ot targets in the table
 *
 * This function creates an #GtkTargetEntry array that contains the
 * same targets as the passed %list. The returned table is newly
 * allocated and should be freed using __gtk_target_table_free() when no
 * longer needed.
 *
 ...
 **/
GtkTargetEntry *
__gtk_target_table_new_from_list (GtkTargetList *list,
                                gint          *n_targets)
{
  ...
  *n_targets = g_list_length (list->list);
  ...
  for (i = 0, tmp_list = list->list;
       ...
       i++, tmp_list = g_list_next (tmp_list))
    {
      ...
      targets[i].target = __gdk_atom_name (pair->target);
 ...
 **/
void
__gtk_target_table_free (GtkTargetEntry *targets,
                       gint            n_targets)
{
  gint i;
...
/**
 * __gtk_selection_owner_set_for_display:
 ...
 */
gboolean
__gtk_selection_owner_set_for_display (GdkDisplay   *display,
				     GtkWidget    *widget,
				     GdkAtom       selection,
				     guint32       time)
{
  ...
  g_return_val_if_fail (widget == NULL || __gtk_widget_get_realized (widget), FALSE);
  g_return_val_if_fail (widget == NULL || __gtk_widget_get_display (widget) == display, FALSE);
  ...
  if (__gdk_selection_owner_set_for_display (display, window, selection, time, TRUE))
    {
      old_owner = NULL;
      ...
      if (old_owner && old_owner != widget)
	{
	  ...
	  __gtk_widget_event (old_owner, event);
 ...
 **/
gboolean
__gtk_selection_owner_set (GtkWidget *widget,
			 GdkAtom    selection,
			 guint32    time)
{
  GdkDisplay *display;
  ...
  return __gtk_selection_owner_set_for_display (display, widget,
					      selection, time);
}
			       ...
			       GdkAtom       selection)
{
  ...
  sellist->list = __gtk_target_list_new (NULL, 0);
  ...
  while (tmp_list)
    {
      ...
      __gtk_target_list_unref (sellist->list);
 ...
 **/
void 
__gtk_selection_clear_targets (GtkWidget *widget,
			     GdkAtom    selection)
{
  GtkSelectionTargetList *sellist;
  GList *tmp_list;
 ...
 **/
void 
__gtk_selection_add_target (GtkWidget	    *widget, 
			  GdkAtom	     selection,
			  GdkAtom	     target,
			  guint              info)
{
 ...
 **/
void 
__gtk_selection_add_targets (GtkWidget            *widget, 
			   GdkAtom               selection,
			   const GtkTargetEntry *targets,
			   guint                 ntargets)
{
  ...
  list = gtk_selection_target_list_get (widget, selection);
  __gtk_target_list_add_table (list, targets, ntargets);
 ...
 **/
void
__gtk_selection_remove_all (GtkWidget *widget)
{
  GList *tmp_list;
  GList *next;
  ...
  while (tmp_list)
	...
	{	
	  __gdk_selection_owner_set_for_display (selection_info->display,
					       NULL, 
 ...
 **/
gboolean
__gtk_selection_convert (GtkWidget *widget, 
		       GdkAtom	  selection, 
		       GdkAtom	  target,
		       guint32	  time_)
{
  ...
  display = __gtk_widget_get_display (widget);
  owner_window = __gdk_selection_owner_get_for_display (display, selection);
  ...
  if (owner_window != NULL)
    {
      ...
      __gdk_window_get_user_data (owner_window, &owner_widget_ptr);
...
/**
 * __gtk_selection_data_get_selection:
 ...
 **/
GdkAtom
__gtk_selection_data_get_selection (GtkSelectionData *selection_data)
{
...
/**
 * __gtk_selection_data_get_target:
 * @selection_data: a pointer to a #GtkSelectionData structure.
 *
 ...
 **/
GdkAtom
__gtk_selection_data_get_target (GtkSelectionData *selection_data)
{
  g_return_val_if_fail (selection_data != NULL, 0);
...
/**
 * __gtk_selection_data_get_data_type:
 * @selection_data: a pointer to a #GtkSelectionData structure.
 *
 ...
 **/
GdkAtom
__gtk_selection_data_get_data_type (GtkSelectionData *selection_data)
{
  g_return_val_if_fail (selection_data != NULL, 0);
...
/**
 * __gtk_selection_data_get_format:
 * @selection_data: a pointer to a #GtkSelectionData structure.
 *
 ...
 **/
gint
__gtk_selection_data_get_format (GtkSelectionData *selection_data)
{
  g_return_val_if_fail (selection_data != NULL, 0);
 ...
 **/
const guchar*
__gtk_selection_data_get_data (GtkSelectionData *selection_data)
{
...
/**
 * __gtk_selection_data_get_length:
 * @selection_data: a pointer to a #GtkSelectionData structure.
 *
 ...
 */
gint
__gtk_selection_data_get_length (GtkSelectionData *selection_data)
{
...
/**
 * __gtk_selection_data_get_display:
 * @selection_data: a pointer to a #GtkSelectionData structure.
 *
 ...
 **/
GdkDisplay *
__gtk_selection_data_get_display (GtkSelectionData *selection_data)
{
  g_return_val_if_fail (selection_data != NULL, NULL);
 ...
 **/
void 
__gtk_selection_data_set (GtkSelectionData *selection_data,
			GdkAtom		  type,
			gint		  format,
			const guchar	 *data,
			gint		  length)
		      ...
		      gint              len)
{
  ...
  gchar *latin1 = __gdk_utf8_to_string_target (tmp);
  g_free (tmp);
			     ...
			     gint              len)
{
  ...
  if (__gdk_x11_display_utf8_to_compound_text (selection_data->display, tmp,
                                             &encoding, &format, &text, &new_length))
    {
      __gtk_selection_data_set (selection_data, encoding, format, text, new_length);
      __gdk_x11_free_compound_text (text);
 ...
 **/
gboolean
__gtk_selection_data_set_text (GtkSelectionData     *selection_data,
			     const gchar          *str,
			     gint                  len)
{
 ...
 **/
guchar *
__gtk_selection_data_get_text (GtkSelectionData *selection_data)
{
  guchar *result = NULL;
  ...
  if (selection_data->length >= 0 &&
       ...
       selection_data->type == utf8_atom))
    {
      gchar **list;
      gint i;
      gint count = __gdk_text_property_to_utf8_list_for_display (selection_data->display,
      							       selection_data->type,
						   	       selection_data->format, 
						               selection_data->data,
						               selection_data->length,
...
/**
 * __gtk_selection_data_set_pixbuf:
 * @selection_data: a #GtkSelectionData
 * @pixbuf: a #GdkPixbuf
 * 
 ...
 **/
gboolean
__gtk_selection_data_set_pixbuf (GtkSelectionData *selection_data,
			       GdkPixbuf        *pixbuf)
{
  GSList *formats, *f;
  gchar **mimes, **m;
...
/**
 * __gtk_selection_data_get_pixbuf:
 * @selection_data: a #GtkSelectionData
 * 
 ...
 **/
GdkPixbuf *
__gtk_selection_data_get_pixbuf (GtkSelectionData *selection_data)
{
 ...
 **/
gboolean
__gtk_selection_data_set_uris (GtkSelectionData  *selection_data,
			     gchar            **uris)
{
  g_return_val_if_fail (selection_data != NULL, FALSE);
 ...
 **/
gchar **
__gtk_selection_data_get_uris (GtkSelectionData *selection_data)
{
  gchar **result = NULL;
...
/**
 * __gtk_selection_data_get_targets:
 ...
 **/
gboolean
__gtk_selection_data_get_targets (GtkSelectionData  *selection_data,
				GdkAtom          **targets,
				gint              *n_atoms)
{
  g_return_val_if_fail (selection_data != NULL, FALSE);
 ...
 **/
gboolean 
__gtk_targets_include_text (GdkAtom *targets,
                          gint     n_targets)
{
  gint i;
  gboolean result = FALSE;
  ...
  /* Keep in sync with __gtk_target_list_add_text_targets()
   */
...
/**
 * __gtk_targets_include_rich_text:
 * @targets: (array length=n_targets): an array of #GdkAtom<!-- -->s
 * @n_targets: the length of @targets
 * @buffer: a #GtkTextBuffer
 *
 ...
 **/
gboolean
__gtk_targets_include_rich_text (GdkAtom       *targets,
                               gint           n_targets,
                               GtkTextBuffer *buffer)
{
  ...
  rich_targets = __gtk_text_buffer_get_deserialize_formats (buffer,
                                                          &n_rich_targets);
...
/**
 * __gtk_selection_data_targets_include_text:
 * @selection_data: a #GtkSelectionData object
 * 
 ...
 **/
gboolean
__gtk_selection_data_targets_include_text (GtkSelectionData *selection_data)
{
  GdkAtom *targets;
  gint n_targets;
  gboolean result = FALSE;
  ...
  if (__gtk_selection_data_get_targets (selection_data, &targets, &n_targets))
    {
      result = __gtk_targets_include_text (targets, n_targets);
      g_free (targets);
    }
...
/**
 * __gtk_selection_data_targets_include_rich_text:
 * @selection_data: a #GtkSelectionData object
 * @buffer: a #GtkTextBuffer
 *
 * Given a #GtkSelectionData object holding a list of targets,
 ...
 **/
gboolean
__gtk_selection_data_targets_include_rich_text (GtkSelectionData *selection_data,
                                              GtkTextBuffer    *buffer)
{
  GdkAtom *targets;
  gint n_targets;
  ...
  if (__gtk_selection_data_get_targets (selection_data, &targets, &n_targets))
    {
      result = __gtk_targets_include_rich_text (targets, n_targets, buffer);
      g_free (targets);
    }
 ...
 **/
gboolean 
__gtk_targets_include_image (GdkAtom *targets,
			   gint     n_targets,
			   gboolean writable)
{
  GtkTargetList *list;
  ...
  __gtk_target_list_add_image_targets (list, 0, writable);
...
/**
 * __gtk_selection_data_targets_include_image:
 * @selection_data: a #GtkSelectionData object
 * @writable: whether to accept only targets for which GTK+ knows
 *   how to convert a pixbuf into the format
 * 
 ...
 **/
gboolean 
__gtk_selection_data_targets_include_image (GtkSelectionData *selection_data,
					  gboolean          writable)
{
  GdkAtom *targets;
  gint n_targets;
  ...
  if (__gtk_selection_data_get_targets (selection_data, &targets, &n_targets))
    {
      result = __gtk_targets_include_image (targets, n_targets, writable);
      g_free (targets);
    }
 ...
 **/
gboolean 
__gtk_targets_include_uri (GdkAtom *targets,
			 gint     n_targets)
{
  gint i;
  gboolean result = FALSE;
  ...
  /* Keep in sync with __gtk_target_list_add_uri_targets()
   */
...
/**
 * __gtk_selection_data_targets_include_uri:
 * @selection_data: a #GtkSelectionData object
 * 
 * Given a #GtkSelectionData object holding a list of targets,
 * determines if any of the targets in @targets can be used to
 ...
 **/
gboolean
__gtk_selection_data_targets_include_uri (GtkSelectionData *selection_data)
{
  GdkAtom *targets;
  gint n_targets;
  gboolean result = FALSE;
  ...
  if (__gtk_selection_data_get_targets (selection_data, &targets, &n_targets))
    {
      result = __gtk_targets_include_uri (targets, n_targets);
      g_free (targets);
    }
...
{
  gtk_selection_atoms[INCR] = __gdk_atom_intern_static_string ("INCR");
  gtk_selection_atoms[MULTIPLE] = __gdk_atom_intern_static_string ("MULTIPLE");
  gtk_selection_atoms[TIMESTAMP] = __gdk_atom_intern_static_string ("TIMESTAMP");
  gtk_selection_atoms[TARGETS] = __gdk_atom_intern_static_string ("TARGETS");
  gtk_selection_atoms[SAVE_TARGETS] = __gdk_atom_intern_static_string ("SAVE_TARGETS");

  initialize = FALSE;
}

/**
 * __gtk_selection_clear:
 * @widget: a #GtkWidget
 * @event: the event
 * 
 ...
 **/
gboolean
__gtk_selection_clear (GtkWidget         *widget,
...
gboolean
___gtk_selection_request (GtkWidget *widget,
			GdkEventSelection *event)
{
...
#ifdef GDK_WINDOWING_X11
  info->requestor = __gdk_x11_window_foreign_new_for_display (display, event->requestor);
  ...
  if (event->target == gtk_selection_atoms[MULTIPLE])
    {
      ...
      if (!__gdk_property_get (info->requestor, event->property, GDK_NONE, /* AnyPropertyType */
			     0, selection_max_size, FALSE,
			     &type, &format, &length, &mult_atoms))
	{
	  __gdk_selection_send_notify_for_display (display,
						 event->requestor, 
						 event->selection,
						 event->target, 
						 GDK_NONE, 
	  ...
	  type != __gdk_atom_intern_static_string ("ATOM_PAIR"))
	{
	  ...
	  for (i=0; i<info->num_conversions; i++)
	    {
	      info->conversions[i].target = __gdk_x11_xatom_to_atom_for_display (display,
									       ((glong *)mult_atoms)[2*i]);
	      info->conversions[i].property = __gdk_x11_xatom_to_atom_for_display (display,
  ...
  for (i=0; i<info->num_conversions; i++)
      ...
      if (data.length > selection_max_size)
	{
	  ...
	  __gdk_property_change (info->requestor, 
  ...
  if (info->num_incrs > 0)
    {
	 ...
	 exist */
      ...
      __gdk_window_set_events (info->requestor,
			     __gdk_window_get_events (info->requestor) |
			     GDK_PROPERTY_CHANGE_MASK);
      current_incrs = g_list_append (current_incrs, info);
      __gdk_threads_add_timeout (1000, (GSourceFunc) gtk_selection_incr_timeout, info);
    }
  ...
  if (event->target == gtk_selection_atoms[MULTIPLE])
      ...
      for (i = 0; i < info->num_conversions; i++)
	{
      ...
      __gdk_property_change (info->requestor, event->property,
			   __gdk_atom_intern_static_string ("ATOM_PAIR"), 32, 
			   GDK_PROP_MODE_REPLACE,
  ...
  if (info->num_conversions == 1 &&
      info->conversions[0].property == GDK_NONE)
    {
      ...
      __gdk_selection_send_notify_for_display (__gtk_widget_get_display (widget),
					     event->requestor, 
					     event->selection, 
					     event->target, 
					     GDK_NONE, 
...
gboolean
___gtk_selection_incr_event (GdkWindow	   *window,
			   GdkEventProperty *event)
{
  ...
  selection_max_size = GTK_SELECTION_MAX_SIZE (__gdk_window_get_display (window));
  ...
  for (i=0; i<info->num_conversions; i++)
	  ...
	  info->conversions[i].offset != -1)
	{
	  ...
	  __gdk_property_change (info->requestor, event->atom,
...
gboolean
___gtk_selection_notify (GtkWidget	       *widget,
		       GdkEventSelection *event)
{
  GList *tmp_list;
...
/*************************************************************
 * ___gtk_selection_property_notify:
...
gboolean
___gtk_selection_property_notify (GtkWidget	*widget,
				GdkEventProperty *event)
{
  GList *tmp_list;
  GtkRetrievalInfo *info = NULL;
  ...
  if ((event->state != GDK_PROPERTY_NEW_VALUE) ||  /* property was deleted */
      (event->atom != __gdk_atom_intern_static_string ("GDK_SELECTION"))) /* not the right property */
#endif
    return FALSE;
				...
				guint32 time)
{
  ...
  data.length = length;
  data.data = buffer;
  data.display = __gtk_widget_get_display (info->widget);
			      ...
			      guint             time)
{
  ...
  if (data->target != gtk_selection_atoms[SAVE_TARGETS] &&
      target_list &&
      __gtk_target_list_find (target_list, data->target, &info))
    {
			       ...
			       GtkSelectionData *data)
{
  if (data->target == gtk_selection_atoms[TIMESTAMP])
    ...
    {
      __gtk_selection_data_set (data,
			      __gdk_atom_intern_static_string ("NULL"),
			      32, NULL, 0);
    }
  else
    {
 ...
 **/
GtkSelectionData*
__gtk_selection_data_copy (GtkSelectionData *data)
{
 ...
 **/
void
__gtk_selection_data_free (GtkSelectionData *data)
{
