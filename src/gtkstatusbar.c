...
gtk_statusbar_init (GtkStatusbar *statusbar)
{
  ...
  __gtk_widget_set_redraw_on_allocate (GTK_WIDGET (box), TRUE);
...
/**
 * __gtk_statusbar_get_context_id:
 ...
 */
guint
__gtk_statusbar_get_context_id (GtkStatusbar *statusbar,
 ...
 *          __gtk_statusbar_remove().
 */
guint
__gtk_statusbar_push (GtkStatusbar *statusbar,
		    guint	  context_id,
		    const gchar  *text)
{
  GtkStatusbarMsg *msg;
 ...
 */
void
__gtk_statusbar_pop (GtkStatusbar *statusbar,
		   guint	 context_id)
{
  GtkStatusbarMsg *msg;
 ...
 * @statusbar: a #GtkStatusBar
 * @context_id: a context identifier
 * @message_id: a message identifier, as returned by __gtk_statusbar_push()
 *
 * Forces the removal of a message from a statusbar's stack. 
 * The exact @context_id and @message_id must be specified.
 */
void
__gtk_statusbar_remove (GtkStatusbar *statusbar,
		      guint	   context_id,
		      guint        message_id)
{
  GtkStatusbarMsg *msg;
 ...
 */
void
__gtk_statusbar_remove_all (GtkStatusbar *statusbar,
                          guint         context_id)
{
  GtkStatusbarMsg *msg;
  GSList *prev, *list;
...
/**
 * __gtk_statusbar_set_has_resize_grip:
 * @statusbar: a #GtkStatusBar
 ...
 */
void
__gtk_statusbar_set_has_resize_grip (GtkStatusbar *statusbar,
				   gboolean      setting)
{
  g_return_if_fail (GTK_IS_STATUSBAR (statusbar));
...
/**
 * __gtk_statusbar_get_has_resize_grip:
 * @statusbar: a #GtkStatusBar
 * 
 ...
 */
gboolean
__gtk_statusbar_get_has_resize_grip (GtkStatusbar *statusbar)
{
...
/**
 * __gtk_statusbar_get_message_area:
 * @statusbar: a #GtkStatusBar
 *
 ...
 */
GtkWidget*
__gtk_statusbar_get_message_area (GtkStatusbar *statusbar)
{
			    ...
			    GParamSpec   *pspec)
{
  ...
  switch (prop_id) 
    ...
    case PROP_HAS_RESIZE_GRIP:
      __gtk_statusbar_set_has_resize_grip (statusbar, g_value_get_boolean (value));
      break;
    default:
               ...
               GdkRectangle *rect)
{
  ...
  if (w > gtk_widget_get_props (widget)->allocation.width)
    w = gtk_widget_get_props (widget)->allocation.width;

  if (h > gtk_widget_get_props (widget)->allocation.height - gtk_widget_get_props (widget)->style->ythickness)
    h = gtk_widget_get_props (widget)->allocation.height - gtk_widget_get_props (widget)->style->ythickness;
  
  rect->width = w;
  rect->height = h;
  rect->y = gtk_widget_get_props (widget)->allocation.y + gtk_widget_get_props (widget)->allocation.height - h;

  if (__gtk_widget_get_direction (widget) == GTK_TEXT_DIR_LTR) 
    rect->x = gtk_widget_get_props (widget)->allocation.x + gtk_widget_get_props (widget)->allocation.width - w;
  else 
    rect->x = gtk_widget_get_props (widget)->allocation.x + gtk_widget_get_props (widget)->style->xthickness;
}
                            ...
                            GdkEventButton *event)
{
  ...
  if (event->button == 1)
    __gtk_window_begin_resize_drag (GTK_WINDOW (ancestor),
                                  edge,
                                  event->button,
                                  event->x_root, event->y_root,
                                  event->time);
  else if (event->button == 2)
    __gtk_window_begin_move_drag (GTK_WINDOW (ancestor),
                                event->button,
                                event->x_root, event->y_root,
                                event->time);
  else
  ...
  if (gtk_statusbar_get_props (statusbar)->has_resize_grip)
    {
      ...
      __gtk_paint_resize_grip (gtk_widget_get_props (widget)->style,
                              ...
                              */
                             rect.width - gtk_widget_get_props (widget)->style->xthickness,
                             rect.height - gtk_widget_get_props (widget)->style->ythickness);
    }
	  ...
	  if (gtk_widget_get_props (child)->allocation.width + rect.width > gtk_widget_get_props (gtk_statusbar_get_props (statusbar)->frame)->allocation.width)
	    {
	      ...
	      *allocation = gtk_widget_get_props (child)->allocation;
	      ...
	      if (__gtk_widget_get_direction (widget) == GTK_TEXT_DIR_RTL)
		allocation->x += gtk_widget_get_props (child)->allocation.width - allocation->width;

	      __gtk_widget_size_allocate (child, allocation);
	    }
	}
