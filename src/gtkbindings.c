...
/* --- defines --- */
#define	BINDING_MOD_MASK()	(__gtk_accelerator_get_default_mod_mask () | GDK_RELEASE_MASK)
...
{
  ___gtk_rc_free_widget_class_path (pspec->path);
  if (pspec->pspec)
  ...
  *params_p = params;
  ...
  for (i = 1; i < query->n_params + 1 && valid; i++)
	...
	case G_TYPE_LONG:
	  ...
	  break;
	case G_TYPE_STRING:
	  /* __gtk_rc_parse_flags/enum() has fancier parsing for this; we can't call
	   ...
	   * values in __gtk_rc_parse_flags(), this isn't very useful.
	   */
  ...
  for (sig = entry->signals; sig; sig = sig->next)
    {
      ...
      if (!signal_id)
	{
	  accelerator = __gtk_accelerator_name (entry->keyval, entry->modifiers);
 ...
 */
GtkBindingSet*
__gtk_binding_set_new (const gchar *set_name)
{
...
__gtk_binding_set_by_class (gpointer object_class)
{
  ...
  binding_set = __gtk_binding_set_new (g_type_name (G_OBJECT_CLASS_TYPE (class)));
  __gtk_binding_set_add_path (binding_set,
			    GTK_PATH_CLASS,
...
/**
 * __gtk_binding_set_find:
 * @set_name: unique binding set name
 *
 * Find a binding set by its globally unique name. The @set_name can
 * either be a name used for __gtk_binding_set_new() or the type name of
 * a class used in __gtk_binding_set_by_class().
 *
 * Return value: (transfer none): %NULL or the specified binding set
 */
GtkBindingSet*
__gtk_binding_set_find (const gchar *set_name)
{
  GSList *slist;
...
static void
__gtk_binding_entry_clear_internal (GtkBindingSet  *binding_set,
                                  guint           keyval,
                                  GdkModifierType modifiers)
{
  GtkBindingEntry *entry;
 ...
 *
 * Deprecated: 2.12: Use __gtk_binding_entry_add_signal() instead.
 */

/**
 * __gtk_binding_entry_clear:
 ...
 *
 * Deprecated: 2.12: Use __gtk_binding_entry_remove() instead.
 */
void
__gtk_binding_entry_clear (GtkBindingSet	*binding_set,
			 guint		 keyval,
			 GdkModifierType modifiers)
{
  g_return_if_fail (binding_set != NULL);

  __gtk_binding_entry_clear_internal (binding_set, keyval, modifiers);
}

/**
 * __gtk_binding_entry_skip:
 ...
 */
void
__gtk_binding_entry_skip (GtkBindingSet  *binding_set,
                        guint           keyval,
                        GdkModifierType modifiers)
{
  GtkBindingEntry *entry;
...
/**
 * __gtk_binding_entry_remove:
 * @binding_set: a #GtkBindingSet to remove an entry of
 * @keyval:      key value of binding to remove
 * @modifiers:   key modifier of binding to remove
 *
 * Remove a binding previously installed via
 * __gtk_binding_entry_add_signal() on @binding_set.
 */
void
__gtk_binding_entry_remove (GtkBindingSet	 *binding_set,
			  guint		  keyval,
			  GdkModifierType modifiers)
{
  GtkBindingEntry *entry;
...
/**
 * __gtk_binding_entry_add_signal:
 ...
 */
void
__gtk_binding_entry_add_signal (GtkBindingSet  *binding_set,
			      guint           keyval,
			      GdkModifierType modifiers,
			      const gchar    *signal_name,
			      guint           n_args,
			      ...)
{
  ...
  for (i = 0; i < n_args; i++)
	...
	case G_TYPE_STRING:
	  ...
	  if (!arg->d.string_data)
	    {
	      g_warning ("__gtk_binding_entry_add_signal(): type `%s' arg[%u] is `NULL'",
	  ...
	  break;
	default:
	  g_warning ("__gtk_binding_entry_add_signal(): unsupported type `%s' for arg[%u]",
  ...
  if (i == n_args || i == 0)
    {
      ...
      ___gtk_binding_entry_add_signall (binding_set, keyval, modifiers, signal_name, slist);
    }
 ...
 */
void
__gtk_binding_set_add_path (GtkBindingSet	     *binding_set,
			  ...
			  GtkPathPriorityType priority)
{
  ...
  if (path_type == GTK_PATH_WIDGET_CLASS)
    {
      pspec->pspec = NULL;
      pspec->path = ___gtk_rc_parse_widget_class_path (path_pattern);
    }
  else
    {
  ...
  for (slist = pspec_list; slist; slist = slist->next)
    {
      ...
      if (pspec->type == GTK_PATH_WIDGET_CLASS)
        {
          if (___gtk_rc_match_widget_class (pspec->path, path_length, path, path_reversed))
		       ...
		       GdkModifierType  modifiers)
{
  ...
  display = __gtk_widget_get_display (GTK_WIDGET (object));
  key_hash = binding_key_hash_for_keymap (__gdk_keymap_get_for_display (display));
  
  entries = _gtk_key_hash_lookup_keyval (key_hash, keyval, modifiers);

  handled = __gtk_bindings_activate_list (object, entries, is_release);
                             ...
                             GdkEventKey *event)
{
  ...
  handled = __gtk_bindings_activate_list (object, entries,
			...
			GtkBindingSet  *binding_set)
{
  ...
  if (scanner->token != (guint) G_TOKEN_STRING)
    return G_TOKEN_STRING;
  __gtk_accelerator_parse (scanner->value.v_string, &keyval, &modifiers);
  ...
  if (unbind)
    {
      __gtk_binding_entry_skip (binding_set, keyval, modifiers);
      return G_TOKEN_NONE;
    }
...
___gtk_binding_parse_binding (GScanner *scanner)
{
  ...
  binding_set = __gtk_binding_set_find (name);
  if (!binding_set)
    {
      binding_set = __gtk_binding_set_new (name);
