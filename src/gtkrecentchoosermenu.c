				     ...
				     GObjectConstructParam *params)
{
   ...
   */
  priv->placeholder = __gtk_menu_item_new_with_label (_("No items found"));
  __gtk_widget_set_sensitive (priv->placeholder, FALSE);
  g_object_set_data (G_OBJECT (priv->placeholder),
                     "gtk-recent-menu-placeholder",
                     GINT_TO_POINTER (TRUE));

  __gtk_menu_shell_insert (GTK_MENU_SHELL (menu), priv->placeholder, 0);
  __gtk_widget_set_no_show_all (priv->placeholder, TRUE);
  __gtk_widget_show (priv->placeholder);
  ...
  for (l = children; l != NULL; l = l->next)
    {
      ...
      if (strcmp (uri, __gtk_recent_info_get_uri (info)) == 0)
        {
          __gtk_menu_shell_activate_item (GTK_MENU_SHELL (menu),
	                                menu_item,
					TRUE);
	  found = TRUE;
...
gtk_recent_chooser_menu_get_current_uri (GtkRecentChooser  *chooser)
{
  ...
  menu_item = __gtk_menu_get_active (GTK_MENU (menu));
  if (!menu_item)
    return NULL;
  ...
  return g_strdup (__gtk_recent_info_get_uri (info));
}
				    ...
				    GError           **error)
{
  ...
  for (l = children; l != NULL; l = l->next)
    {
      ...
      if (0 == strcmp (uri, __gtk_recent_info_get_uri (info)))
        found = TRUE;
    }
  ...
  if (!found)  
    ...
    {
      __gtk_menu_shell_select_item (GTK_MENU_SHELL (menu), menu_item);
				       ...
				       const gchar     *uri)
{
  ...
  __gtk_menu_shell_deselect (GTK_MENU_SHELL (menu));
}
  ...
  if (path)
    {
      ...
      __gtk_widget_set_tooltip_text (item, tip_text);
      __gtk_widget_set_has_tooltip (item, priv->show_tips);
  ...
  if (priv->show_numbers)
    {
      ...
      name = g_strdup (__gtk_recent_info_get_display_name (info));
      if (!name)
      ...
      item = __gtk_image_menu_item_new_with_mnemonic (text);
    ...
    {
      text = g_strdup (__gtk_recent_info_get_display_name (info));
      item = __gtk_image_menu_item_new_with_label (text);
    }

  g_free (text);

  __gtk_image_menu_item_set_always_show_image (GTK_IMAGE_MENU_ITEM (item),
                                             TRUE);
  ...
  if (GTK_IS_LABEL (label))
    {
      ...
      __gtk_label_set_max_width_chars (GTK_LABEL (label), priv->label_width);
    }
  
  if (priv->show_icons)
    {
      icon = __gtk_recent_info_get_icon (info, priv->icon_size);
        
      image = __gtk_image_new_from_pixbuf (icon);
      __gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (item), image);
      g_object_unref (icon);
    }
                                     ...
                                     gint                  position)
{
  ...
  __gtk_menu_shell_insert (GTK_MENU_SHELL (menu), menuitem,
      ...
      if (has_mark)
        {
          ...
          __gtk_container_remove (GTK_CONTAINER (menu), menu_item);
        }
    }
...
idle_populate_func (gpointer data)
{
  ...
  if (!pdata->items)
    {
      pdata->items = __gtk_recent_chooser_get_items (GTK_RECENT_CHOOSER (pdata->menu));
      if (!pdata->items)
        {
          /* show the placeholder here */
          __gtk_widget_show (pdata->placeholder);
	  ...
	  return FALSE;
        ...
        __gtk_widget_hide (pdata->placeholder);
  ...
  g_object_set_data_full (G_OBJECT (item), "gtk-recent-info",
      			  __gtk_recent_info_ref (info),
      			  (GDestroyNotify) __gtk_recent_info_unref);
...
check_and_return:
  ...
  if (pdata->loaded_items == pdata->n_items)
    {
      g_list_foreach (pdata->items, (GFunc) __gtk_recent_info_unref, NULL);
...
gtk_recent_chooser_menu_populate (GtkRecentChooserMenu *menu)
{
  ...
  priv->populate_id = __gdk_threads_add_idle_full (G_PRIORITY_HIGH_IDLE + 30,
  					         idle_populate_func,
					         pdata,
		    ...
		    GtkRecentManager     *manager)
{
  ...
  if (manager)
    ...
    priv->manager = __gtk_recent_manager_get_default ();
...
get_icon_size_for_widget (GtkWidget *widget)
{
  ...
  if (__gtk_icon_size_lookup_for_settings (settings, GTK_ICON_SIZE_MENU,
                                         &width, &height))
                       ...
                       gpointer   user_data)
{
  ...
  if (has_mark)
    __gtk_widget_set_has_tooltip (widget, priv->show_tips);
}
				       ...
				       gboolean              show_tips)
{
  ...
  __gtk_container_foreach (GTK_CONTAINER (menu), foreach_set_shot_tips, menu);
}
 ...
 * __gtk_recent_chooser_menu_new_for_manager() function to know how to create
 ...
 */
GtkWidget *
__gtk_recent_chooser_menu_new (void)
{
...
/**
 * __gtk_recent_chooser_menu_new_for_manager:
 ...
 */
GtkWidget *
__gtk_recent_chooser_menu_new_for_manager (GtkRecentManager *manager)
{
...
/**
 * __gtk_recent_chooser_menu_get_show_numbers:
 * @menu: a #GtkRecentChooserMenu
 *
 * Returns the value set by __gtk_recent_chooser_menu_set_show_numbers().
 * 
 ...
 */
gboolean
__gtk_recent_chooser_menu_get_show_numbers (GtkRecentChooserMenu *menu)
{
...
/**
 * __gtk_recent_chooser_menu_set_show_numbers:
 * @menu: a #GtkRecentChooserMenu
 ...
 */
void
__gtk_recent_chooser_menu_set_show_numbers (GtkRecentChooserMenu *menu,
