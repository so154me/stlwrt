  ...
  if (data->uri_hits) 
    {
      ...
      __gdk_threads_add_idle (search_thread_add_hits_idle, hits);
    }
  data->uri_hits = NULL;
}


// have fun debugging this lmao
static GPrivate search_thread_data =  G_PRIVATE_INIT (g_free);
...
search_thread_func (gpointer user_data)
{
  ...
  g_private_set (&search_thread_data, data, NULL);
  ...
  __gdk_threads_add_idle (search_thread_done_idle, data);
#endif /* HAVE_FTW_H */
