...
struct _GtkRcFile
{
  ...
  gchar *directory;
  guint  reload    : 1;
  guint  is_string : 1;	/* If TRUE, name is a string to parse with __gtk_rc_parse_string() */
};
...
static void        gtk_rc_context_parse_file         (GtkRcContext    *context,
						      ...
						      gint             priority,
                                                      gboolean         reload);
static void        __gtk_rc_parse_any                  (GtkRcContext    *context,
						      const gchar     *input_name,
                                                      gint             input_fd,
                                                      const gchar     *input_string);
static guint       __gtk_rc_parse_statement            (GtkRcContext    *context,
						      GScanner        *scanner);
static guint       __gtk_rc_parse_style                (GtkRcContext    *context,
						      GScanner        *scanner);
static guint       __gtk_rc_parse_assignment           (GScanner        *scanner,
                                                      GtkRcStyle      *style,
						      GtkRcProperty   *prop);
static guint       __gtk_rc_parse_bg                   (GScanner        *scanner,
                                                      GtkRcStyle      *style);
static guint       __gtk_rc_parse_fg                   (GScanner        *scanner,
                                                      GtkRcStyle      *style);
static guint       __gtk_rc_parse_text                 (GScanner        *scanner,
                                                      GtkRcStyle      *style);
static guint       __gtk_rc_parse_base                 (GScanner        *scanner,
                                                      GtkRcStyle      *style);
static guint       __gtk_rc_parse_xthickness           (GScanner        *scanner,
                                                      GtkRcStyle      *style);
static guint       __gtk_rc_parse_ythickness           (GScanner        *scanner,
                                                      GtkRcStyle      *style);
static guint       __gtk_rc_parse_bg_pixmap            (GtkRcContext    *context,
						      GScanner        *scanner,
                                                      GtkRcStyle      *rc_style);
static guint       __gtk_rc_parse_font                 (GScanner        *scanner,
                                                      GtkRcStyle      *rc_style);
static guint       __gtk_rc_parse_fontset              (GScanner        *scanner,
                                                      GtkRcStyle      *rc_style);
static guint       __gtk_rc_parse_font_name            (GScanner        *scanner,
                                                      GtkRcStyle      *rc_style);
static guint       __gtk_rc_parse_engine               (GtkRcContext    *context,
						      GScanner        *scanner,
                                                      GtkRcStyle     **rc_style);
static guint       __gtk_rc_parse_pixmap_path          (GtkRcContext    *context,
						      GScanner        *scanner);
static void        __gtk_rc_parse_pixmap_path_string   (GtkRcContext    *context,
						      GScanner        *scanner,
						      const gchar     *pix_path);
static guint       __gtk_rc_parse_module_path          (GScanner        *scanner);
static guint       __gtk_rc_parse_im_module_file       (GScanner        *scanner);
static guint       __gtk_rc_parse_path_pattern         (GtkRcContext    *context,
						      GScanner        *scanner);
static guint       __gtk_rc_parse_stock                (GtkRcContext    *context,
						      GScanner        *scanner,
                                                      GtkRcStyle      *rc_style,
                                                      GtkIconFactory  *factory);
static guint       __gtk_rc_parse_logical_color        (GScanner        *scanner,
                                                      GtkRcStyle      *rc_style,
                                                      GHashTable      *hash);
...
static void        __gtk_rc_style_copy_icons_and_colors(GtkRcStyle      *rc_style,
 ...
 */
gchar *
__gtk_rc_get_im_module_path (void)
{
  gchar **paths = ___gtk_get_module_path ("immodules");
 ...
 */
gchar *
__gtk_rc_get_im_module_file (void)
{
...
gchar *
__gtk_rc_get_theme_dir (void)
{
  const gchar *var;
  gchar *path;

  var = g_getenv ("STLWRT_DATA_PREFIX");
  ...
  if (var)
    {
      ...
      while (files[i])
	{
	  __gtk_rc_add_default_file (files[i]);
	  i++;
	}
      g_strfreev (files);
    }
 ...
 **/
void
__gtk_rc_add_default_file (const gchar *filename)
{
  guint n;
 ...
 **/
void
__gtk_rc_set_default_files (gchar **filenames)
{
  gint i;
  ...
  while (filenames[i] != NULL)
    {
      __gtk_rc_add_default_file (filenames[i]);
      i++;
    }
}

/**
 * __gtk_rc_get_default_files:
 *
 * Retrieves the current list of RC files that will be parsed
 * at the end of __gtk_init().
 *
 ...
 **/
gchar **
__gtk_rc_get_default_files (void)
{
  gtk_rc_add_initial_default_files ();
			 ...
			 GtkRcContext *context)
{
  ...
  if ((new_theme_name != context->theme_name && 
    ...
    {
      __gtk_rc_reparse_all_for_settings (settings, TRUE);
    }
...
{
  if (!context->reloading)
    ___gtk_rc_context_get_default_font_name (settings);
}
...
void
___gtk_rc_context_destroy (GtkSettings *settings)
{
  GtkRcContext *context;
  ...
  ___gtk_settings_reset_rc_values (context->settings);
...
static void
__gtk_rc_parse_named (GtkRcContext *context,
		    const gchar  *name,
		    const gchar  *type)
{
  gchar *path = NULL;
  ...
  if (!path)
    {
      gchar *theme_dir = __gtk_rc_get_theme_dir ();
...
static void
__gtk_rc_parse_default_files (GtkRcContext *context)
{
  gint i;
...
___gtk_rc_init (void)
{
  ...
  /* Default RC string */
  __gtk_rc_parse_string ("style \"gtk-default-tooltips-style\" {\n"
		       "  bg[NORMAL] = \"#eee1b3\"\n"
		       "  fg[NORMAL] = \"#000000\"\n"
		       "}\n"
		       "\n"
...
{
  __gtk_rc_parse_any (context, "-", -1, rc_string);
}

void
__gtk_rc_parse_string (const gchar *rc_string)
{
  GtkRcFile *rc_file;
  GSList *tmp_list;
			       ...
			       gboolean      reload)
{
  ...
  if (!g_lstat (rc_file->canonical_name, &statbuf))
	...
	goto out;
      ...
      __gtk_rc_parse_any (context, filename, fd, NULL);
...
void
__gtk_rc_parse (const gchar *filename)
{
  GSList *tmp_list;
 ...
 **/
GtkRcStyle *
__gtk_rc_style_copy (GtkRcStyle *orig)
{
  GtkRcStyle *style;
  ...
  __gtk_rc_style_copy_icons_and_colors (style, orig, NULL);

  return style;
}

void
___gtk_rc_style_set_rc_property (GtkRcStyle *rc_style,
			       GtkRcProperty *property)
{
...
void
___gtk_rc_style_unset_rc_property (GtkRcStyle *rc_style,
				 GQuark      type_name,
				 GQuark      property_name)
{
  ...
  node = (GtkRcProperty *) ___gtk_rc_style_lookup_rc_property (rc_style,
                                                             type_name,
                                                             property_name);
...
void      
__gtk_rc_style_ref (GtkRcStyle *rc_style)
{
  g_return_if_fail (GTK_IS_RC_STYLE (rc_style));
...
void      
__gtk_rc_style_unref (GtkRcStyle *rc_style)
{
  g_return_if_fail (GTK_IS_RC_STYLE (rc_style));
...
{
  return __gtk_style_new ();
}

static void
gtk_rc_style_prepend_empty_icon_factory (GtkRcStyle *rc_style)
{
  GtkIconFactory *factory = __gtk_icon_factory_new ();
...
static void
__gtk_rc_style_copy_icons_and_colors (GtkRcStyle   *rc_style,
                                    GtkRcStyle   *src_style,
                                    GtkRcContext *context)
{
  GtkRcStylePrivate *priv = gtk_rc_style_get_instance_private (rc_style);
...
gtk_rc_reset_widgets (GtkSettings *settings)
{
  ...
  ___gtk_icon_set_invalidate_caches ();
...
/**
 * __gtk_rc_reset_styles:
 * @settings: a #GtkSettings
 * 
 * This function recomputes the styles for all widgets that use a
 * particular #GtkSettings object. (There is one #GtkSettings object
 * per #GdkScreen, see __gtk_settings_get_for_screen()); It is useful
 ...
 * with __gtk_widget_set_style().
 *
 * Since: 2.4
 **/
void
__gtk_rc_reset_styles (GtkSettings *settings)
{
  GtkRcContext *context;
...
const gchar*
___gtk_rc_context_get_default_font_name (GtkSettings *settings)
{
  GtkRcContext *context;
  gchar *new_font_name;
  ...
  if (new_font_name != context->font_name && !(new_font_name && strcmp (context->font_name, new_font_name) == 0))
    {
       ...
       __gtk_rc_reset_styles (settings);
    }
...
/**
 * __gtk_rc_reparse_all_for_settings:
 * @settings: a #GtkSettings
 ...
 **/
gboolean
__gtk_rc_reparse_all_for_settings (GtkSettings *settings,
				 gboolean     force_load)
{
  ...
  if (force_load || mtime_modified)
    {
      ...
      __gtk_rc_parse_default_files (context);
	...
	__gtk_rc_parse_named (context, context->theme_name, NULL);
      if (context->key_theme_name && context->key_theme_name[0])
	__gtk_rc_parse_named (context, context->key_theme_name, "key");
  ...
  for (tmp_list = rc_contexts; tmp_list; tmp_list = tmp_list->next)
    {
      ...
      if (__gtk_rc_reparse_all_for_settings (context->settings, FALSE))
	result = TRUE;
    }
  ...
  while (sets)
    {
      ...
      if (rc_set->type == GTK_PATH_WIDGET_CLASS)
        {
          if (___gtk_rc_match_widget_class (rc_set->path, path_length, path, path_reversed))
 ...
 **/
GtkStyle *
__gtk_rc_get_style (GtkWidget *widget)
{
  ...
  context = gtk_rc_context_get (__gtk_widget_get_settings (widget));
  ...
  if (context->rc_sets_widget)
    {
      ...
      __gtk_widget_path (widget, &path_length, &path, &path_reversed);
  ...
  if (context->rc_sets_widget_class)
    {
      ...
      __gtk_widget_class_path (widget, &path_length, &path, &path_reversed);
  ...
  if (rc_styles)
  ...
  else
    {
      if (!context->default_style)
	{
	  context->default_style = __gtk_style_new ();
	  ___gtk_style_init_for_settings (context->default_style, context->settings);
	}
 ...
 *
 * The action of __gtk_rc_get_style() is similar to:
 * |[
 *  __gtk_widget_path (widget, NULL, &path, NULL);
 *  __gtk_widget_class_path (widget, NULL, &class_path, NULL);
 *  __gtk_rc_get_style_by_paths (__gtk_widget_get_settings (widget), 
 *                             path, class_path,
 ...
 **/
GtkStyle *
__gtk_rc_get_style_by_paths (GtkSettings *settings,
		    ...
		    GtkPathType  path_type)
{
  ...
  *new_style = *rc_style;
  ...
  if (path_type == GTK_PATH_WIDGET_CLASS)
    {
      rc_set->pspec = NULL;
      rc_set->path = ___gtk_rc_parse_widget_class_path (pattern);
    }
  else
    {
...
void
__gtk_rc_add_widget_name_style (GtkRcStyle  *rc_style,
			      const gchar *pattern)
{
  GtkRcContext *context;
  ...
  context = gtk_rc_context_get (__gtk_settings_get_default ());
  
  context->rc_sets_widget = gtk_rc_add_rc_sets (context->rc_sets_widget, rc_style, pattern, GTK_PATH_WIDGET);
}

void
__gtk_rc_add_widget_class_style (GtkRcStyle  *rc_style,
			       const gchar *pattern)
{
  GtkRcContext *context;
...
void
__gtk_rc_add_class_style (GtkRcStyle  *rc_style,
			const gchar *pattern)
{
  GtkRcContext *context;
...
GScanner*
__gtk_rc_scanner_new (void)
{
  return g_scanner_new (&gtk_rc_scanner_config);
}

static void
__gtk_rc_parse_any (GtkRcContext *context,
		  const gchar  *input_name,
		  gint		input_fd,
		  const gchar  *input_string)
{
  ...
  while (!done)
      ...
      else
	{
	  ...
	  expected_token = __gtk_rc_parse_statement (context, scanner);
		       ...
		       GtkRcStyle   *rc_style)
{
  ...
  ___gtk_style_init_for_settings (style, context->settings);
...
static guint
__gtk_rc_parse_assignment (GScanner      *scanner,
                         GtkRcStyle    *style,
			 GtkRcProperty *prop)
{
...
static guint
__gtk_rc_parse_statement (GtkRcContext *context,
			GScanner     *scanner)
{
  guint token;
  ...
  switch (token)
    ...
    case GTK_RC_TOKEN_STYLE:
      return __gtk_rc_parse_style (context, scanner);
      
    case GTK_RC_TOKEN_BINDING:
      return ___gtk_binding_parse_binding (scanner);
      
    case GTK_RC_TOKEN_PIXMAP_PATH:
      return __gtk_rc_parse_pixmap_path (context, scanner);
      
    case GTK_RC_TOKEN_WIDGET:
      return __gtk_rc_parse_path_pattern (context, scanner);
    ...
    case GTK_RC_TOKEN_MODULE_PATH:
      return __gtk_rc_parse_module_path (scanner);
      
    case GTK_RC_TOKEN_IM_MODULE_FILE:
      return __gtk_rc_parse_im_module_file (scanner);
      ...
      if (is_c_identifier (scanner->next_value.v_identifier))
	{
	  ...
	  token = __gtk_rc_parse_assignment (scanner, NULL, &prop);
	  if (token == G_TOKEN_NONE)
	    {
	      ...
	      ___gtk_settings_set_property_value_from_rc (context->settings,
							name,
							&svalue);
	    }
	  g_free (prop.origin);
...
static guint
__gtk_rc_parse_style (GtkRcContext *context,
		    GScanner     *scanner)
{
  GtkRcStyle *rc_style;
   ...
   */
  __gtk_rc_style_copy_icons_and_colors (rc_style, parent_style, context);
  ...
  while (token != G_TOKEN_RIGHT_CURLY)
	...
	case GTK_RC_TOKEN_BG:
	  token = __gtk_rc_parse_bg (scanner, rc_style);
	  break;
	case GTK_RC_TOKEN_FG:
	  token = __gtk_rc_parse_fg (scanner, rc_style);
	  break;
	case GTK_RC_TOKEN_TEXT:
	  token = __gtk_rc_parse_text (scanner, rc_style);
	  break;
	case GTK_RC_TOKEN_BASE:
	  token = __gtk_rc_parse_base (scanner, rc_style);
	  break;
	case GTK_RC_TOKEN_XTHICKNESS:
	  token = __gtk_rc_parse_xthickness (scanner, rc_style);
	  break;
	case GTK_RC_TOKEN_YTHICKNESS:
	  token = __gtk_rc_parse_ythickness (scanner, rc_style);
	  break;
	case GTK_RC_TOKEN_BG_PIXMAP:
	  token = __gtk_rc_parse_bg_pixmap (context, scanner, rc_style);
	  break;
	case GTK_RC_TOKEN_FONT:
	  token = __gtk_rc_parse_font (scanner, rc_style);
	  break;
	case GTK_RC_TOKEN_FONTSET:
	  token = __gtk_rc_parse_fontset (scanner, rc_style);
	  break;
	case GTK_RC_TOKEN_FONT_NAME:
	  token = __gtk_rc_parse_font_name (scanner, rc_style);
	  break;
	case GTK_RC_TOKEN_ENGINE:
	  token = __gtk_rc_parse_engine (context, scanner, &rc_style);
	  break;
        case GTK_RC_TOKEN_STOCK:
          if (our_factory == NULL)
            gtk_rc_style_prepend_empty_icon_factory (rc_style);
          our_factory = rc_style->icon_factories->data;
          token = __gtk_rc_parse_stock (context, scanner, rc_style, our_factory);
          break;
        case GTK_RC_TOKEN_COLOR:
          ...
          token = __gtk_rc_parse_logical_color (scanner, rc_style, our_hash);
          break;
	case G_TOKEN_IDENTIFIER:
	  if (is_c_identifier (scanner->next_value.v_identifier))
	    {
		  ...
		  !is_c_identifier (scanner->value.v_identifier))
		{
	      ...
	      token = __gtk_rc_parse_assignment (scanner, rc_style, &prop);
	      if (token == G_TOKEN_NONE)
		{
...
const GtkRcProperty*
___gtk_rc_style_lookup_rc_property (GtkRcStyle *rc_style,
				  GQuark      type_name,
				  GQuark      property_name)
{
...
static guint
__gtk_rc_parse_bg (GScanner   *scanner,
		 GtkRcStyle *style)
{
  GtkStateType state;
  guint token;
  ...
  token = __gtk_rc_parse_state (scanner, &state);
  if (token != G_TOKEN_NONE)
    return token;
  ...
  return __gtk_rc_parse_color_full (scanner, style, &style->bg[state]);
}

static guint
__gtk_rc_parse_fg (GScanner   *scanner,
		 GtkRcStyle *style)
{
  GtkStateType state;
  guint token;
  ...
  return __gtk_rc_parse_color_full (scanner, style, &style->fg[state]);
}

static guint
__gtk_rc_parse_text (GScanner   *scanner,
		   GtkRcStyle *style)
{
  GtkStateType state;
  guint token;
  ...
  return __gtk_rc_parse_color_full (scanner, style, &style->text[state]);
}

static guint
__gtk_rc_parse_base (GScanner   *scanner,
		   GtkRcStyle *style)
{
  GtkStateType state;
  guint token;
  ...
  return __gtk_rc_parse_color_full (scanner, style, &style->base[state]);
}

static guint
__gtk_rc_parse_xthickness (GScanner   *scanner,
			 GtkRcStyle *style)
{
...
static guint
__gtk_rc_parse_ythickness (GScanner   *scanner,
			 GtkRcStyle *style)
{
...
static guint
__gtk_rc_parse_bg_pixmap (GtkRcContext *context,
			GScanner     *scanner,
			GtkRcStyle   *rc_style)
{
  GtkStateType state;
  ...
  if ((strcmp (scanner->value.v_string, "<parent>") == 0) ||
    ...
    pixmap_file = __gtk_rc_find_pixmap_in_path (context->settings,
 ...
 **/
gchar*
__gtk_rc_find_pixmap_in_path (GtkSettings  *settings,
			    GScanner     *scanner,
			    const gchar  *pixmap_file)
{
  gint i;
 ...
 **/
gchar*
__gtk_rc_find_module_in_path (const gchar *module_file)
{
  return ___gtk_find_module (module_file, "engines");
}

static guint
__gtk_rc_parse_font (GScanner   *scanner,
		   GtkRcStyle *rc_style)
{
  guint token;
...
static guint
__gtk_rc_parse_fontset (GScanner	 *scanner,
		      GtkRcStyle *rc_style)
{
  guint token;
...
static guint
__gtk_rc_parse_font_name (GScanner   *scanner,
			GtkRcStyle *rc_style)
{
  guint token;
...
static guint	   
__gtk_rc_parse_engine (GtkRcContext *context,
		     GScanner	  *scanner,
		     GtkRcStyle	 **rc_style)
{
  guint token;
...
guint
__gtk_rc_parse_state (GScanner	 *scanner,
		    GtkStateType *state)
{
  guint old_scope;
  guint token;
...
guint
__gtk_rc_parse_priority (GScanner	           *scanner,
 ...
 * in a RC file. 
 *
 * Note that theme engines should use __gtk_rc_parse_color_full() in 
 ...
 */
guint
__gtk_rc_parse_color (GScanner *scanner,
		    GdkColor *color)
{
  return __gtk_rc_parse_color_full (scanner, NULL, color);
}

/**
 * __gtk_rc_parse_color_full:
 * @scanner: a #GScanner
 ...
 */
guint
__gtk_rc_parse_color_full (GScanner   *scanner,
                         GtkRcStyle *style,
                         GdkColor   *color)
{
  guint token;
  ...
  switch (token)
    ...
    case G_TOKEN_STRING:
      if (!__gdk_color_parse (scanner->value.v_string, color))
	{
    ...
    case G_TOKEN_IDENTIFIER:
      if (strcmp (scanner->value.v_identifier, "mix") == 0)
        {
          ...
          token = __gtk_rc_parse_color_full (scanner, style, &c1);
          if (token != G_TOKEN_NONE)
            return token;
	  ...
	  token = __gtk_rc_parse_color_full (scanner, style, &c2);
	  if (token != G_TOKEN_NONE)
            return token;
...
static guint
__gtk_rc_parse_pixmap_path (GtkRcContext *context,
			  GScanner     *scanner)
{
  guint token;
  ...
  __gtk_rc_parse_pixmap_path_string (context, scanner, scanner->value.v_string);
  
  return G_TOKEN_NONE;
}

static void
__gtk_rc_parse_pixmap_path_string (GtkRcContext *context,
				 GScanner     *scanner,
				 const gchar  *pix_path)
{
  g_strfreev (context->pixmap_path);
...
static guint
__gtk_rc_parse_module_path (GScanner *scanner)
{
  guint token;
...
static guint
__gtk_rc_parse_im_module_file (GScanner *scanner)
{
  guint token;
...
static guint
__gtk_rc_parse_path_pattern (GtkRcContext *context,
			   GScanner     *scanner)
{
  guint token;
  GtkPathType path_type;
  ...
  if (g_scanner_peek_next_token (scanner) == ':')
    {
      token = __gtk_rc_parse_priority (scanner, &priority);
      if (token != G_TOKEN_NONE)
	{
	  g_free (pattern);
	  return token;
  ...
  if (is_binding)
    {
      ...
      binding = __gtk_binding_set_find (scanner->value.v_string);
      if (!binding)
	{
	  g_free (pattern);
	  return G_TOKEN_STRING;
	}
      __gtk_binding_set_add_path (binding, path_type, pattern, priority);
    }
  else
    {
      GtkRcStyle *rc_style;
...
static guint
__gtk_rc_parse_hash_key (GScanner  *scanner,
                       gchar    **hash_key)
{
  guint token;
...
static guint
__gtk_rc_parse_icon_source (GtkRcContext   *context,
			  GScanner	 *scanner,
                          GtkIconSet     *icon_set,
                          gboolean       *icon_set_valid)
{
  ...
  if (token == G_TOKEN_STRING)
    {
      ...
      source = __gtk_icon_source_new ();      
      full_filename = __gtk_rc_find_pixmap_in_path (context->settings, scanner, scanner->value.v_string);
      if (full_filename)
	{
	  __gtk_icon_source_set_filename (source, full_filename);
	...
	return G_TOKEN_STRING;
      ...
      source = __gtk_icon_source_new ();
      __gtk_icon_source_set_icon_name (source, scanner->value.v_string);
    }
  ...
  switch (token)
    ...
    case GTK_RC_TOKEN_RTL:
      __gtk_icon_source_set_direction_wildcarded (source, FALSE);
      __gtk_icon_source_set_direction (source, GTK_TEXT_DIR_RTL);
      break;

    case GTK_RC_TOKEN_LTR:
      __gtk_icon_source_set_direction_wildcarded (source, FALSE);
      __gtk_icon_source_set_direction (source, GTK_TEXT_DIR_LTR);
      break;
  ...
  switch (token)
    {
    case GTK_RC_TOKEN_NORMAL:
      __gtk_icon_source_set_state_wildcarded (source, FALSE);
      __gtk_icon_source_set_state (source, GTK_STATE_NORMAL);
      break;

    case GTK_RC_TOKEN_PRELIGHT:
      __gtk_icon_source_set_state_wildcarded (source, FALSE);
      __gtk_icon_source_set_state (source, GTK_STATE_PRELIGHT);
      break;
      

    case GTK_RC_TOKEN_INSENSITIVE:
      __gtk_icon_source_set_state_wildcarded (source, FALSE);
      __gtk_icon_source_set_state (source, GTK_STATE_INSENSITIVE);
      break;

    case GTK_RC_TOKEN_ACTIVE:
      __gtk_icon_source_set_state_wildcarded (source, FALSE);
      __gtk_icon_source_set_state (source, GTK_STATE_ACTIVE);
      break;

    case GTK_RC_TOKEN_SELECTED:
      __gtk_icon_source_set_state_wildcarded (source, FALSE);
      __gtk_icon_source_set_state (source, GTK_STATE_SELECTED);
      break;
  ...
  if (token != '*')
    {
      ...
      size = __gtk_icon_size_from_name (scanner->value.v_string);

      if (size != GTK_ICON_SIZE_INVALID)
        {
          __gtk_icon_source_set_size_wildcarded (source, FALSE);
          __gtk_icon_source_set_size (source, size);
        }
    }
  ...
  if (__gtk_icon_source_get_filename (source) ||
      __gtk_icon_source_get_icon_name (source))
    {
      __gtk_icon_set_add_source (icon_set, source);
      *icon_set_valid = TRUE;
    }
  __gtk_icon_source_free (source);
...
static guint
__gtk_rc_parse_stock (GtkRcContext   *context,
		    GScanner       *scanner,
                    GtkRcStyle     *rc_style,
                    GtkIconFactory *factory)
{
  ...
  token = __gtk_rc_parse_hash_key (scanner, &stock_id);
  if (token != G_TOKEN_NONE)
    return token;
  ...
  while (token != G_TOKEN_RIGHT_CURLY)
    {
      if (icon_set == NULL)
        icon_set = __gtk_icon_set_new ();
      
      token = __gtk_rc_parse_icon_source (context, 
                                        scanner, icon_set, &icon_set_valid);
      if (token != G_TOKEN_NONE)
        {
          g_free (stock_id);
          __gtk_icon_set_unref (icon_set);
          return token;
        }
  ...
  if (icon_set)
    {
      if (icon_set_valid)
        __gtk_icon_factory_add (factory,
                              stock_id,
                              icon_set);
...
static guint
__gtk_rc_parse_logical_color (GScanner   *scanner,
                            GtkRcStyle *rc_style,
                            GHashTable *hash)
{
  gchar *color_id = NULL;
  ...
  token = __gtk_rc_parse_hash_key (scanner, &color_id);
  if (token != G_TOKEN_NONE)
    return token;
  ...
  token = __gtk_rc_parse_color_full (scanner, rc_style, &color);
  if (token != G_TOKEN_NONE)
    {
      g_free (color_id);
      return token;
   ...
   */
  g_hash_table_insert (hash, color_id, __gdk_color_copy (&color));
...
GSList *
___gtk_rc_parse_widget_class_path (const gchar *pattern)
{
  GSList *result;
  PathElt *path_elt;
  const gchar *current;
...
void
___gtk_rc_free_widget_class_path (GSList *list)
{
...
gtk_rc_set_free (GtkRcSet *rc_set)
{
  ...
  ___gtk_rc_free_widget_class_path (rc_set->path);
...
gboolean
___gtk_rc_match_widget_class (GSList  *list,
                            gint     length,
                            gchar   *path,
                            gchar   *path_reversed)
{
...
#undef __gtk_rc_add_default_file
...
__gtk_rc_add_default_file (const gchar *filename)
{
  ...
  __gtk_rc_add_default_file_utf8 (utf8_filename);

  g_free (utf8_filename);
}

#undef __gtk_rc_set_default_files
...
__gtk_rc_set_default_files (gchar **filenames)
{
  ...
  __gtk_rc_set_default_files_utf8 (utf8_filenames);

  g_strfreev (utf8_filenames);
}

#undef __gtk_rc_parse
...
__gtk_rc_parse (const gchar *filename)
{
  ...
  __gtk_rc_parse_utf8 (utf8_filename);
