...
#define PACK_DIRECTION(m)                                 \
   (GTK_IS_MENU_BAR (m)                                   \
     ? __gtk_menu_bar_get_pack_direction (GTK_MENU_BAR (m)) \
...
gtk_menu_shell_class_init (GtkMenuShellClass *klass)
{
   ...
   * keyboard focus. See __gtk_menu_shell_set_take_focus() and
   * __gtk_menu_shell_get_take_focus().
   *
   * Since: 2.8
   **/
                             ...
                             GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_TAKE_FOCUS:
      __gtk_menu_shell_set_take_focus (menu_shell, g_value_get_boolean (value));
      break;
    default:
                             ...
                             GParamSpec  *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_TAKE_FOCUS:
      g_value_set_boolean (value, __gtk_menu_shell_get_take_focus (menu_shell));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
void
__gtk_menu_shell_append (GtkMenuShell *menu_shell,
		       GtkWidget    *child)
{
  __gtk_menu_shell_insert (menu_shell, child, -1);
}

void
__gtk_menu_shell_prepend (GtkMenuShell *menu_shell,
			GtkWidget    *child)
{
  __gtk_menu_shell_insert (menu_shell, child, 0);
}

void
__gtk_menu_shell_insert (GtkMenuShell *menu_shell,
		       GtkWidget    *child,
		       gint          position)
{
...
void
__gtk_menu_shell_deactivate (GtkMenuShell *menu_shell)
{
  g_return_if_fail (GTK_IS_MENU_SHELL (menu_shell));
			     ...
			     GdkEventButton *event)
{
  ...
  if (!gtk_menu_shell_get_props (menu_shell)->active || !gtk_menu_shell_get_props (menu_shell)->button)
    ...
    {
      widget = __gtk_get_event_widget ((GdkEvent*) event);
      ...
      if ((event->time - gtk_menu_shell_get_props (menu_shell)->activate_time) > MENU_SHELL_TIMEOUT)
        {
          if (menu_item && (gtk_menu_shell_get_props (menu_shell)->active_menu_item == menu_item) &&
              ___gtk_menu_item_is_selectable (menu_item))
            {
              ...
              if (submenu == NULL)
                {
                  __gtk_menu_shell_activate_item (menu_shell, menu_item, TRUE);
                       ...
                       priv->activated_submenu)
                {
                  ...
                  if (!priv->activated_submenu &&
                    ...
                    {
                      ___gtk_menu_item_popdown_submenu (menu_item);
                    }
                  else
                    {
                      __gtk_menu_item_select (GTK_MENU_ITEM (menu_item));
                    }
...
void
___gtk_menu_shell_update_mnemonics (GtkMenuShell *menu_shell)
{
  GtkMenuShell *target;
  ...
  while (target)
    {
      ...
      if (GTK_IS_MENU_BAR (target))
        {
          __gtk_window_set_mnemonics_visible (GTK_WINDOW (toplevel), FALSE);
          ___gtk_label_mnemonics_visible_apply_recursively (GTK_WIDGET (target),
                                                          mnemonics_visible);
        }
      else
  ...
  if (gtk_menu_shell_get_props (menu_shell)->active)
	  ...
	  GTK_IS_MENU_ITEM (menu_item))
	{
	  ...
	  if (event->detail != GDK_NOTIFY_INFERIOR)
            {
	      if (__gtk_widget_get_state (menu_item) != GTK_STATE_PRELIGHT)
                __gtk_menu_shell_select_item (menu_shell, menu_item);
                  ...
                  if (!__gtk_widget_get_visible (gtk_menu_item_get_props (GTK_MENU_ITEM (menu_item))->submenu))
                    {
                      ...
                      if (touchscreen_mode)
                        ___gtk_menu_item_popup_submenu (menu_item, TRUE);
                    }
                }
	    }
	}
			     ...
			     GdkEventCrossing *event)
{
  ...
  if (__gtk_widget_get_visible (widget))
    {
      GtkMenuShell *menu_shell = GTK_MENU_SHELL (widget);
      GtkWidget *event_widget = __gtk_get_event_widget ((GdkEvent*) event);
	...
	return TRUE;
      ...
      if (!___gtk_menu_item_is_selectable (event_widget))
        {
          GtkMenuShellPrivate *priv;
...
gtk_menu_shell_add (GtkContainer *container,
		    GtkWidget    *widget)
{
  __gtk_menu_shell_append (GTK_MENU_SHELL (container), widget);
}
...
void
__gtk_menu_shell_select_item (GtkMenuShell *menu_shell,
			    GtkWidget    *menu_item)
{
...
void
__gtk_menu_shell_deselect (GtkMenuShell *menu_shell)
{
  g_return_if_fail (GTK_IS_MENU_SHELL (menu_shell));
...
void
__gtk_menu_shell_activate_item (GtkMenuShell      *menu_shell,
			      GtkWidget         *menu_item,
			      gboolean           force_deactivate)
{
  ...
  if (deactivate)
      ...
      do
	{
       ...
       */
      __gdk_display_sync (__gtk_widget_get_display (menu_item));
    }

  __gtk_widget_activate (menu_item);
  ...
  if (gtk_menu_shell_get_props (menu_shell)->active_menu_item)
      ...
      if (distance > 0)
	{
	  ...
	  while (node != start_node && 
		 (!node || !___gtk_menu_item_is_selectable (node->data)))
	    {
	      if (node)
		node = node->next;
              else if (wrap_around)
      ...
      else
	{
      ...
      if (node)
	__gtk_menu_shell_select_item (menu_shell, node->data);
    }
 ...
 **/
void
__gtk_menu_shell_select_first (GtkMenuShell *menu_shell,
			     gboolean      search_sensitive)
{
  ...
  while (tmp_list)
    {
      ...
      if ((!search_sensitive && __gtk_widget_get_visible (child)) ||
	  ___gtk_menu_item_is_selectable (child))
	{
	  to_select = child;
  ...
  if (to_select)
    __gtk_menu_shell_select_item (menu_shell, to_select);
}

void
___gtk_menu_shell_select_last (GtkMenuShell *menu_shell,
			     gboolean      search_sensitive)
{
  GtkWidget *to_select = NULL;
  GList *tmp_list;
  ...
  switch (direction)
	       ...
	       gtk_menu_item_get_props (GTK_MENU_ITEM (gtk_menu_shell_get_props (menu_shell)->active_menu_item))->submenu)
	{
	  ...
	  if (GTK_MENU_SHELL_GET_CLASS (menu_shell)->submenu_placement !=
	    ...
	    ___gtk_menu_shell_select_last (submenu, TRUE);
	}
      break;
				 ...
				 GtkDirectionType   dir)
{
  ...
  if (menu_shell)
    ___gtk_menu_bar_cycle_focus (GTK_MENU_BAR (menu_shell), dir);
}

gint
___gtk_menu_shell_get_popup_delay (GtkMenuShell *menu_shell)
{
 ...
 */
void
__gtk_menu_shell_cancel (GtkMenuShell *menu_shell)
{
  g_return_if_fail (GTK_IS_MENU_SHELL (menu_shell));
  ...
  if (!private->key_hash && create && __gtk_widget_has_screen (widget))
    {
      ...
      GdkKeymap *keymap = __gdk_keymap_get_for_display (__gdk_screen_get_display (screen));
...
void
___gtk_menu_shell_add_mnemonic (GtkMenuShell *menu_shell,
			      guint      keyval,
			      GtkWidget *target)
{
  g_return_if_fail (GTK_IS_MENU_SHELL (menu_shell));
...
void
___gtk_menu_shell_remove_mnemonic (GtkMenuShell *menu_shell,
				 guint      keyval,
				 GtkWidget *target)
{
  g_return_if_fail (GTK_IS_MENU_SHELL (menu_shell));
 ...
 **/
gboolean
__gtk_menu_shell_get_take_focus (GtkMenuShell *menu_shell)
{
  GtkMenuShellPrivate *priv;
...
/**
 * __gtk_menu_shell_set_take_focus:
 * @menu_shell: a #GtkMenuShell
 ...
 *
 * See also __gdk_keyboard_grab()
 *
 * Since: 2.8
 **/
void
__gtk_menu_shell_set_take_focus (GtkMenuShell *menu_shell,
                               gboolean      take_focus)
{
  GtkMenuShellPrivate *priv;
