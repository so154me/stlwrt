 ...
 * </para>
 * <para>
 * Use __gtk_widget_class_install_style_property() to install style properties for
 * a widget class, __gtk_widget_class_find_style_property() or
 * __gtk_widget_class_list_style_properties() to get information about existing
 * style properties and __gtk_widget_style_get_property(), __gtk_widget_style_get() or
 * __gtk_widget_style_get_valist() to obtain the value of a style property.
 * </para>
 * </refsect2>
...
static void	__gtk_widget_set_property		 (GObject           *object,
						  guint              prop_id,
						  const GValue      *value,
						  GParamSpec        *pspec);
...
static void		__gtk_widget_set_style_internal		(GtkWidget	  *widget,
								 GtkStyle	  *style,
								 gboolean	   initial_emission);
static gint		__gtk_widget_event_internal		(GtkWidget	  *widget,
								 GdkEvent	  *event);
...
static AtkObject*	__gtk_widget_ref_accessible		(AtkImplementor *implementor);
static void             gtk_widget_invalidate_widget_windows    (GtkWidget        *widget,
								 GdkRegion        *region);
static GdkScreen *      __gtk_widget_get_screen_unchecked         (GtkWidget        *widget);
...
static void __gtk_widget_set_usize_internal (GtkWidget *widget,
					   gint       width,
					   gint       height);
...
gtk_widget_class_init (GtkWidgetClass *klass)
{
  ...
  gobject_class->set_property = __gtk_widget_set_property;
  ...
  klass->show_all = __gtk_widget_show;
  klass->hide = gtk_widget_real_hide;
  klass->hide_all = __gtk_widget_hide;
  ...
  klass->property_notify_event = ___gtk_selection_property_notify;
  klass->selection_clear_event = __gtk_selection_clear;
  klass->selection_request_event = ___gtk_selection_request;
  klass->selection_notify_event = ___gtk_selection_notify;
  ...
  g_object_class_install_property (gobject_class,
				   ...
				   g_param_spec_boolean ("no-show-all",
 							 P_("No show all"),
 							 P_("Whether __gtk_widget_show_all() should not affect this widget"),
 							 FALSE,
 							 GTK_PARAM_READWRITE));
 ...
 */
   ...
   *
   * Also see __gtk_tooltip_set_text().
   *
   ...
   * Also see __gtk_tooltip_set_markup().
   *
   * This is a convenience property which will take care of getting the
   * tooltip shown if the given string is not %NULL: #GtkWidget:has-tooltip
   * will automatically be set to %TRUE and there will be taken care of
   ...
   * __gtk_widget_modify_base() also cause this signal to be emitted.
   */
 ...
 */
   ...
   * being removed.
   * 
   * A widget is shadowed by a __gtk_grab_add() when the topmost 
 ...
 */
   ...
   * See __gdk_screen_is_composited().
   */
   ...
   * See __gtk_widget_keynav_failed() for details.
   *
   ...
   * destroys the window. Connecting __gtk_widget_hide_on_delete() to
   ...
   * custom drag icon with __gtk_drag_source_set_icon().
   *
  ...
  widget_signals[DRAG_FAILED] =
    g_signal_new (I_("drag-failed"),
		  G_TYPE_FROM_CLASS (gobject_class),
		  G_SIGNAL_RUN_LAST,
		  0, ___gtk_boolean_handled_accumulator, NULL,
		  NULL,
		  G_TYPE_BOOLEAN, 2,
		  GDK_TYPE_DRAG_CONTEXT,
   ...
   * displaying feedback to the user, by calling __gdk_drag_status().
   *
   * If the decision whether the drop will be accepted or rejected can't be
   * made based solely on the cursor position and the type of the data, the
   * handler may inspect the dragged data by calling __gtk_drag_get_data() and
   * defer the __gdk_drag_status() call to the #GtkWidget::drag-data-received
   * handler. Note that you cannot not pass #GTK_DEST_DEFAULT_DROP,
   * #GTK_DEST_DEFAULT_MOTION or #GTK_DEST_DEFAULT_ALL to __gtk_drag_dest_set()
   ...
   * the drop site with __gtk_drag_highlight().
   * |[
   * static void
   ...
   *      * supposed to call __gdk_drag_status (), not actually paste in 
   *      * the data.
   *      &ast;/
   ...
   *        __gdk_drag_status (context, 0, time);
   *      else
   *        __gdk_drag_status (context, private_data->suggested_action, time);
   *    }
   *   else
   *    {
   ...
   * ensure that __gtk_drag_finish() is called to let the source know that 
   * the drop is done. The call to __gtk_drag_finish() can be done either 
   * directly or in a #GtkWidget::drag-data-received handler which gets 
   * triggered by calling __gtk_drag_get_data() to receive the data for one 
   ...
   * is indicated by @info. See __gtk_selection_data_set() and 
   * __gtk_selection_data_set_text().
   */
   ...
   * to call __gdk_drag_status() and <emphasis>not</emphasis> finish the drag. 
   * If the data was received in response to a #GtkWidget::drag-drop signal 
   * (and this is the last target to be received), the handler for this 
   * signal is expected to process the received data and then call 
   * __gtk_drag_finish(), setting the @success parameter depending on whether 
   * the data was processed successfully. 
   * 
   * The handler may inspect and modify @drag_context->action before calling 
   * __gtk_drag_finish(), e.g. to implement %GDK_ACTION_ASK as shown in the 
   * following example:
   * |[
   * void  
   * drag_data_received (GtkWidget          *widget,
   ...
   *           gint response;
   *           
   *           dialog = __gtk_message_dialog_new (NULL,
   *                                            GTK_DIALOG_MODAL | 
   *                                            GTK_DIALOG_DESTROY_WITH_PARENT,
   *                                            GTK_MESSAGE_INFO,
   *                                            GTK_BUTTONS_YES_NO,
   *                                            "Move the data ?\n");
   *           response = __gtk_dialog_run (GTK_DIALOG (dialog));
   ...
   *          }
   *          
   *       __gtk_drag_finish (drag_context, TRUE, FALSE, time);
   *       return;
   *     }
   *       
   *    __gtk_drag_finish (drag_context, FALSE, FALSE, time);
   *  }
   * ]|
   */
   ...
   * drawn as a copy of another #GdkDrawable (with __gdk_draw_drawable() or
   * __gdk_window_copy_area()) which was completely unobscured. If the source
...
/**
   ...
   *     been emitted, relative to @gtk_widget_get_props (widget)->window
  ...
  __gtk_binding_entry_add_signal (binding_set, GDK_F10, GDK_SHIFT_MASK,
                                "popup-menu", 0);
  __gtk_binding_entry_add_signal (binding_set, GDK_Menu, 0,
                                "popup-menu", 0);  

  __gtk_binding_entry_add_signal (binding_set, GDK_F1, GDK_CONTROL_MASK,
                                "show-help", 1,
                                GTK_TYPE_WIDGET_HELP_TYPE,
                                GTK_WIDGET_HELP_TOOLTIP);
  __gtk_binding_entry_add_signal (binding_set, GDK_KP_F1, GDK_CONTROL_MASK,
                                "show-help", 1,
                                GTK_TYPE_WIDGET_HELP_TYPE,
                                GTK_WIDGET_HELP_TOOLTIP);
  __gtk_binding_entry_add_signal (binding_set, GDK_F1, GDK_SHIFT_MASK,
                                "show-help", 1,
                                GTK_TYPE_WIDGET_HELP_TYPE,
                                GTK_WIDGET_HELP_WHATS_THIS);  
  __gtk_binding_entry_add_signal (binding_set, GDK_KP_F1, GDK_SHIFT_MASK,
                                "show-help", 1,
                                GTK_TYPE_WIDGET_HELP_TYPE,
                                GTK_WIDGET_HELP_WHATS_THIS);

  __gtk_widget_class_install_style_property (klass,
...
static void
__gtk_widget_set_property (GObject         *object,
			 guint            prop_id,
			 const GValue    *value,
			 GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_NAME:
      __gtk_widget_set_name (widget, g_value_get_string (value));
      break;
    case PROP_PARENT:
      __gtk_container_add (GTK_CONTAINER (g_value_get_object (value)), widget);
      break;
    case PROP_WIDTH_REQUEST:
      __gtk_widget_set_usize_internal (widget, g_value_get_int (value), -2);
      break;
    case PROP_HEIGHT_REQUEST:
      __gtk_widget_set_usize_internal (widget, -2, g_value_get_int (value));
      break;
    case PROP_VISIBLE:
      __gtk_widget_set_visible (widget, g_value_get_boolean (value));
      break;
    case PROP_SENSITIVE:
      __gtk_widget_set_sensitive (widget, g_value_get_boolean (value));
      break;
    case PROP_APP_PAINTABLE:
      __gtk_widget_set_app_paintable (widget, g_value_get_boolean (value));
      break;
    case PROP_CAN_FOCUS:
      __gtk_widget_set_can_focus (widget, g_value_get_boolean (value));
      break;
    case PROP_HAS_FOCUS:
    ...
    case PROP_CAN_DEFAULT:
      __gtk_widget_set_can_default (widget, g_value_get_boolean (value));
      break;
    case PROP_HAS_DEFAULT:
      if (g_value_get_boolean (value))
	__gtk_widget_grab_default (widget);
      break;
    case PROP_RECEIVES_DEFAULT:
      __gtk_widget_set_receives_default (widget, g_value_get_boolean (value));
      break;
    case PROP_STYLE:
      __gtk_widget_set_style (widget, g_value_get_object (value));
      break;
    case PROP_EVENTS:
      if (!__gtk_widget_get_realized (widget) && __gtk_widget_get_has_window (widget))
	__gtk_widget_set_events (widget, g_value_get_flags (value));
      break;
    case PROP_EXTENSION_EVENTS:
      __gtk_widget_set_extension_events (widget, g_value_get_enum (value));
      break;
    case PROP_NO_SHOW_ALL:
      __gtk_widget_set_no_show_all (widget, g_value_get_boolean (value));
      break;
    ...
    case PROP_DOUBLE_BUFFERED:
      __gtk_widget_set_double_buffered (widget, g_value_get_boolean (value));
      break;
    default:
			 ...
			 GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_NAME:
      if (gtk_widget_get_props (widget)->name)
	g_value_set_string (value, gtk_widget_get_props (widget)->name);
      else
	g_value_set_static_string (value, "");
      break;
    case PROP_PARENT:
      g_value_set_object (value, gtk_widget_get_props (widget)->parent);
      break;
    case PROP_WIDTH_REQUEST:
      {
        int w;
        __gtk_widget_get_size_request (widget, &w, NULL);
    ...
    case PROP_HEIGHT_REQUEST:
      {
        int h;
        __gtk_widget_get_size_request (widget, NULL, &h);
        g_value_set_int (value, h);
      }
      break;
    case PROP_VISIBLE:
      g_value_set_boolean (value, (__gtk_widget_get_visible (widget) != FALSE));
      break;
    case PROP_SENSITIVE:
      g_value_set_boolean (value, (__gtk_widget_get_sensitive (widget) != FALSE));
      break;
    case PROP_APP_PAINTABLE:
      g_value_set_boolean (value, (__gtk_widget_get_app_paintable (widget) != FALSE));
      break;
    case PROP_CAN_FOCUS:
      g_value_set_boolean (value, (__gtk_widget_get_can_focus (widget) != FALSE));
      break;
    case PROP_HAS_FOCUS:
      g_value_set_boolean (value, (__gtk_widget_has_focus (widget) != FALSE));
      break;
    case PROP_IS_FOCUS:
      g_value_set_boolean (value, (__gtk_widget_is_focus (widget)));
      break;
    case PROP_CAN_DEFAULT:
      g_value_set_boolean (value, (__gtk_widget_get_can_default (widget) != FALSE));
      break;
    case PROP_HAS_DEFAULT:
      g_value_set_boolean (value, (__gtk_widget_has_default (widget) != FALSE));
      break;
    case PROP_RECEIVES_DEFAULT:
    ...
    case PROP_EXTENSION_EVENTS:
      ...
      break;
    case PROP_NO_SHOW_ALL:
      g_value_set_boolean (value, __gtk_widget_get_no_show_all (widget));
      break;
    case PROP_HAS_TOOLTIP:
    ...
    case PROP_TOOLTIP_MARKUP:
      ...
      break;
    case PROP_WINDOW:
      g_value_set_object (value, __gtk_widget_get_window (widget));
      break;
    case PROP_DOUBLE_BUFFERED:
      g_value_set_boolean (value, __gtk_widget_get_double_buffered (widget));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gtk_widget_init (GtkWidget *widget)
{
  ...
  gtk_widget_get_props (widget)->state = GTK_STATE_NORMAL;
  gtk_widget_get_props (widget)->saved_state = GTK_STATE_NORMAL;
  gtk_widget_get_props (widget)->name = NULL;
  gtk_widget_get_props (widget)->requisition.width = 0;
  gtk_widget_get_props (widget)->requisition.height = 0;
  gtk_widget_get_props (widget)->allocation.x = -1;
  gtk_widget_get_props (widget)->allocation.y = -1;
  ...
  gtk_widget_get_props (widget)->style = __gtk_widget_get_default_style ();
  g_object_ref (gtk_widget_get_props (widget)->style);
}
...
gtk_widget_dispatch_child_properties_changed (GtkWidget   *widget,
					      ...
					      GParamSpec **pspecs)
{
  GtkWidget *container = gtk_widget_get_props (widget)->parent;
  guint i;

  for (i = 0; gtk_widget_get_props (widget)->parent == container && i < n_pspecs; i++)
    g_signal_emit (widget, widget_signals[CHILD_NOTIFY], g_quark_from_string (pspecs[i]->name), pspecs[i]);
}

/**
 * __gtk_widget_freeze_child_notify:
 * @widget: a #GtkWidget
 * 
 * Stops emission of #GtkWidget::child-notify signals on @widget. The 
 * signals are queued until __gtk_widget_thaw_child_notify() is called 
 * on @widget. 
 *
 * This is the analogue of g_object_freeze_notify() for child properties.
 **/
void
__gtk_widget_freeze_child_notify (GtkWidget *widget)
{
 ...
 **/
void
__gtk_widget_child_notify (GtkWidget    *widget,
			 const gchar  *child_property)
{
  ...
  pspec = g_param_spec_pool_lookup (_gtk_widget_child_property_pool,
				    child_property,
				    G_OBJECT_TYPE (gtk_widget_get_props (widget)->parent),
				    TRUE);
  if (!pspec)
    g_warning ("%s: container class `%s' has no child property named `%s'",
	       G_STRLOC,
	       G_OBJECT_TYPE_NAME (gtk_widget_get_props (widget)->parent),
	       child_property);
  else
    {
...
/**
 * __gtk_widget_thaw_child_notify:
 * @widget: a #GtkWidget
 *
 * Reverts the effect of a previous call to __gtk_widget_freeze_child_notify().
 * This causes all queued #GtkWidget::child-notify signals on @widget to be 
 * emitted.
 */ 
void
__gtk_widget_thaw_child_notify (GtkWidget *widget)
{
 ...
 * <literal>__gtk_widget_new (GTK_TYPE_LABEL, "label", "Hello World", "xalign",
 ...
 **/
void
__gtk_widget_set (GtkWidget   *widget,
		const gchar *first_property_name,
		...)
{
  va_list var_args;
...
static inline void	   
__gtk_widget_queue_draw_child (GtkWidget *widget)
{
  GtkWidget *parent;

  parent = gtk_widget_get_props (widget)->parent;
  if (parent && __gtk_widget_is_drawable (parent))
    __gtk_widget_queue_draw_area (parent,
 ...
 **/
void
__gtk_widget_unparent (GtkWidget *widget)
{
  GObjectNotifyQueue *nqueue;
  GtkWidget *toplevel;
  ...
  g_return_if_fail (GTK_IS_WIDGET (widget));
  if (gtk_widget_get_props (widget)->parent == NULL)
    return;
  
  /* keep this function in sync with __gtk_menu_detach()
   */
   ...
   */
  if (GTK_WIDGET_ANCHORED (gtk_widget_get_props (widget)->parent))
    g_object_ref (toplevel);
  else
    toplevel = NULL;

  __gtk_widget_queue_draw_child (widget);
  ...
  old_parent = gtk_widget_get_props (widget)->parent;
  gtk_widget_get_props (widget)->parent = NULL;
  __gtk_widget_set_parent_window (widget, NULL);
  g_signal_emit (widget, widget_signals[PARENT_SET], 0, old_parent);
  if (toplevel)
    {
      ___gtk_widget_propagate_hierarchy_changed (widget, toplevel);
 ...
 **/
void
__gtk_widget_destroyed (GtkWidget      *widget,
 ...
 * container, it's easier to call __gtk_widget_show_all() on the
 ...
 **/
void
__gtk_widget_show (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  ...
  if (!__gtk_widget_get_visible (widget))
    {
	  ...
	  !__gtk_widget_get_mapped (widget))
	__gtk_widget_map (widget);
    }
}

static void
__gtk_widget_show_map_callback (GtkWidget *widget, GdkEvent *event, gint *flag)
{
  *flag = TRUE;
  g_signal_handlers_disconnect_by_func (widget,
					__gtk_widget_show_map_callback, 
					flag);
}

/**
 * __gtk_widget_show_now:
 * @widget: a #GtkWidget
 * 
 ...
 **/
void
__gtk_widget_show_now (GtkWidget *widget)
{
  gint flag = FALSE;
  ...
  if (!__gtk_widget_get_mapped (widget) &&
      __gtk_widget_is_toplevel (widget))
    {
      __gtk_widget_show (widget);

      g_signal_connect (widget, "map-event",
			G_CALLBACK (__gtk_widget_show_map_callback), 
			&flag);
 ...
 * @widget: a #GtkWidget
 * 
 * Reverses the effects of __gtk_widget_show(), causing the widget to be
 * hidden (invisible to the user).
 */
void
__gtk_widget_hide (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  ...
  if (__gtk_widget_get_visible (widget))
    {
      ...
      g_object_ref (widget);
      if (toplevel != widget && __gtk_widget_is_toplevel (toplevel))
	___gtk_window_unset_focus_and_default (GTK_WINDOW (toplevel), widget);
...
/**
 * __gtk_widget_hide_on_delete:
 * @widget: a #GtkWidget
 * 
 * Utility function; intended to be connected to the #GtkWidget::delete-event
 * signal on a #GtkWindow. The function calls __gtk_widget_hide() on its
 ...
 **/
gboolean
__gtk_widget_hide_on_delete (GtkWidget *widget)
{
 ...
 **/
void
__gtk_widget_show_all (GtkWidget *widget)
{
  GtkWidgetClass *class;

  g_return_if_fail (GTK_IS_WIDGET (widget));

  if (__gtk_widget_get_no_show_all (widget))
    return;
...
/**
 * __gtk_widget_hide_all:
 * @widget: a #GtkWidget
 * 
 * Recursively hides a widget and any child widgets.
 *
 * Deprecated: 2.24: Use __gtk_widget_hide() instead.
 */
void
__gtk_widget_hide_all (GtkWidget *widget)
{
  GtkWidgetClass *class;
 ...
 **/
void
__gtk_widget_map (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  g_return_if_fail (__gtk_widget_get_visible (widget));
 ...
 **/
void
__gtk_widget_unmap (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  ...
  if (__gtk_widget_get_mapped (widget))
    {
      if (!__gtk_widget_get_has_window (widget))
	__gdk_window_invalidate_rect (gtk_widget_get_props (widget)->window, &gtk_widget_get_props (widget)->allocation, FALSE);
      ___gtk_tooltip_hide (widget);
      g_signal_emit (widget, widget_signals[UNMAP], 0);
    }
}

static void
__gtk_widget_set_extension_events_internal (GtkWidget        *widget,
                                          ...
                                          GList            *window_list)
{
  ...
  if (window_list == NULL)
    {
      if (__gtk_widget_get_has_window (widget))
        window_list = g_list_prepend (NULL, gtk_widget_get_props (widget)->window);
      else
        window_list = __gdk_window_get_children (gtk_widget_get_props (widget)->window);
  ...
  for (l = window_list; l != NULL; l = l->next)
    {
      ...
      __gdk_window_get_user_data (window, &user_data);
      if (user_data == widget)
        {
          GList *children;

          __gdk_input_set_extension_events (window,
                                          __gdk_window_get_events (window),
                                          mode);

          children = __gdk_window_get_children (window);
          if (children)
            {
              __gtk_widget_set_extension_events_internal (widget, mode, children);
...
/**
 * __gtk_widget_realize:
 * @widget: a #GtkWidget
 * 
 * Creates the GDK (windowing system) resources associated with a
 * gtk_widget_get_props (widget).  For example, @gtk_widget_get_props (widget)->window will be created when a gtk_widget_get_props (widget)
 ...
 * __gtk_widget_realize() realizes the widget's parents in addition to
 ...
 **/
void
__gtk_widget_realize (GtkWidget *widget)
{
  GdkExtensionMode mode;
  ...
  if (!__gtk_widget_get_realized (widget))
    {
      /*
	if (GTK_IS_CONTAINER (widget) && __gtk_widget_get_has_window (widget))
	  g_message ("__gtk_widget_realize(%s)", G_OBJECT_TYPE_NAME (widget));
      */

      if (gtk_widget_get_props (widget)->parent == NULL &&
          !__gtk_widget_is_toplevel (widget))
        g_warning ("Calling __gtk_widget_realize() on a widget that isn't "
                   "inside a toplevel window is not going to work very well. "
                   "Widgets must be inside a toplevel container before realizing them.");
      
      if (gtk_widget_get_props (widget)->parent && !__gtk_widget_get_realized (gtk_widget_get_props (widget)->parent))
	__gtk_widget_realize (gtk_widget_get_props (widget)->parent);
      ...
      if (GTK_WIDGET_HAS_SHAPE_MASK (widget))
	{
	  ...
	  __gdk_window_shape_combine_mask (gtk_widget_get_props (widget)->window,
      ...
      if (shape_info)
	__gdk_window_input_shape_combine_mask (gtk_widget_get_props (widget)->window,
					     shape_info->shape_mask,
					     shape_info->offset_x,
					     shape_info->offset_y);

      mode = __gtk_widget_get_extension_events (widget);
      if (mode != GDK_EXTENSION_EVENTS_NONE)
        __gtk_widget_set_extension_events_internal (widget, mode, NULL);
    }
}

/**
 * __gtk_widget_unrealize:
 * @widget: a #GtkWidget
 *
 * This function is only useful in widget implementations.
 * Causes a widget to be unrealized (frees all GDK resources
 * associated with the gtk_widget_get_props (widget), such as @gtk_widget_get_props (widget)->window).
 **/
void
__gtk_widget_unrealize (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));

  if (GTK_WIDGET_HAS_SHAPE_MASK (widget))
    __gtk_widget_shape_combine_mask (widget, NULL, 0, 0);

  if (g_object_get_qdata (G_OBJECT (widget), quark_input_shape_info))
    __gtk_widget_input_shape_combine_mask (widget, NULL, 0, 0);
 ...
 * @width and @height by calling __gdk_window_invalidate_rect() on the
 ...
 * __gdk_window_invalidate_rect() directly, to schedule a redraw of a
 * #GtkDrawingArea or some portion thereof.
 *
 * Frequently you can just call __gdk_window_invalidate_rect() or
 * __gdk_window_invalidate_region() instead of this function. Those
 ...
 **/
void	   
__gtk_widget_queue_draw_area (GtkWidget *widget,
			    gint       x,
			    gint       y,
			    gint       width,
 			    gint       height)
{
  ...
  if (__gtk_widget_get_has_window (widget))
      ...
      if (gtk_widget_get_props (widget)->parent)
	{
	  ...
	  __gdk_window_get_position (gtk_widget_get_props (widget)->window, &wx, &wy);
	  x -= wx - gtk_widget_get_props (widget)->allocation.x;
	  y -= wy - gtk_widget_get_props (widget)->allocation.y;
	  
	  wwidth = __gdk_window_get_width (gtk_widget_get_props (widget)->window);
	  wheight = __gdk_window_get_height (gtk_widget_get_props (widget)->window);
				 ...
				 GdkRectangle *rect)
{
  ...
  __gdk_rectangle_union (rect, &child_rect, rect);
}
  ...
  if (!__gtk_widget_get_has_window (widget))
    {
      ...
      *rect = gtk_widget_get_props (widget)->allocation;
  ...
  else
    {
      rect->x = 0;
      rect->y = 0;
      rect->width = gtk_widget_get_props (widget)->allocation.width;
      rect->height = gtk_widget_get_props (widget)->allocation.height;
    }
}

/**
 * __gtk_widget_queue_draw:
 * @widget: a #GtkWidget
 *
 * Equivalent to calling __gtk_widget_queue_draw_area() for the
 * entire area of a widget.
 **/
void	   
__gtk_widget_queue_draw (GtkWidget *widget)
{
  GdkRectangle rect;
 ...
 * __gtk_widget_queue_draw_area(), though it once was. Now it just calls
 * __gtk_widget_queue_draw_area(). Originally
 * __gtk_widget_queue_clear_area() would force a redraw of the
 * background for %GTK_NO_WINDOW widgets, and
 * __gtk_widget_queue_draw_area() would not. Now both functions ensure
 * the background will be redrawn.
 * 
 * Deprecated: 2.2: Use __gtk_widget_queue_draw_area() instead.
 **/
void	   
__gtk_widget_queue_clear_area (GtkWidget *widget,
			     gint       x,
			     gint       y,
			     gint       width,
			     gint       height)
{
  ...
  __gtk_widget_queue_draw_area (widget, x, y, width, height);
}

/**
 * __gtk_widget_queue_clear:
 * @widget: a #GtkWidget
 * 
 * This function does the same as __gtk_widget_queue_draw().
 *
 * Deprecated: 2.2: Use __gtk_widget_queue_draw() instead.
 **/
void	   
__gtk_widget_queue_clear (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 **/
void
__gtk_widget_queue_resize (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  ...
  ___gtk_size_group_queue_resize (widget);
}

/**
 * __gtk_widget_queue_resize_no_redraw:
 * @widget: a #GtkWidget
 *
 * This function works like __gtk_widget_queue_resize(), 
 * except that the widget is not invalidated.
 *
 * Since: 2.4
 **/
void
__gtk_widget_queue_resize_no_redraw (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 * __gtk_widget_draw() simply invalidates the specified region of the
 * widget, then updates the invalid region of the widget immediately.
 * Usually you don't want to update the region immediately for
 * performance reasons, so in general __gtk_widget_queue_draw_area() is
 * a better choice if you want to draw a region of a widget.
 **/
void
__gtk_widget_draw (GtkWidget          *widget,
  ...
  if (__gtk_widget_is_drawable (widget))
    {
      ...
      __gdk_window_process_updates (gtk_widget_get_props (widget)->window, TRUE);
    }
}

/**
 * __gtk_widget_size_request:
 * @widget: a #GtkWidget
 ...
 * size allocations to give them with __gtk_widget_size_allocate().
 *
 ...
 *
 * See also __gtk_widget_get_child_requisition().
 **/
void
__gtk_widget_size_request (GtkWidget	*widget,
			 GtkRequisition *requisition)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));

#ifdef G_ENABLE_DEBUG
  if (requisition == &gtk_widget_get_props (widget)->requisition)
    g_warning ("__gtk_widget_size_request() called on child gtk_widget_get_props (widget) with request equal\n to gtk_widget_get_props (widget)->requisition. __gtk_widget_set_usize() may not work properly.");
#endif /* G_ENABLE_DEBUG */

  ___gtk_size_group_compute_requisition (widget, requisition);
}

/**
 * __gtk_widget_get_child_requisition:
 * @widget: a #GtkWidget
 * @requisition: a #GtkRequisition to be filled in
 * 
 * This function is only for use in widget implementations. Obtains
 * @gtk_widget_get_props (widget)->requisition, unless someone has forced a particular
 * geometry on the widget (e.g. with __gtk_widget_set_size_request()),
 * in which case it returns that geometry instead of the widget's
 * requisition.
 *
 * This function differs from __gtk_widget_size_request() in that
 * it retrieves the last size request value from @gtk_widget_get_props (widget)->requisition,
 * while __gtk_widget_size_request() actually calls the "size_request" method
 * on @gtk_widget_get_props (widget) to compute the size request and fill in @gtk_widget_get_props (widget)->requisition,
 * and only then returns @gtk_widget_get_props (widget)->requisition.
 *
 * Because this function does not call the "size_request" method, it
 * can only be used when you know that @gtk_widget_get_props (widget)->requisition is
 * up-to-date, that is, __gtk_widget_size_request() has been called
 * since the last time a resize was queued. In general, only container
 * implementations have this information; applications should use
 * __gtk_widget_size_request().
 **/
void
__gtk_widget_get_child_requisition (GtkWidget	 *widget,
				  GtkRequisition *requisition)
{
  ___gtk_size_group_get_child_requisition (widget, requisition);
}
...
/* Invalidate @region in gtk_widget_get_props (widget)->window and all children
 * of gtk_widget_get_props (widget)->window owned by gtk_widget_get_props (widget). @region is in the
 * same coordinates as gtk_widget_get_props (widget)->allocation and will be
  ...
  if (__gtk_widget_get_has_window (widget) && gtk_widget_get_props (widget)->parent)
    {
      ...
      __gdk_window_get_position (gtk_widget_get_props (widget)->window, &x, &y);
      __gdk_region_offset (region, -x, -y);
    }

  __gdk_window_invalidate_maybe_recurse (gtk_widget_get_props (widget)->window, region,
 ...
 * @widget: a #GtkWidget
 *
 * Like __gtk_widget_queue_draw(), but only windows owned
...
gtk_widget_queue_shallow_draw (GtkWidget *widget)
{
  ...
  region = __gdk_region_rectangle (&rect);
  gtk_widget_invalidate_widget_windows (widget, region);
  __gdk_region_destroy (region);
}

/**
 * __gtk_widget_size_allocate:
 * @widget: a #GtkWidget
 ...
 **/
void
__gtk_widget_size_allocate (GtkWidget	*widget,
			  GtkAllocation *allocation)
{
  ...
  if (gtk_debug_flags & GTK_DEBUG_GEOMETRY)
	...
	{
	  depth++;
	  parent = __gtk_widget_get_parent (parent);
	}
      
      name = g_type_name (G_OBJECT_TYPE (G_OBJECT (widget)));
      g_print ("__gtk_widget_size_allocate: %*s%s %d %d\n", 
	       2 * depth, " ", name, 
  ...
  old_allocation = gtk_widget_get_props (widget)->allocation;
  ...
  if (real_allocation.width < 0 || real_allocation.height < 0)
    {
      g_warning ("__gtk_widget_size_allocate(): attempt to allocate widget with width %d and height %d",
  ...
  if (__gtk_widget_get_mapped (widget))
    {
      if (!__gtk_widget_get_has_window (widget) && GTK_WIDGET_REDRAW_ON_ALLOC (widget) && position_changed)
	{
	  /* Invalidate union(old_allaction,gtk_widget_get_props (widget)->allocation) in gtk_widget_get_props (widget)->window
	   */
	  GdkRegion *invalidate = __gdk_region_rectangle (&gtk_widget_get_props (widget)->allocation);
	  __gdk_region_union_with_rect (invalidate, &old_allocation);

	  __gdk_window_invalidate_region (gtk_widget_get_props (widget)->window, invalidate, FALSE);
      ...
      if (size_changed)
	{
	  if (GTK_WIDGET_REDRAW_ON_ALLOC (widget))
	    {
	      /* Invalidate union(old_allaction,gtk_widget_get_props (widget)->allocation) in gtk_widget_get_props (widget)->window and descendents owned by gtk_widget_get_props (widget)
	       */
	      GdkRegion *invalidate = __gdk_region_rectangle (&gtk_widget_get_props (widget)->allocation);
	      __gdk_region_union_with_rect (invalidate, &old_allocation);
...
/**
 * __gtk_widget_translate_coordinates:
 ...
 **/
gboolean
__gtk_widget_translate_coordinates (GtkWidget  *src_widget,
				  ...
				  gint       *dest_y)
{
  ...
  if (!ancestor || !__gtk_widget_get_realized (src_widget) || !__gtk_widget_get_realized (dest_widget))
    return FALSE;
  ...
  if (__gtk_widget_get_has_window (src_widget) && gtk_widget_get_props (src_widget)->parent)
    {
      ...
      src_x -= wx - gtk_widget_get_props (src_widget)->allocation.x;
      src_y -= wy - gtk_widget_get_props (src_widget)->allocation.y;
    }
  else
    {
      src_x += gtk_widget_get_props (src_widget)->allocation.x;
      src_y += gtk_widget_get_props (src_widget)->allocation.y;
    }

  /* Translate to the common ancestor */
  window = gtk_widget_get_props (src_widget)->window;
  while (window != gtk_widget_get_props (ancestor)->window)
    {
      gdouble dx, dy;

      __gdk_window_coords_to_parent (window, src_x, src_y, &dx, &dy);
  ...
  /* And back */
  window = gtk_widget_get_props (dest_widget)->window;
  while (window != gtk_widget_get_props (ancestor)->window)
    {
  ...
  while (dest_list)
    {
      ...
      __gdk_window_coords_from_parent (dest_list->data, src_x, src_y, &dx, &dy);
                                    ...
                                    guint      signal_id)
{
  ...
  return __gtk_widget_is_sensitive (widget) &&
         __gtk_widget_is_drawable (widget) &&
         __gdk_window_is_viewable (gtk_widget_get_props (widget)->window);
}

/**
 * __gtk_widget_can_activate_accel:
 * @widget: a #GtkWidget
 * @signal_id: the ID of a signal installed on @widget
 * 
 * Determines whether an accelerator that activates the signal
 ...
 **/
gboolean
__gtk_widget_can_activate_accel (GtkWidget *widget,
                               guint      signal_id)
{
			...
			gpointer      marshal_data)
{
  ...
  gboolean can_activate = __gtk_widget_can_activate_accel (closure->data, aclosure->signal_id);
			  ...
			  guint      signal_id)
{
  ...
  for (slist = closures; slist; slist = slist->next)
    if (!__gtk_accel_group_from_accel_closure (slist->data))
      {
	/* reuse this closure */
	closure = slist->data;
	break;
 ...
 * __gtk_window_add_accel_group(), and the signal must be of type %G_RUN_ACTION.
 * Accelerators added through this function are not user changeable during
 * runtime. If you want to support accelerators that can be changed by the
 * user, use __gtk_accel_map_add_entry() and __gtk_widget_set_accel_path() or
 * __gtk_menu_item_set_accel_path() instead.
 */
void
__gtk_widget_add_accelerator (GtkWidget      *widget,
			    ...
			    GtkAccelFlags   accel_flags)
{
   ...
   */
  __gtk_accel_group_connect (accel_group,
			   accel_key,
			   accel_mods,
...
/**
 * __gtk_widget_remove_accelerator:
 * @widget:       widget to install an accelerator on
 ...
 */
gboolean
__gtk_widget_remove_accelerator (GtkWidget      *widget,
			       GtkAccelGroup  *accel_group,
			       guint           accel_key,
			       GdkModifierType accel_mods)
{
  ...
  ag_entry = __gtk_accel_group_query (accel_group, accel_key, accel_mods, &n);
  clist = __gtk_widget_list_accel_closures (widget);
  for (slist = clist; slist; slist = slist->next)
	...
	if (slist->data == (gpointer) ag_entry[i].closure)
	  {
	    gboolean is_removed = __gtk_accel_group_disconnect (accel_group, slist->data);
...
/**
 * __gtk_widget_list_accel_closures:
 * @widget:  widget to list accelerator closures for
 *
 * Lists the closures used by @widget for accelerator group connections
 * with __gtk_accel_group_connect_by_path() or __gtk_accel_group_connect().
 * The closures can be used to monitor accelerator changes on @widget,
 * by connecting to the @GtkAccelGroup::accel-changed signal of the
 * #GtkAccelGroup of a closure which can be found out with
 * __gtk_accel_group_from_accel_closure().
 *
 * Return value: (transfer container) (element-type GClosure):
 *     a newly allocated #GList of closures
 */
GList*
__gtk_widget_list_accel_closures (GtkWidget *widget)
{
  GSList *slist;
  GList *clist = NULL;
...
destroy_accel_path (gpointer data)
{
  ...
  __gtk_accel_group_disconnect (apath->accel_group, apath->closure);
 ...
 * __gtk_widget_set_accel_path(). Associating accelerators with
 * paths allows them to be modified by the user and the modifications
 * to be saved for future use. (See __gtk_accel_map_save().)
 *
 ...
 * set up accelerators on menu items __gtk_menu_item_set_accel_path()
 ...
 **/
void
__gtk_widget_set_accel_path (GtkWidget     *widget,
			   ...
			   GtkAccelGroup *accel_group)
{
  ...
  if (accel_path)
    {
      g_return_if_fail (GTK_IS_ACCEL_GROUP (accel_group));
      g_return_if_fail (___gtk_accel_path_is_valid (accel_path));

      __gtk_accel_map_add_entry (accel_path, 0, 0);
  ...
  if (apath)
    __gtk_accel_group_connect_by_path (apath->accel_group, g_quark_to_string (apath->path_quark), apath->closure);

  g_signal_emit (widget, widget_signals[ACCEL_CLOSURES_CHANGED], 0);
}

const gchar*
___gtk_widget_get_accel_path (GtkWidget *widget,
			    gboolean  *locked)
{
  AccelPath *apath;
...
/**
 * __gtk_widget_mnemonic_activate:
 * @widget: a #GtkWidget
 ...
 */
gboolean
__gtk_widget_mnemonic_activate (GtkWidget *widget,
                              gboolean   group_cycling)
{
  gboolean handled;
                                   ...
                                   gboolean   group_cycling)
{
  ...
  else if (__gtk_widget_get_can_focus (widget))
    __gtk_widget_grab_focus (widget);
  else
    {
...
#define WIDGET_REALIZED_FOR_EVENT(widget, event) \
     (event->type == GDK_FOCUS_CHANGE || __gtk_widget_get_realized(widget))
 ...
 * instead, use __gtk_main_do_event() so the event will behave as if
 * it were in the event queue. Don't synthesize expose events; instead,
 * use __gdk_window_invalidate_rect() to invalidate a region of the
 * window.
 * 
 ...
 **/
gboolean
__gtk_widget_event (GtkWidget *widget,
		  GdkEvent  *event)
{
  ...
  if (event->type == GDK_EXPOSE)
    {
      g_warning ("Events of type GDK_EXPOSE cannot be synthesized. To get "
		 "the same effect, call __gdk_window_invalidate_rect/region(), "
		 "followed by __gdk_window_process_updates().");
      return TRUE;
    }
  
  return __gtk_widget_event_internal (widget, event);
}


/**
 * __gtk_widget_send_expose:
 * @widget: a #GtkWidget
 ...
 * that is normally done using __gtk_container_propagate_expose().
 *
 * If you want to force an area of a window to be redrawn, 
 * use __gdk_window_invalidate_rect() or __gdk_window_invalidate_region().
 * To cause the redraw to be done immediately, follow that call
 * with a call to __gdk_window_process_updates().
 * 
 * Return value: return from the event signal emission (%TRUE if 
 *               the event was handled)
 **/
gint
__gtk_widget_send_expose (GtkWidget *widget,
			GdkEvent  *event)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), TRUE);
  g_return_val_if_fail (__gtk_widget_get_realized (widget), TRUE);
...
event_window_is_still_viewable (GdkEvent *event)
{
  ...
  switch (event->type)
    ...
    case GDK_SCROLL:
      return event->any.window && __gdk_window_is_viewable (event->any.window);
...
static gint
__gtk_widget_event_internal (GtkWidget *widget,
			   GdkEvent  *event)
{
  ...
  if (!return_val)
	...
	case GDK_FOCUS_CHANGE:
	  ...
	  if (event->focus_change.in)
	    ___gtk_tooltip_focus_in (widget);
	  else
	    ___gtk_tooltip_focus_out (widget);
	  break;
	case GDK_CONFIGURE:
	...
	case GDK_DAMAGE:
	  ...
	  break;
	default:
	  g_warning ("__gtk_widget_event(): unhandled event type: %d", event->type);
	  signal_num = -1;
	  break;
	}
      if (signal_num != -1)
 ...
 **/
gboolean
__gtk_widget_activate (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
/**
 * __gtk_widget_set_scroll_adjustments:
 * @widget: a #GtkWidget
 ...
 **/
gboolean
__gtk_widget_set_scroll_adjustments (GtkWidget     *widget,
...
static void
__gtk_widget_reparent_subwindows (GtkWidget *widget,
				GdkWindow *new_window)
{
  if (!__gtk_widget_get_has_window (widget))
    {
      GList *children = __gdk_window_get_children (gtk_widget_get_props (widget)->window);
      GList *tmp_list;
  ...
  else
   {
     ...
     parent = __gdk_window_get_parent (gtk_widget_get_props (widget)->window);

     if (parent == NULL)
       __gdk_window_reparent (gtk_widget_get_props (widget)->window, new_window, 0, 0);
     else
       {
	 children = __gdk_window_get_children (parent);
...
static void
__gtk_widget_reparent_fixup_child (GtkWidget *widget,
				 gpointer   client_data)
{
  ...
  if (!__gtk_widget_get_has_window (widget))
	...
	g_object_unref (gtk_widget_get_props (widget)->window);
      gtk_widget_get_props (widget)->window = (GdkWindow*) client_data;
      if (gtk_widget_get_props (widget)->window)
	g_object_ref (gtk_widget_get_props (widget)->window);

      if (GTK_IS_CONTAINER (widget))
        __gtk_container_forall (GTK_CONTAINER (widget),
                              __gtk_widget_reparent_fixup_child,
                              client_data);
    }
}

/**
 * __gtk_widget_reparent:
 * @widget: a #GtkWidget
 ...
 **/
void
__gtk_widget_reparent (GtkWidget *widget,
		     GtkWidget *new_parent)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  g_return_if_fail (GTK_IS_CONTAINER (new_parent));
  g_return_if_fail (gtk_widget_get_props (widget)->parent != NULL);
  
  if (gtk_widget_get_props (widget)->parent != new_parent)
    {
      /* First try to see if we can get away without unrealizing
       * the widget as we reparent it. if so we set a flag so
       * that __gtk_widget_unparent doesn't unrealize widget
       */
      if (__gtk_widget_get_realized (widget) && __gtk_widget_get_realized (new_parent))
	GTK_PRIVATE_SET_FLAG (widget, GTK_IN_REPARENT);
      
      g_object_ref (widget);
      __gtk_container_remove (GTK_CONTAINER (gtk_widget_get_props (widget)->parent), gtk_widget_get_props (widget));
      __gtk_container_add (GTK_CONTAINER (new_parent), widget);
      g_object_unref (widget);
      ...
      if (GTK_WIDGET_IN_REPARENT (widget))
	{
	  ...
	  __gtk_widget_reparent_subwindows (widget, __gtk_widget_get_parent_window (widget));
	  __gtk_widget_reparent_fixup_child (widget,
					   __gtk_widget_get_parent_window (widget));
	}
 ...
 **/
gboolean
__gtk_widget_intersect (GtkWidget	         *widget,
		      const GdkRectangle *area,
		      GdkRectangle       *intersection)
{
  ...
  return_val = __gdk_rectangle_intersect (&gtk_widget_get_props (widget)->allocation, area, dest);
  
  if (return_val && intersection && __gtk_widget_get_has_window (widget))
    {
      intersection->x -= gtk_widget_get_props (widget)->allocation.x;
      intersection->y -= gtk_widget_get_props (widget)->allocation.y;
    }
...
/**
 * __gtk_widget_region_intersect:
 * @widget: a #GtkWidget
 * @region: a #GdkRegion, in the same coordinate system as 
 *          @gtk_widget_get_props (widget)->allocation. That is, relative to @gtk_widget_get_props (widget)->window
 *          for %NO_WINDOW widgets; relative to the parent window
 *          of @gtk_widget_get_props (widget)->window for widgets with their own window.
 * @returns: A newly allocated region holding the intersection of @widget
 *           and @region. The coordinates of the return value are
 *           relative to @gtk_widget_get_props (widget)->window for %NO_WINDOW widgets, and
 *           relative to the parent window of @gtk_widget_get_props (widget)->window for
 *           widgets with their own window.
 * 
 * Computes the intersection of a @widget's area and @region, returning
 * the intersection. The result may be empty, use __gdk_region_empty() to
 * check.
 **/
GdkRegion *
__gtk_widget_region_intersect (GtkWidget       *widget,
			     const GdkRegion *region)
{
  ...
  dest = __gdk_region_rectangle (&rect);
 
  __gdk_region_intersect (dest, region);

  return dest;
}

/**
 * ___gtk_widget_grab_notify:
 * @widget: a #GtkWidget
 ...
 **/
void
___gtk_widget_grab_notify (GtkWidget *widget,
 ...
 * __gtk_widget_set_can_focus() to modify that flag.
 *
 ...
 **/
void
__gtk_widget_grab_focus (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  ...
  if (GTK_IS_CONTAINER (widget))
    {
      ...
      __gtk_container_set_focus_child (container, NULL);

      __gtk_container_foreach (container,
			     reset_focus_recurse,
			     NULL);
    }
}
...
{
  if (__gtk_widget_get_can_focus (focus_widget))
    {
      GtkWidget *toplevel;
      GtkWidget *widget;
       ...
       */
      toplevel = __gtk_widget_get_toplevel (focus_widget);
      if (__gtk_widget_is_toplevel (toplevel) && GTK_IS_WINDOW (toplevel))
	{
	  widget = gtk_window_get_props (GTK_WINDOW (toplevel))->focus_widget;
	  
	  if (widget == focus_widget)
	    {
	      /* We call ___gtk_window_internal_set_focus() here so that the
	       * toplevel window can request the focus if necessary.
	       * This is needed when the toplevel is a GtkPlug
	       */
	      if (!__gtk_widget_has_focus (widget))
		___gtk_window_internal_set_focus (GTK_WINDOW (toplevel), focus_widget);
	...
	{
	  /* __gtk_widget_grab_focus() operates on a tree without window...
	   * actually, this is very questionable behaviour.
	   */
	  
	  __gtk_container_foreach (GTK_CONTAINER (toplevel),
				 reset_focus_recurse,
				 NULL);
	}
			       ...
			       GtkTooltip *tooltip)
{
  ...
  if (has_tooltip && tooltip_markup)
    {
      __gtk_tooltip_set_markup (tooltip, tooltip_markup);
      return TRUE;
    }
                           ...
                           GtkWidgetHelpType help_type)
{
  if (help_type == GTK_WIDGET_HELP_TOOLTIP)
    {
      ___gtk_tooltip_toggle_keyboard_mode (widget);
...
gtk_widget_real_focus (GtkWidget         *widget,
                       GtkDirectionType   direction)
{
  if (!__gtk_widget_get_can_focus (widget))
    return FALSE;
...
/**
 * __gtk_widget_set_can_focus:
 * @widget: a #GtkWidget
 * @can_focus: whether or not @widget can own the input focus.
 *
 * Specifies whether @widget can own the input focus. See
 * __gtk_widget_grab_focus() for actually setting the input focus on a
 * widget.
 *
 * Since: 2.18
 **/
void
__gtk_widget_set_can_focus (GtkWidget *widget,
                          gboolean   can_focus)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 **/
gboolean
__gtk_widget_get_can_focus (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
/**
 * __gtk_widget_has_focus:
 * @widget: a #GtkWidget
 *
 * Determines if the widget has the global input focus. See
 * __gtk_widget_is_focus() for the difference between having the global
 ...
 **/
gboolean
__gtk_widget_has_focus (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
 ...
 **/
gboolean
__gtk_widget_is_focus (GtkWidget *widget)
{
  GtkWidget *toplevel;
  ...
  if (GTK_IS_WINDOW (toplevel))
    return widget == gtk_window_get_props (GTK_WINDOW (toplevel))->focus_widget;
  else
    return FALSE;
}

/**
 * __gtk_widget_set_can_default:
 * @widget: a #GtkWidget
 * @can_default: whether or not @widget can be a default widget.
 *
 * Specifies whether @widget can be a default widget. See
 * __gtk_widget_grab_default() for details about the meaning of
 * "default".
 *
 * Since: 2.18
 **/
void
__gtk_widget_set_can_default (GtkWidget *widget,
 ...
 **/
gboolean
__gtk_widget_get_can_default (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
/**
 * __gtk_widget_has_default:
 * @widget: a #GtkWidget
 *
 * Determines whether @widget is the current default widget within its
 * toplevel. See __gtk_widget_set_can_default().
 *
 ...
 */
gboolean
__gtk_widget_has_default (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
void
___gtk_widget_set_has_default (GtkWidget *widget,
...
/**
 * __gtk_widget_grab_default:
 * @widget: a #GtkWidget
 *
 * Causes @widget to become the default widget. @widget must have the
 * %GTK_CAN_DEFAULT flag set; typically you have to set this flag
 * yourself by calling <literal>__gtk_widget_set_can_default (@widget,
 * %TRUE)</literal>. The default widget is activated when 
 * the user presses Enter in a window. Default widgets must be 
 * activatable, that is, __gtk_widget_activate() should affect them.
 **/
void
__gtk_widget_grab_default (GtkWidget *widget)
{
  GtkWidget *window;
  
  g_return_if_fail (GTK_IS_WIDGET (widget));
  g_return_if_fail (__gtk_widget_get_can_default (widget));
  
  window = __gtk_widget_get_toplevel (widget);
  
  if (window && __gtk_widget_is_toplevel (window))
    __gtk_window_set_default (GTK_WINDOW (window), widget);
  else
    g_warning (G_STRLOC ": widget not within a GtkWindow");
}

/**
 * __gtk_widget_set_receives_default:
 * @widget: a #GtkWidget
 ...
 * is the default.
 *
 * See __gtk_widget_grab_default() for details about the meaning of
 * "default".
 *
 * Since: 2.18
 **/
void
__gtk_widget_set_receives_default (GtkWidget *widget,
...
/**
 * __gtk_widget_get_receives_default:
 * @widget: a #GtkWidget
 *
 ...
 * is the default.
 *
 * See __gtk_widget_set_receives_default().
 *
 ...
 **/
gboolean
__gtk_widget_get_receives_default (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
/**
 * __gtk_widget_has_grab:
 * @widget: a #GtkWidget
 *
 * Determines whether the widget is currently grabbing events, so it
 * is the only widget receiving input events (keyboard and mouse).
 *
 * See also __gtk_grab_add().
 *
 ...
 **/
gboolean
__gtk_widget_has_grab (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
void
___gtk_widget_set_has_grab (GtkWidget *widget,
                          gboolean   has_grab)
{
  if (has_grab)
 ...
 * __gtk_widget_path()), so names with embedded periods may cause confusion.
 **/
void
__gtk_widget_set_name (GtkWidget	 *widget,
		     const gchar *name)
{
  gchar *new_name;
  ...
  g_free (gtk_widget_get_props (widget)->name);
  gtk_widget_get_props (widget)->name = new_name;

  if (__gtk_widget_has_rc_style (widget))
...
/**
 * __gtk_widget_get_name:
 * @widget: a #GtkWidget
 * 
 * Retrieves the name of a widget. See __gtk_widget_set_name() for the
 ...
 **/
const gchar*
__gtk_widget_get_name (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
  
  if (gtk_widget_get_props (widget)->name)
    return gtk_widget_get_props (widget)->name;
  return G_OBJECT_TYPE_NAME (widget);
}

/**
 * __gtk_widget_set_state:
 * @widget: a #GtkWidget
 * @state: new state for @widget
 *
 * This function is for use in widget implementations. Sets the state
 * of a widget (insensitive, prelighted, etc.) Usually you should set
 * the state using wrapper functions such as __gtk_widget_set_sensitive().
 **/
void
__gtk_widget_set_state (GtkWidget           *widget,
		      GtkStateType         state)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));

  if (state == __gtk_widget_get_state (widget))
    return;

  if (state == GTK_STATE_INSENSITIVE)
    __gtk_widget_set_sensitive (widget, FALSE);
  else
    {
      GtkStateData data;
      ...
      if (gtk_widget_get_props (widget)->parent)
	data.parent_sensitive = (__gtk_widget_is_sensitive (gtk_widget_get_props (widget)->parent) != FALSE);
      else
...
/**
 * __gtk_widget_get_state:
 * @widget: a #GtkWidget
 *
 * Returns the widget's state. See __gtk_widget_set_state().
 *
 ...
 */
GtkStateType
__gtk_widget_get_state (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), GTK_STATE_NORMAL);

  return gtk_widget_get_props (widget)->state;
}

/**
 * __gtk_widget_set_visible:
 * @widget: a #GtkWidget
 * @visible: whether the widget should be shown or not
 *
 * Sets the visibility state of @widget. Note that setting this to
 * %TRUE doesn't mean the widget is actually viewable, see
 * __gtk_widget_get_visible().
 *
 * This function simply calls __gtk_widget_show() or __gtk_widget_hide()
 ...
 **/
void
__gtk_widget_set_visible (GtkWidget *widget,
                        gboolean   visible)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));

  if (visible != __gtk_widget_get_visible (widget))
    {
      if (visible)
        __gtk_widget_show (widget);
      else
 ...
 **/
gboolean
__gtk_widget_get_visible (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
/**
 * __gtk_widget_set_has_window:
 * @widget: a #GtkWidget
 * @has_window: whether or not @widget has a window.
 *
 * Specifies whether @widget has a #GdkWindow of its own. Note that
 * all realized widgets have a non-%NULL "window" pointer
 * (__gtk_widget_get_window() never returns a %NULL window when a widget
 ...
 **/
void
__gtk_widget_set_has_window (GtkWidget *widget,
                           gboolean   has_window)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 **/
gboolean
__gtk_widget_get_has_window (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
 ...
 **/
gboolean
__gtk_widget_is_toplevel (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
void
___gtk_widget_set_is_toplevel (GtkWidget *widget,
 ...
 **/
gboolean
__gtk_widget_is_drawable (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);

  return (__gtk_widget_get_visible (widget) &&
          __gtk_widget_get_mapped (widget));
}

/**
 * __gtk_widget_get_realized:
 * @widget: a #GtkWidget
 *
 ...
 **/
gboolean
__gtk_widget_get_realized (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
 ...
 */
void
__gtk_widget_set_realized (GtkWidget *widget,
                         gboolean   realized)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 */
gboolean
__gtk_widget_get_mapped (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
 ...
 */
void
__gtk_widget_set_mapped (GtkWidget *widget,
                       gboolean   mapped)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
/**
 * __gtk_widget_set_app_paintable:
 * @widget: a #GtkWidget
 ...
 **/
void
__gtk_widget_set_app_paintable (GtkWidget *widget,
...
/**
 * __gtk_widget_get_app_paintable:
 * @widget: a #GtkWidget
 *
 * Determines whether the application intends to draw on the widget in
 * an #GtkWidget::expose-event handler.
 *
 * See __gtk_widget_set_app_paintable()
 *
 ...
 **/
gboolean
__gtk_widget_get_app_paintable (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
/**
 * __gtk_widget_set_double_buffered:
 * @widget: a #GtkWidget
 * @double_buffered: %TRUE to double-buffer a widget
 *
 * Widgets are double buffered by default; you can use this function
 * to turn off the buffering. "Double buffered" simply means that
 * __gdk_window_begin_paint_region() and __gdk_window_end_paint() are called
 * automatically around expose events sent to the
 * widget. gdk_window_begin_paint() diverts all drawing to a widget's
 * window to an offscreen buffer, and __gdk_window_end_paint() draws the
 ...
 **/
void
__gtk_widget_set_double_buffered (GtkWidget *widget,
...
/**
 * __gtk_widget_get_double_buffered:
 * @widget: a #GtkWidget
 *
 * Determines whether the widget is double buffered.
 *
 * See __gtk_widget_set_double_buffered()
 *
 ...
 **/
gboolean
__gtk_widget_get_double_buffered (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
/**
 * __gtk_widget_set_redraw_on_allocate:
 * @widget: a #GtkWidget
 ...
 * %NO_WINDOW gtk_widget_get_props (widget) that <emphasis>does</emphasis> draw on @gtk_widget_get_props (widget)->window, 
 * you are responsible for invalidating both the old and new allocation 
 * of the widget when the widget is moved and responsible for invalidating
 * regions newly when the widget increases size.
 **/
void
__gtk_widget_set_redraw_on_allocate (GtkWidget *widget,
 ...
 **/
void
__gtk_widget_set_sensitive (GtkWidget *widget,
			  gboolean   sensitive)
{
  GtkStateData data;
  ...
  if (sensitive == (__gtk_widget_get_sensitive (widget) != FALSE))
    return;
...
/**
 * __gtk_widget_get_sensitive:
 * @widget: a #GtkWidget
 *
 * Returns the widget's sensitivity (in the sense of returning
 * the value that has been set using __gtk_widget_set_sensitive()).
 *
 * The effective sensitivity of a widget is however determined by both its
 * own and its parent widget's sensitivity. See __gtk_widget_is_sensitive().
 *
 ...
 */
gboolean
__gtk_widget_get_sensitive (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
 ...
 */
gboolean
__gtk_widget_is_sensitive (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
 ...
 **/
void
__gtk_widget_set_parent (GtkWidget *widget,
		       GtkWidget *parent)
{
  GtkStateData data;
  ...
  /* keep this function in sync with __gtk_menu_attach_to_widget()
   */

  g_object_ref_sink (widget);
  gtk_widget_get_props (widget)->parent = parent;

  if (__gtk_widget_get_state (parent) != GTK_STATE_NORMAL)
    data.state = __gtk_widget_get_state (parent);
  else
    data.state = __gtk_widget_get_state (widget);
  data.state_restoration = FALSE;
  data.parent_sensitive = (__gtk_widget_is_sensitive (parent) != FALSE);
  data.use_forall = __gtk_widget_is_sensitive (parent) != __gtk_widget_is_sensitive (widget);

  gtk_widget_propagate_state (widget, &data);
  
  __gtk_widget_reset_rc_styles (widget);

  g_signal_emit (widget, widget_signals[PARENT_SET], 0, NULL);
  if (GTK_WIDGET_ANCHORED (gtk_widget_get_props (widget)->parent))
    ___gtk_widget_propagate_hierarchy_changed (widget, NULL);
  g_object_notify (G_OBJECT (widget), "parent");

  /* Enforce realized/mapped invariants
   */
  if (__gtk_widget_get_realized (gtk_widget_get_props (widget)->parent))
    __gtk_widget_realize (widget);

  if (__gtk_widget_get_visible (gtk_widget_get_props (widget)->parent) &&
      __gtk_widget_get_visible (widget))
    {
 ...
 **/
GtkWidget *
__gtk_widget_get_parent (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);

  return gtk_widget_get_props (widget)->parent;
}
 ...
 **/
void
__gtk_widget_style_attach (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 **/
gboolean
__gtk_widget_has_rc_style (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
/**
 * __gtk_widget_set_style:
 * @widget: a #GtkWidget
 * @style: (allow-none): a #GtkStyle, or %NULL to remove the effect of a previous
 *         __gtk_widget_set_style() and go back to the default style
 *
 * Sets the #GtkStyle for a gtk_widget_get_props (widget) (@gtk_widget_get_props (widget)->style). You probably don't
 * want to use this function; it interacts badly with themes, because
 * themes work by replacing the #GtkStyle. Instead, use
 * __gtk_widget_modify_style().
 **/
void
__gtk_widget_set_style (GtkWidget *widget,
		      GtkStyle	*style)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
/**
 * __gtk_widget_ensure_style:
 * @widget: a #GtkWidget
 *
 * Ensures that @gtk_widget_get_props (widget) has a style (@gtk_widget_get_props (widget)->style). Not a very useful
 * function; most of the time, if you want the style, the widget is
 * realized, and realized widgets are guaranteed to have a style
 * already.
 **/
void
__gtk_widget_ensure_style (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));

  if (!GTK_WIDGET_USER_STYLE (widget) &&
      !__gtk_widget_has_rc_style (widget))
    gtk_widget_reset_rc_style (widget);
}
...
gtk_widget_reset_rc_style (GtkWidget *widget)
{
  ...
  if (!new_style)
    new_style = __gtk_widget_get_default_style ();

  if (initial_emission || new_style != gtk_widget_get_props (widget)->style)
    __gtk_widget_set_style_internal (widget, new_style, initial_emission);
}

/**
 * __gtk_widget_get_style:
 * @widget: a #GtkWidget
 * 
 * Simply an accessor function that returns @gtk_widget_get_props (widget)->style.
 *
 * Return value: (transfer none): the widget's #GtkStyle
 **/
GtkStyle*
__gtk_widget_get_style (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
  
  return gtk_widget_get_props (widget)->style;
}
 ...
 * the widget using __gtk_widget_set_style(). The #GtkRcStyle structure
 * is designed so each field can either be set or unset, so it is
 * possible, using this function, to modify some style values and
 * leave the others unchanged.
 *
 * Note that modifications made with this function are not cumulative
 * with previous calls to __gtk_widget_modify_style() or with such
 * functions as __gtk_widget_modify_fg(). If you wish to retain
 * previous values, you must first call __gtk_widget_get_modifier_style(),
 * make your modifications to the returned style, then call
 * __gtk_widget_modify_style() with that style. On the other hand,
 * if you first call __gtk_widget_modify_style(), subsequent calls
 * to such functions __gtk_widget_modify_fg() will have a cumulative
 * effect with the initial modifications.
 **/
void       
__gtk_widget_modify_style (GtkWidget      *widget,
			 GtkRcStyle     *style)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
/**
 * __gtk_widget_get_modifier_style:
 * @widget: a #GtkWidget
 * 
 * Returns the current modifier style for the widget. (As set by
 * __gtk_widget_modify_style().) If no style has previously set, a new
 * #GtkRcStyle will be created with all values unset, and set as the
 * modifier style for the widget. If you make changes to this rc
 * style, you must call __gtk_widget_modify_style(), passing in the
 * returned rc style, to make sure that your changes take effect.
 *
 * Caution: passing the style back to __gtk_widget_modify_style() will
 * normally end up destroying it, because __gtk_widget_modify_style() copies
 ...
 **/
GtkRcStyle *
__gtk_widget_get_modifier_style (GtkWidget      *widget)
{
  GtkRcStyle *rc_style;
...
{
  GtkRcStyle *rc_style = __gtk_widget_get_modifier_style (widget);  
  ...
  __gtk_widget_modify_style (widget, rc_style);
}

/**
 * __gtk_widget_modify_fg:
 * @widget: a #GtkWidget
 * @state: the state for which to set the foreground color
 * @color: (allow-none): the color to assign (does not need to be allocated),
 *         or %NULL to undo the effect of previous calls to
 *         of __gtk_widget_modify_fg().
 *
 ...
 **/
void
__gtk_widget_modify_fg (GtkWidget      *widget,
		      GtkStateType    state,
		      const GdkColor *color)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
/**
 * __gtk_widget_modify_bg:
 * @widget: a #GtkWidget
 * @state: the state for which to set the background color
 * @color: (allow-none): the color to assign (does not need to be allocated),
 *         or %NULL to undo the effect of previous calls to
 *         of __gtk_widget_modify_bg().
 *
 ...
 **/
void
__gtk_widget_modify_bg (GtkWidget      *widget,
		      GtkStateType    state,
		      const GdkColor *color)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
/**
 * __gtk_widget_modify_text:
 * @widget: a #GtkWidget
 * @state: the state for which to set the text color
 * @color: (allow-none): the color to assign (does not need to be allocated),
 *         or %NULL to undo the effect of previous calls to
 *         of __gtk_widget_modify_text().
 *
 * Sets the text color for a widget in a particular state.  All other
 * style values are left untouched. The text color is the foreground
 * color used along with the base color (see __gtk_widget_modify_base())
 * for widgets such as #GtkEntry and #GtkTextView. See also
 * __gtk_widget_modify_style().
 **/
void
__gtk_widget_modify_text (GtkWidget      *widget,
			GtkStateType    state,
			const GdkColor *color)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
/**
 * __gtk_widget_modify_base:
 * @widget: a #GtkWidget
 * @state: the state for which to set the base color
 * @color: (allow-none): the color to assign (does not need to be allocated),
 *         or %NULL to undo the effect of previous calls to
 *         of __gtk_widget_modify_base().
 *
 * Sets the base color for a widget in a particular state.
 * All other style values are left untouched. The base color
 * is the background color used along with the text color
 * (see __gtk_widget_modify_text()) for widgets such as #GtkEntry
 * and #GtkTextView. See also __gtk_widget_modify_style().
 *
 * Note that "no window" widgets (which have the %GTK_NO_WINDOW flag set)
 * draw on their parent container's window and thus may not draw any 
 * background themselves. This is the case for e.g. #GtkLabel. To modify 
 ...
 **/
void
__gtk_widget_modify_base (GtkWidget      *widget,
			GtkStateType    state,
			const GdkColor *color)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  ...
  if (color)
    {
      ...
      color_name = __gdk_color_to_string (color);
      g_value_init (&rc_property.value, G_TYPE_STRING);
      g_value_take_string (&rc_property.value, color_name);

      ___gtk_rc_style_set_rc_property (rc_style, &rc_property);

      g_value_unset (&rc_property.value);
    }
  else
    ___gtk_rc_style_unset_rc_property (rc_style, type_name, property_name);
}

/**
 * __gtk_widget_modify_cursor:
 * @widget: a #GtkWidget
 * @primary: the color to use for primary cursor (does not need to be
 *           allocated), or %NULL to undo the effect of previous calls to
 *           of __gtk_widget_modify_cursor().
 ...
 * See also __gtk_widget_modify_style().
 *
 * Since: 2.12
 **/
void
__gtk_widget_modify_cursor (GtkWidget      *widget,
			  ...
			  const GdkColor *secondary)
{
  ...
  rc_style = __gtk_widget_get_modifier_style (widget);
...
/**
 * __gtk_widget_modify_font:
 * @widget: a #GtkWidget
 * @font_desc: (allow-none): the font description to use, or %NULL to undo
 *   the effect of previous calls to __gtk_widget_modify_font().
 *
 * Sets the font to use for a widget.  All other style values are left
 * untouched. See also __gtk_widget_modify_style().
 **/
void
__gtk_widget_modify_font (GtkWidget            *widget,
...
static void
__gtk_widget_set_style_internal (GtkWidget *widget,
			       GtkStyle	 *style,
			       gboolean   initial_emission)
{
  g_object_ref (widget);
  ...
  if (gtk_widget_get_props (widget)->style != style)
    {
      GtkStyle *previous_style;

      if (__gtk_widget_get_realized (widget))
	{
	  __gtk_widget_reset_shapes (widget);
	  __gtk_style_detach (gtk_widget_get_props (widget)->style);
	}
      
      previous_style = gtk_widget_get_props (widget)->style;
      gtk_widget_get_props (widget)->style = style;
      g_object_ref (gtk_widget_get_props (widget)->style);
						...
						gpointer   client_data)
{
  ...
  gboolean new_anchored = __gtk_widget_is_toplevel (widget) ||
                 (gtk_widget_get_props (widget)->parent && GTK_WIDGET_ANCHORED (gtk_widget_get_props (widget)->parent));
...
/**
 * ___gtk_widget_propagate_hierarchy_changed:
 * @widget: a #GtkWidget
 ...
 **/
void
___gtk_widget_propagate_hierarchy_changed (GtkWidget    *widget,
					 GtkWidget    *previous_toplevel)
{
  HierarchyChangedInfo info;

  info.previous_toplevel = previous_toplevel;
  info.previous_screen = previous_toplevel ? __gtk_widget_get_screen (previous_toplevel) : NULL;

  if (__gtk_widget_is_toplevel (widget) ||
      (gtk_widget_get_props (widget)->parent && GTK_WIDGET_ANCHORED (gtk_widget_get_props (widget)->parent)))
    info.new_screen = __gtk_widget_get_screen (widget);
  else
 ...
 * window in its hierarchy (as set by __gdk_window_set_composited()).
 * 
 ...
 */
gboolean
__gtk_widget_is_composited (GtkWidget *widget)
{
  GdkScreen *screen;
  ...
  return __gdk_screen_is_composited (screen);
}
...
void
___gtk_widget_propagate_composited_changed (GtkWidget *widget)
{
  propagate_composited_changed (widget, NULL);
}

/**
 * ___gtk_widget_propagate_screen_changed:
 * @widget: a #GtkWidget
 ...
 **/
void
___gtk_widget_propagate_screen_changed (GtkWidget    *widget,
				      GdkScreen    *previous_screen)
{
  HierarchyChangedInfo info;
 ...
 */
void
__gtk_widget_reset_rc_styles (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
/**
 * __gtk_widget_get_default_style:
 * 
 ...
 */
GtkStyle*
__gtk_widget_get_default_style (void)
{
  if (!gtk_default_style)
    {
      gtk_default_style = __gtk_style_new ();
...
/**
 * __gtk_widget_get_pango_context:
 * @widget: a #GtkWidget
 * 
 * Gets a #PangoContext with the appropriate font map, font description,
 * and base direction for this widget. Unlike the context returned
 * by __gtk_widget_create_pango_context(), this context is owned by
 ...
 **/
PangoContext *
__gtk_widget_get_pango_context (GtkWidget *widget)
{
  ...
  if (!context)
    {
      context = __gtk_widget_create_pango_context (GTK_WIDGET (widget));
      g_object_set_qdata_full (G_OBJECT (widget),
			       quark_pango_context,
			       context,
			       g_object_unref);
...
update_pango_context (GtkWidget    *widget,
		      PangoContext *context)
{
  pango_context_set_font_description (context, gtk_widget_get_props (widget)->style->font_desc);
  pango_context_set_base_dir (context,
			      __gtk_widget_get_direction (widget) == GTK_TEXT_DIR_LTR ?
  ...
  if (context)
      ...
      if (screen)
	{
	  pango_cairo_context_set_resolution (context,
					      __gdk_screen_get_resolution (screen));
	  pango_cairo_context_set_font_options (context,
						__gdk_screen_get_font_options (screen));
	}
    }
}

/**
 * __gtk_widget_create_pango_context:
 * @widget: a #GtkWidget
 *
 * Creates a new #PangoContext with the appropriate font map,
 * font description, and base direction for drawing text for
 * this widget. See also __gtk_widget_get_pango_context().
 *
 * Return value: (transfer full): the new #PangoContext
 **/
PangoContext *
__gtk_widget_create_pango_context (GtkWidget *widget)
{
  GdkScreen *screen;
  PangoContext *context;
  ...
  context = __gdk_pango_context_get_for_screen (screen);

  update_pango_context (widget, context);
  pango_context_set_language (context, __gtk_get_default_language ());

  return context;
}

/**
 * __gtk_widget_create_pango_layout:
 * @widget: a #GtkWidget
 ...
 **/
PangoLayout *
__gtk_widget_create_pango_layout (GtkWidget   *widget,
				const gchar *text)
{
  PangoLayout *layout;
  PangoContext *context;
 ...
 **/
GdkPixbuf*
__gtk_widget_render_icon (GtkWidget      *widget,
                        ...
                        const gchar    *detail)
{
  ...
  icon_set = __gtk_style_lookup_icon_set (gtk_widget_get_props (widget)->style, stock_id);

  if (icon_set == NULL)
    return NULL;

  retval = __gtk_icon_set_render_icon (icon_set,
...
/**
 * __gtk_widget_set_parent_window:
 * @widget: a #GtkWidget.
 ...
 **/
void
__gtk_widget_set_parent_window   (GtkWidget           *widget,
...
/**
 * __gtk_widget_get_parent_window:
 * @widget: a #GtkWidget.
 *
 ...
 **/
GdkWindow *
__gtk_widget_get_parent_window (GtkWidget *widget)
{
  GdkWindow *parent_window;
...
/**
 * __gtk_widget_set_child_visible:
 * @widget: a #GtkWidget
 * @is_visible: if %TRUE, @widget should be mapped along with its parent.
 *
 * Sets whether @widget should be mapped along with its when its parent
 * is mapped and @widget has been shown with __gtk_widget_show(). 
 *
 * The child visibility can be set for widget before it is added to
 * a container with __gtk_widget_set_parent(), to avoid mapping
 ...
 **/
void
__gtk_widget_set_child_visible (GtkWidget *widget,
			      gboolean   is_visible)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  g_return_if_fail (!__gtk_widget_is_toplevel (widget));
  ...
  if (gtk_widget_get_props (widget)->parent && __gtk_widget_get_realized (gtk_widget_get_props (widget)->parent))
    {
      if (__gtk_widget_get_mapped (gtk_widget_get_props (widget)->parent) &&
	  GTK_WIDGET_CHILD_VISIBLE (widget) &&
	  __gtk_widget_get_visible (widget))
	__gtk_widget_map (widget);
      else
...
/**
 * __gtk_widget_get_child_visible:
 * @widget: a #GtkWidget
 * 
 * Gets the value set with __gtk_widget_set_child_visible().
 ...
 **/
gboolean
__gtk_widget_get_child_visible (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
static GdkScreen *
__gtk_widget_get_screen_unchecked (GtkWidget *widget)
{
  GtkWidget *toplevel;
  ...
  if (__gtk_widget_is_toplevel (toplevel))
    {
      if (GTK_IS_WINDOW (toplevel))
	return gtk_window_get_props (GTK_WINDOW (toplevel))->screen;
      else if (GTK_IS_INVISIBLE (toplevel))
	return gtk_invisible_get_props (GTK_INVISIBLE (widget))->screen;
    }
 ...
 **/
GdkScreen*
__gtk_widget_get_screen (GtkWidget *widget)
{
  GdkScreen *screen;
 ...
 **/
gboolean
__gtk_widget_has_screen (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);

  return (__gtk_widget_get_screen_unchecked (widget) != NULL);
}

/**
 * __gtk_widget_get_display:
 * @widget: a #GtkWidget
 * 
 ...
 **/
GdkDisplay*
__gtk_widget_get_display (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
  
  return __gdk_screen_get_display (__gtk_widget_get_screen (widget));
}

/**
 * __gtk_widget_get_root_window:
 * @widget: a #GtkWidget
 * 
 ...
 **/
GdkWindow*
__gtk_widget_get_root_window (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);

  return __gdk_screen_get_root_window (__gtk_widget_get_screen (widget));
}

/**
 * __gtk_widget_child_focus:
 * @widget: a #GtkWidget
 * @direction: direction of focus movement
 *
 * This function is used by custom widget implementations; if you're
 * writing an app, you'd use __gtk_widget_grab_focus() to move the focus
 * to a particular widget, and __gtk_container_set_focus_chain() to
 * change the focus tab order. So you may want to investigate those
 * functions instead.
 * 
 * __gtk_widget_child_focus() is called by containers as the user moves
 * around the window using keyboard shortcuts. @direction indicates
 * what kind of motion is taking place (up, down, left, right, tab
 * forward, tab backward). __gtk_widget_child_focus() emits the
 ...
 * call __gtk_widget_grab_focus() to place the focus accordingly;
                        ...
                        GtkDirectionType direction)
{
  ...
  if (!__gtk_widget_get_visible (widget) ||
      !__gtk_widget_is_sensitive (widget))
    return FALSE;
   ...
   */
  if (!GTK_IS_CONTAINER (widget) &&
      !__gtk_widget_get_can_focus (widget))
    return FALSE;
 ...
 * __gtk_widget_child_focus() on the widget's toplevel.
 *
 ...
 * __gtk_widget_error_bell() to notify the user of the failed keyboard
 * navigation.
 *
 ...
 **/
gboolean
__gtk_widget_keynav_failed (GtkWidget        *widget,
                          GtkDirectionType  direction)
{
  gboolean return_val;
...
/**
 * __gtk_widget_error_bell:
 * @widget: a #GtkWidget
 *
 * Notifies the user about an input-related error on this widget. 
 * If the #GtkSettings:gtk-error-bell setting is %TRUE, it calls
 * __gdk_window_beep(), otherwise it does nothing.
 *
 * Note that the effect of __gdk_window_beep() can be configured in many
 ...
 **/
void
__gtk_widget_error_bell (GtkWidget *widget)
{
  ...
  if (beep && gtk_widget_get_props (widget)->window)
    __gdk_window_beep (gtk_widget_get_props (widget)->window);
}

/**
 * __gtk_widget_set_uposition:
 * @widget: a #GtkWidget
 ...
 * if you call __gtk_window_set_transient_for(), and it's really not
 * possible to get the centering to work correctly in all cases from
 * application code. But if you insist, use __gtk_window_set_position()
 ...
 **/
void
__gtk_widget_set_uposition (GtkWidget *widget,
			  gint	     x,
			  gint	     y)
{
  ...
  aux_info =___gtk_widget_get_aux_info (widget, TRUE);
...
static void
__gtk_widget_set_usize_internal (GtkWidget *widget,
			       gint       width,
			       gint       height)
{
  GtkWidgetAuxInfo *aux_info;
  ...
  if (__gtk_widget_get_visible (widget) && changed)
    __gtk_widget_queue_resize (widget);
 ...
 * __gtk_window_set_default_size() is a better choice for toplevel
 * windows than this function; setting the default size will still
 * allow users to shrink the window. Setting the usize will force them
 * to leave the window at least as large as the usize. When dealing
 * with window sizes, __gtk_window_set_geometry_hints() can be a useful
 * function as well.
 * 
 ...
 * correct.
 * 
 * Deprecated: 2.2: Use __gtk_widget_set_size_request() instead.
 **/
void
__gtk_widget_set_usize (GtkWidget *widget,
		      gint	 width,
		      gint	 height)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  
  __gtk_widget_set_usize_internal (widget, width, height);
}

/**
 * __gtk_widget_set_size_request:
 * @widget: a #GtkWidget
 ...
 * would be.
 *
 * In most cases, __gtk_window_set_default_size() is a better choice for
 * toplevel windows than this function; setting the default size will
 * still allow users to shrink the window. Setting the size request
 * will force them to leave the window at least as large as the size
 * request. When dealing with window sizes,
 * __gtk_window_set_geometry_hints() can be a useful function as well.
 * 
 * Note the inherent danger of setting any fixed size - themes,
 * translations into other languages, different fonts, and user action
 * can all change the appropriate size for a given widget. So, it's
 ...
 **/
void
__gtk_widget_set_size_request (GtkWidget *widget,
                             gint       width,
                             gint       height)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 * __gtk_widget_set_size_request(). A value of -1 stored in @width or
 * @height indicates that that dimension has not been set explicitly
 * and the natural requisition of the widget will be used intead. See
 * __gtk_widget_set_size_request(). To get the size a widget will
 * actually use, call __gtk_widget_size_request() instead of
 * this function.
 **/
void
__gtk_widget_get_size_request (GtkWidget *widget,
                             gint      *width,
                             gint      *height)
{
  GtkWidgetAuxInfo *aux_info;
 ...
 * unrealized. Consider __gtk_widget_add_events() for widgets that are
 ...
 **/
void
__gtk_widget_set_events (GtkWidget *widget,
		       gint	  events)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  g_return_if_fail (!__gtk_widget_get_realized (widget));
...
static void
__gtk_widget_add_events_internal (GtkWidget *widget,
				gint       events,
				GList     *window_list)
{
  GList *l;
  ...
  for (l = window_list; l != NULL; l = l->next)
      ...
      if (user_data == widget)
	{
	  ...
	  __gdk_window_set_events (window, __gdk_window_get_events (window) | events);

	  children = __gdk_window_get_children (window);
	  __gtk_widget_add_events_internal (widget, events, children);
	  g_list_free (children);
	}
    }
}
...
/**
 * __gtk_widget_add_events:
 * @widget: a #GtkWidget
 * @events: an event mask, see #GdkEventMask
 *
 * Adds the events in the bitfield @events to the event mask for
 * @widget. See __gtk_widget_set_events() for details.
 **/
void
__gtk_widget_add_events (GtkWidget *widget,
		       gint	  events)
{
  gint old_events;
  ...
  if (__gtk_widget_get_realized (widget))
	...
	window_list = g_list_prepend (NULL, gtk_widget_get_props (widget)->window);
      ...
      __gtk_widget_add_events_internal (widget, events, window_list);
...
/**
 * __gtk_widget_set_extension_events:
 * @widget: a #GtkWidget
 * @mode: bitfield of extension events to receive
 *
 * Sets the extension events mask to @mode. See #GdkExtensionMode
 * and __gdk_input_set_extension_events().
 **/
void
__gtk_widget_set_extension_events (GtkWidget *widget,
				 GdkExtensionMode mode)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 *
 * Note the difference in behavior vs. __gtk_widget_get_ancestor();
 * <literal>__gtk_widget_get_ancestor (widget, GTK_TYPE_WINDOW)</literal> 
 * would return
 ...
 * __gtk_widget_get_toplevel() and check if the %TOPLEVEL flags
 ...
 **/
GtkWidget*
__gtk_widget_get_toplevel (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
...
/**
 * __gtk_widget_get_ancestor:
 * @widget: a #GtkWidget
 * @widget_type: ancestor type
 * 
 * Gets the first ancestor of @widget with type @widget_type. For example,
 * <literal>__gtk_widget_get_ancestor (widget, GTK_TYPE_BOX)</literal> gets 
 * the first #GtkBox that's an ancestor of @widget. No reference will be 
 * added to the returned widget; it should not be unreferenced. See note 
 * about checking for a toplevel #GtkWindow in the docs for 
 * __gtk_widget_get_toplevel().
 * 
 * Note that unlike __gtk_widget_is_ancestor(), __gtk_widget_get_ancestor() 
 * considers @widget to be an ancestor of itself.
 *
 * Return value: (transfer none): the ancestor widget, or %NULL if not found
 **/
GtkWidget*
__gtk_widget_get_ancestor (GtkWidget *widget,
			 GType      widget_type)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
 ...
 **/
GdkColormap*
__gtk_widget_get_colormap (GtkWidget *widget)
{
  ...
  if (gtk_widget_get_props (widget)->window)
    {
      colormap = __gdk_drawable_get_colormap ((GdkDrawable *)gtk_widget_get_props (widget)->window);
 ...
 **/
GdkVisual*
__gtk_widget_get_visual (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);

  return __gdk_colormap_get_visual (__gtk_widget_get_colormap (widget));
}

/**
 * __gtk_widget_get_settings:
 * @widget: a #GtkWidget
 * 
 ...
 **/
GtkSettings*
__gtk_widget_get_settings (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
  
  return __gtk_settings_get_for_screen (__gtk_widget_get_screen (widget));
}

/**
 * __gtk_widget_set_colormap:
 * @widget: a #GtkWidget
 * @colormap: a colormap
 *
 ...
 **/
void
__gtk_widget_set_colormap (GtkWidget   *widget,
                         GdkColormap *colormap)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  g_return_if_fail (!__gtk_widget_get_realized (widget));
 ...
 **/
gint
__gtk_widget_get_events (GtkWidget *widget)
{
...
/**
 * __gtk_widget_get_extension_events:
 * @widget: a #GtkWidget
 * 
 * Retrieves the extension events the widget will receive; see
 * __gdk_input_set_extension_events().
 * 
 * Return value: extension events for @widget
 **/
GdkExtensionMode
__gtk_widget_get_extension_events (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), 0);
 ...
 * defined as @gtk_widget_get_props (widget)->window coordinates for widgets that are not
 * #GTK_NO_WINDOW widgets, and are relative to @gtk_widget_get_props (widget)->allocation.x,
 * @gtk_widget_get_props (widget)->allocation.y for widgets that are #GTK_NO_WINDOW widgets.
 **/
void
__gtk_widget_get_pointer (GtkWidget *widget,
			gint	  *x,
			gint	  *y)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 **/
gboolean
__gtk_widget_is_ancestor (GtkWidget *widget,
			GtkWidget *ancestor)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
/**
 * __gtk_widget_set_composite_name:
 * @widget: a #GtkWidget.
 * @name: the name to set
 * 
 * Sets a widgets composite name. The widget must be
 * a composite child of its parent; see __gtk_widget_push_composite_child().
 **/
void
__gtk_widget_set_composite_name (GtkWidget   *widget,
			       const gchar *name)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
/**
 * __gtk_widget_get_composite_name:
 * @widget: a #GtkWidget
 *
 ...
 **/
gchar*
__gtk_widget_get_composite_name (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
 ...
 * the corresponding __gtk_widget_pop_composite_child() call.
 * 
 * A composite child is a child that's an implementation detail of the
 * container it's inside and should not be visible to people using the
 * container. Composite children aren't treated differently by GTK (but
 * see __gtk_container_foreach() vs. __gtk_container_forall()), but e.g. GUI 
 * builders might want to treat them in a different way.
 * 
 * Here is a simple example:
 * |[
 *   __gtk_widget_push_composite_child ();
 *   scrolled_window->hscrollbar = __gtk_hscrollbar_new (hadjustment);
 *   __gtk_widget_set_composite_name (scrolled_window->hscrollbar, "hscrollbar");
 *   __gtk_widget_pop_composite_child ();
 *   __gtk_widget_set_parent (scrolled_window->hscrollbar, 
 *                          GTK_WIDGET (scrolled_window));
 *   g_object_ref (scrolled_window->hscrollbar);
 * ]|
 **/
void
__gtk_widget_push_composite_child (void)
{
 ...
 * __gtk_widget_pop_composite_child:
 *
 * Cancels the effect of a previous call to __gtk_widget_push_composite_child().
 **/ 
void
__gtk_widget_pop_composite_child (void)
{
...
/**
 * __gtk_widget_push_colormap:
 * @cmap: a #GdkColormap
 *
 * Pushes @cmap onto a global stack of colormaps; the topmost
 * colormap on the stack will be used to create all widgets.
 * Remove @cmap with __gtk_widget_pop_colormap(). There's little
 * reason to use this function.
 **/
void
__gtk_widget_push_colormap (GdkColormap *cmap)
{
...
/**
 * __gtk_widget_pop_colormap:
 *
 * Removes a colormap pushed with __gtk_widget_push_colormap().
 **/
void
__gtk_widget_pop_colormap (void)
{
  if (colormap_stack)
...
/**
 * __gtk_widget_set_default_colormap:
 * @colormap: a #GdkColormap
 * 
 * Sets the default colormap to use when creating widgets.
 * __gtk_widget_push_colormap() is a better function to use if
 * you only want to affect a few widgets, rather than all widgets.
 **/
void
__gtk_widget_set_default_colormap (GdkColormap *colormap)
{
  g_return_if_fail (GDK_IS_COLORMAP (colormap));
  
  __gdk_screen_set_default_colormap (__gdk_colormap_get_screen (colormap),
				   colormap);
}

/**
 * __gtk_widget_get_default_colormap:
 * 
 ...
 **/
GdkColormap*
__gtk_widget_get_default_colormap (void)
{
  return __gdk_screen_get_default_colormap (__gdk_screen_get_default ());
}

/**
 * __gtk_widget_get_default_visual:
 * 
 * Obtains the visual of the default colormap. Not really useful;
 * used to be useful before __gdk_colormap_get_visual() existed.
 *
 * Return value: (transfer none): visual of the default colormap
 **/
GdkVisual*
__gtk_widget_get_default_visual (void)
{
  return __gdk_colormap_get_visual (__gtk_widget_get_default_colormap ());
}
 ...
 * set by __gtk_widget_set_default_direction() will be used.
 **/
void
__gtk_widget_set_direction (GtkWidget        *widget,
			  GtkTextDirection  dir)
{
  ...
  old_dir = __gtk_widget_get_direction (widget);
  ...
  if (old_dir != __gtk_widget_get_direction (widget))
    gtk_widget_emit_direction_changed (widget, old_dir);
}

/**
 * __gtk_widget_get_direction:
 * @widget: a #GtkWidget
 * 
 ...
 **/
GtkTextDirection
__gtk_widget_get_direction (GtkWidget *widget)
{
...
static void
__gtk_widget_set_default_direction_recurse (GtkWidget *widget, gpointer data)
{
  ...
  if (GTK_IS_CONTAINER (widget))
    __gtk_container_forall (GTK_CONTAINER (widget),
			  __gtk_widget_set_default_direction_recurse,
			  data);
...
/**
 * __gtk_widget_set_default_direction:
 * @dir: the new default direction. This cannot be
 *        %GTK_TEXT_DIR_NONE.
 * 
 * Sets the default reading direction for widgets where the
 * direction has not been explicitly set by __gtk_widget_set_direction().
 **/
void
__gtk_widget_set_default_direction (GtkTextDirection dir)
{
  ...
  if (dir != gtk_default_direction)
    {
      ...
      tmp_list = toplevels = __gtk_window_list_toplevels ();
      g_list_foreach (toplevels, (GFunc)g_object_ref, NULL);
      
      while (tmp_list)
	{
	  __gtk_widget_set_default_direction_recurse (tmp_list->data,
...
/**
 * __gtk_widget_get_default_direction:
 * 
 ...
 **/
GtkTextDirection
__gtk_widget_get_default_direction (void)
{
  return gtk_default_direction;
}
...
/**
 * __gtk_widget_destroy:
 * @widget: a #GtkWidget
 *
 * Destroys a widget. When a widget is
...
__gtk_widget_destroy (GtkWidget *object)
{
  ...
  g_object_unref (gtk_widget_get_props (widget)->style);
  gtk_widget_get_props (widget)->style = __gtk_widget_get_default_style ();
  g_object_ref (gtk_widget_get_props (widget)->style);
...
gtk_widget_finalize (GObject *object)
{
  ...
  g_object_unref (gtk_widget_get_props (widget)->style);
  gtk_widget_get_props (widget)->style = NULL;
...
{
  g_assert (__gtk_widget_get_realized (widget));
...
{
  g_assert (!__gtk_widget_get_has_window (widget));
...
gtk_widget_real_unrealize (GtkWidget *widget)
{
   ...
   */
    ...
    * __gdk_window_destroy() destroys specified xwindow and its sub-xwindows.
  ...
  if (GTK_IS_CONTAINER (widget))
    __gtk_container_forall (GTK_CONTAINER (widget),
			  (GtkCallback) __gtk_widget_unrealize,
			  NULL);

  __gtk_style_detach (gtk_widget_get_props (widget)->style);
  if (__gtk_widget_get_has_window (widget))
    {
      __gdk_window_set_user_data (gtk_widget_get_props (widget)->window, NULL);
  ...
  __gtk_selection_remove_all (widget);
...
gtk_widget_real_size_request (GtkWidget         *widget,
			      GtkRequisition    *requisition)
{
  requisition->width = gtk_widget_get_props (widget)->requisition.width;
  requisition->height = gtk_widget_get_props (widget)->requisition.height;
}

/**
 * ___gtk_widget_peek_colormap:
 * 
 * Returns colormap currently pushed by __gtk_widget_push_colormap, if any.
 * 
 * Return value: the currently pushed colormap, or %NULL if there is none.
 **/
GdkColormap*
___gtk_widget_peek_colormap (void)
{
  if (colormap_stack)
...
/*
 * ___gtk_widget_set_pointer_window:
 * @widget: a #GtkWidget.
 ...
 */
void
___gtk_widget_set_pointer_window (GtkWidget *widget,
                                GdkWindow *pointer_window)
{
  ...
  if (__gtk_widget_get_realized (widget))
    {
      GdkScreen *screen = __gdk_window_get_screen (gtk_widget_get_props (widget)->window);
...
/*
 * ___gtk_widget_get_pointer_window:
 * @widget: a #GtkWidget.
 *
 ...
 */
GdkWindow *
___gtk_widget_get_pointer_window (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
		...
		GdkNotifyType   detail)
{
  ...
  if (!widget)
    widget = __gtk_get_event_widget (event);

  if (widget)
    __gtk_widget_event_internal (widget, event);

  __gdk_event_free (event);
}

/*
 * ___gtk_widget_is_pointer_widget:
 * @widget: a #GtkWidget
 *
 * Returns %TRUE if the pointer window belongs to @widget.
 */
gboolean
___gtk_widget_is_pointer_widget (GtkWidget *widget)
{
  if (GTK_WIDGET_HAS_POINTER (widget))
    {
      ...
      win = ___gtk_widget_get_pointer_window (widget);
      if (win)
        {
          __gdk_window_get_user_data (win, (gpointer *)&wid);
          if (wid == widget)
            return TRUE;
        }
    }
...
/*
 * ___gtk_widget_synthesize_crossing:
 ...
 */
void
___gtk_widget_synthesize_crossing (GtkWidget      *from,
				 GtkWidget      *to,
				 GdkCrossingMode mode)
{
  ...
  if (from_window == NULL && to_window == NULL)
	...
	{
	  from_ancestor = __gdk_window_get_effective_parent (from_ancestor);
	...
	{
	  to_ancestor = __gdk_window_get_effective_parent (to_ancestor);
	  if (to_ancestor == NULL)
            break;
...
gtk_widget_propagate_state (GtkWidget           *widget,
			    GtkStateData        *data)
{
  guint8 old_state = __gtk_widget_get_state (widget);
  guint8 old_saved_state = gtk_widget_get_props (widget)->saved_state;
  ...
  if (__gtk_widget_is_sensitive (widget))
    {
      if (data->state_restoration)
        gtk_widget_get_props (widget)->state = gtk_widget_get_props (widget)->saved_state;
      else
        gtk_widget_get_props (widget)->state = data->state;
    }
  else
    {
      if (!data->state_restoration)
  ...
  if (__gtk_widget_is_focus (widget) && !__gtk_widget_is_sensitive (widget))
    {
      GtkWidget *window;
  ...
  if (old_state != __gtk_widget_get_state (widget) ||
      old_saved_state != gtk_widget_get_props (widget)->saved_state)
    {
      g_object_ref (widget);

      if (!__gtk_widget_is_sensitive (widget) && __gtk_widget_has_grab (widget))
	__gtk_grab_remove (widget);
	...
	{
	  if (!__gtk_widget_is_sensitive (widget))
	    ___gtk_widget_synthesize_crossing (widget, NULL, 
					     GDK_CROSSING_STATE_CHANGED);
	  else if (old_state == GTK_STATE_INSENSITIVE)
	    ___gtk_widget_synthesize_crossing (NULL, widget, 
					     GDK_CROSSING_STATE_CHANGED);
	}

      if (GTK_IS_CONTAINER (widget))
	{
	  data->parent_sensitive = (__gtk_widget_is_sensitive (widget) != FALSE);
	  if (data->use_forall)
	    __gtk_container_forall (GTK_CONTAINER (widget),
 ...
 */
GtkWidgetAuxInfo*
___gtk_widget_get_aux_info (GtkWidget *widget,
			  gboolean   create)
{
  GtkWidgetAuxInfo *aux_info;
...
/**
 * __gtk_widget_shape_combine_mask: 
 * @widget: a #GtkWidget
 ...
 * transparent windows etc., see __gdk_window_shape_combine_mask()
 * for more information.
 **/
void
__gtk_widget_shape_combine_mask (GtkWidget *widget,
			       ...
			       gint	  offset_y)
{
  ...
  g_return_if_fail (__gtk_widget_get_has_window (widget));
  ...
  if (!shape_mask)
    {
      ...
      if (gtk_widget_get_props (widget)->window)
	__gdk_window_shape_combine_mask (gtk_widget_get_props (widget)->window, NULL, 0, 0);
  ...
  else
    {
       ...
       * otherwise the shape is scheduled to be set by __gtk_widget_realize().
       */
      if (gtk_widget_get_props (widget)->window)
	__gdk_window_shape_combine_mask (gtk_widget_get_props (widget)->window, shape_mask,
				       offset_x, offset_y);
    }
}

/**
 * __gtk_widget_input_shape_combine_mask:
 * @widget: a #GtkWidget
 * @shape_mask: (allow-none): shape to be added, or %NULL to remove an existing shape
 * @offset_x: X position of shape mask with respect to @window
 * @offset_y: Y position of shape mask with respect to @window
 ...
 * __gdk_window_input_shape_combine_mask() for more information.
 *
 * Since: 2.10
 **/
void
__gtk_widget_input_shape_combine_mask (GtkWidget *widget,
				     GdkBitmap *shape_mask,
				     gint       offset_x,
				     gint	offset_y)
{
  ...
  if (!shape_mask)
    {
      if (gtk_widget_get_props (widget)->window)
	__gdk_window_input_shape_combine_mask (gtk_widget_get_props (widget)->window, NULL, 0, 0);
  ...
  else
    {
       ...
       */
      if (gtk_widget_get_props (widget)->window)
	__gdk_window_input_shape_combine_mask (gtk_widget_get_props (widget)->window, shape_mask,
					     offset_x, offset_y);
    }
}
			  ...
			  GdkWindow *window)
{
  ...
  __gdk_window_shape_combine_mask (window, NULL, 0, 0);
  for (list = __gdk_window_peek_children (window); list; list = list->next)
    gtk_reset_shapes_recurse (widget, list->data);
}

/**
 * __gtk_widget_reset_shapes:
 * @widget: a #GtkWidget
 *
 ...
 **/
void
__gtk_widget_reset_shapes (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  g_return_if_fail (__gtk_widget_get_realized (widget));
 ...
 **/
GtkWidget*
__gtk_widget_ref (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
 ...
 * @widget: a #GtkWidget
 *
 * Inverse of __gtk_widget_ref(). Equivalent to g_object_unref().
 * 
 * Deprecated: 2.12: Use g_object_unref() instead.
 **/
void
__gtk_widget_unref (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
expose_window (GdkWindow *window)
{
  ...
  if (user_data)
    is_double_buffered = __gtk_widget_get_double_buffered (GTK_WIDGET (user_data));
  else
  ...
  event.expose.area.width = __gdk_window_get_width (window);
  event.expose.area.height = __gdk_window_get_height (window);
  event.expose.region = __gdk_region_rectangle (&event.expose.area);

  /* If this is not double buffered, force a double buffer so that
     redirection works. */
  if (!is_double_buffered)
    __gdk_window_begin_paint_region (window, event.expose.region);
  ...
  children = __gdk_window_peek_children (window);
  for (l = children; l != NULL; l = l->next)
    {
      GdkWindow *child = l->data;

      /* Don't expose input-only windows */
      if (__gdk_drawable_get_depth (GDK_DRAWABLE (child)) != 0)
 ...
 * The function __gdk_pixbuf_get_from_drawable() can be used to convert
 ...
 **/
GdkPixmap*
__gtk_widget_get_snapshot (GtkWidget    *widget,
                         GdkRectangle *clip_rect)
{
  ...
  if (gtk_widget_get_props (widget)->parent && __gtk_widget_get_has_window (widget))
    {
      ...
      for (list = __gdk_window_peek_children (parent_window); list; list = list->next)
        {
          GdkWindow *subwin = list->data;
          gpointer windata;
          int wx, wy, ww, wh;
          __gdk_window_get_user_data (subwin, &windata);
          if (windata != widget)
            continue;
          windows = g_list_prepend (windows, subwin);
          __gdk_window_get_position (subwin, &wx, &wy);
          ww = __gdk_window_get_width (subwin);
          wh = __gdk_window_get_height (subwin);
  ...
  else if (!gtk_widget_get_props (widget)->parent)
    x = y = 0; /* toplevel */
      ...
      if (gtk_widget_get_props (widget)->parent)
        {
          ...
          if (clip_rect->x >= 0)
            clip.x += gtk_widget_get_props (widget)->allocation.x;
          if (clip_rect->y >= 0)
            clip.y += gtk_widget_get_props (widget)->allocation.y;
        }
      if (!__gdk_rectangle_intersect (&snap, &clip, &snap))
        {
          g_list_free (windows);
  ...
  for (list = windows; list; list = list->next) /* !NO_WINDOW widgets */
    {
      GdkWindow *subwin = list->data;
      int wx, wy;
      if (__gdk_drawable_get_depth (GDK_DRAWABLE (subwin)) == 0)
	continue; /* Input only window */
      __gdk_window_get_position (subwin, &wx, &wy);
      __gdk_window_redirect_to_drawable (subwin, pixmap, MAX (0, x - wx), MAX (0, y - wy),
                                       MAX (0, wx - x), MAX (0, wy - y), width, height);

      expose_window (subwin);
    }
  if (!windows) /* NO_WINDOW || toplevel => parent_window == NULL || parent_window == gtk_widget_get_props (widget)->window */
    {
      __gdk_window_redirect_to_drawable (gtk_widget_get_props (widget)->window, pixmap, x, y, 0, 0, width, height);
      expose_window (gtk_widget_get_props (widget)->window);
    }
  for (list = windows; list; list = list->next)
    __gdk_window_remove_redirection (list->data);
  if (!windows) /* NO_WINDOW || toplevel */
    __gdk_window_remove_redirection (gtk_widget_get_props (widget)->window);
  g_list_free (windows);
      ...
      if (gtk_widget_get_props (widget)->parent)
        {
          ...
          clip_rect->x -= gtk_widget_get_props (widget)->allocation.x;
          clip_rect->y -= gtk_widget_get_props (widget)->allocation.y;
        }
      if (0)
        g_printerr ("__gtk_widget_get_snapshot: %s (%d,%d, %dx%d)\n",
                    G_OBJECT_TYPE_NAME (widget),
...
/**
 * __gtk_widget_class_install_style_property_parser:
 ...
 **/
void
__gtk_widget_class_install_style_property_parser (GtkWidgetClass     *klass,
						GParamSpec         *pspec,
...
/**
 * __gtk_widget_class_install_style_property:
 * @klass: a #GtkWidgetClass
 ...
 **/
void
__gtk_widget_class_install_style_property (GtkWidgetClass *klass,
					 GParamSpec     *pspec)
{
  GtkRcPropertyParser parser;
  ...
  parser = ___gtk_rc_property_parser_from_type (G_PARAM_SPEC_VALUE_TYPE (pspec));

  __gtk_widget_class_install_style_property_parser (klass, pspec, parser);
}

/**
 * __gtk_widget_class_find_style_property:
 * @klass: a #GtkWidgetClass
 ...
 */
GParamSpec*
__gtk_widget_class_find_style_property (GtkWidgetClass *klass,
...
/**
 * __gtk_widget_class_list_style_properties:
 * @klass: a #GtkWidgetClass
 ...
 */
GParamSpec**
__gtk_widget_class_list_style_properties (GtkWidgetClass *klass,
					guint          *n_properties)
{
  GParamSpec **pspecs;
  guint n;
...
/**
 * __gtk_widget_style_get_property:
 * @widget: a #GtkWidget
 ...
 */
void
__gtk_widget_style_get_property (GtkWidget   *widget,
			       const gchar *property_name,
			       GValue      *value)
{
  GParamSpec *pspec;
  ...
  if (!pspec)
  ...
  else
    {
      ...
      peek_value = ___gtk_style_peek_property_value (gtk_widget_get_props (widget)->style,
						   G_OBJECT_TYPE (widget),
						   pspec,
 ...
 * 
 * Non-vararg variant of __gtk_widget_style_get(). Used primarily by language 
 * bindings.
 */ 
void
__gtk_widget_style_get_valist (GtkWidget   *widget,
			     const gchar *first_property_name,
			     va_list      var_args)
{
  const gchar *name;
 ...
 */
void
__gtk_widget_style_get (GtkWidget   *widget,
		      const gchar *first_property_name,
		      ...)
{
  va_list var_args;
  ...
  va_start (var_args, first_property_name);
  __gtk_widget_style_get_valist (widget, first_property_name, var_args);
  va_end (var_args);
}
 ...
 * __gtk_widget_get_name(). Paths are used to apply styles to a widget
 * in gtkrc configuration files. Widget names are the type of the
 * widget by default (e.g. "GtkButton") or can be set to an
 * application-specific value with __gtk_widget_set_name(). By setting
 ...
 **/
void
__gtk_widget_path (GtkWidget *widget,
		 guint     *path_length,
		 gchar    **path,
		 gchar    **path_reversed)
{
  ...
  do
    {
      ...
      string = __gtk_widget_get_name (widget);
      l = strlen (string);
 ...
 *    string, or %NULL
 *
 * Same as __gtk_widget_path(), but always uses the name of a widget's type,
 * never uses a custom name set with __gtk_widget_set_name().
 * 
 **/
void
__gtk_widget_class_path (GtkWidget *widget,
		       guint     *path_length,
		       gchar    **path,
		       gchar    **path_reversed)
{
 ...
 **/
GtkRequisition *
__gtk_requisition_copy (const GtkRequisition *requisition)
{
 ...
 **/
void
__gtk_requisition_free (GtkRequisition *requisition)
{
  g_free (requisition);
}
 ...
 */
AtkObject*
__gtk_widget_get_accessible (GtkWidget *widget)
{
...
{
  iface->ref_accessible = __gtk_widget_ref_accessible;
}

static AtkObject*
__gtk_widget_ref_accessible (AtkImplementor *implementor)
{
  AtkObject *accessible;

  accessible = __gtk_widget_get_accessible (GTK_WIDGET (implementor));
  if (accessible)
					 ...
					 const gchar  *childname)
{
  if (strcmp (childname, "accessible") == 0)
    return G_OBJECT (__gtk_widget_get_accessible (GTK_WIDGET (buildable)));
				      ...
				      GtkBuilder   *builder)
{
  ...
  if (g_object_get_qdata (G_OBJECT (buildable), quark_builder_has_default))
    __gtk_widget_grab_default (GTK_WIDGET (buildable));
  if (g_object_get_qdata (G_OBJECT (buildable), quark_builder_has_focus))
    __gtk_widget_grab_focus (GTK_WIDGET (buildable));
  ...
  if (atk_relations)
    {
      ...
      accessible = __gtk_widget_get_accessible (GTK_WIDGET (buildable));
      ...
      for (l = atk_relations; l; l = l->next)
	{
	  ...
	  target = __gtk_builder_get_object (builder, relation->target);
	  if (!target)
	    {
	  ...
	  target_accessible = __gtk_widget_get_accessible (GTK_WIDGET (target));
			   ...
			   GError              **error)
{
  ...
  for (i = 0; names[i]; i++)
	...
	key = __gdk_keyval_from_name (values[i]);
...
void
___gtk_widget_buildable_finish_accelerator (GtkWidget *widget,
					  GtkWidget *toplevel,
					  gpointer   user_data)
{
  ...
  accel_groups = __gtk_accel_groups_from_object (G_OBJECT (toplevel));
  if (g_slist_length (accel_groups) == 0)
    {
      accel_group = __gtk_accel_group_new ();
      __gtk_window_add_accel_group (GTK_WINDOW (toplevel), accel_group);
    }
  else
    {
  ...
  __gtk_widget_add_accelerator (GTK_WIDGET (accel_data->object),
			      accel_data->signal,
			      accel_group,
			      accel_data->key,
  ...
  if (strcmp (tagname, "accelerator") == 0)
    {
      ...
      toplevel = __gtk_widget_get_toplevel (GTK_WIDGET (accel_data->object));
	          ...
	          if (i < n_actions)
                    {
                      ...
                      if (action_data->translatable && action_data->description->len)
                        description = _gtk_builder_parser_translate (__gtk_builder_get_translation_domain (builder),
                                                                     action_data->context,
 ...
 **/
GtkClipboard *
__gtk_widget_get_clipboard (GtkWidget *widget, GdkAtom selection)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
  g_return_val_if_fail (__gtk_widget_has_screen (widget), NULL);
  
  return __gtk_clipboard_get_for_display (__gtk_widget_get_display (widget),
					selection);
}

/**
 * __gtk_widget_list_mnemonic_labels:
 * @widget: a #GtkWidget
 * 
 * Returns a newly allocated list of the widgets, normally labels, for 
 * which this widget is a the target of a mnemonic (see for example, 
 * __gtk_label_set_mnemonic_widget()).
 ...
 **/
GList *
__gtk_widget_list_mnemonic_labels (GtkWidget *widget)
{
  GList *list = NULL;
  GSList *l;
...
/**
 * __gtk_widget_add_mnemonic_label:
 * @widget: a #GtkWidget
 * @label: a #GtkWidget that acts as a mnemonic label for @widget
 * 
 * Adds a widget to the list of mnemonic labels for
 * this widget. (See __gtk_widget_list_mnemonic_labels()). Note the
 ...
 **/
void
__gtk_widget_add_mnemonic_label (GtkWidget *widget,
                               GtkWidget *label)
{
...
/**
 * __gtk_widget_remove_mnemonic_label:
 * @widget: a #GtkWidget
 * @label: a #GtkWidget that was previously set as a mnemnic label for
 *         @widget with __gtk_widget_add_mnemonic_label().
 * 
 * Removes a widget from the list of mnemonic labels for
 * this widget. (See __gtk_widget_list_mnemonic_labels()). The widget
 ...
 **/
void
__gtk_widget_remove_mnemonic_label (GtkWidget *widget,
                                  GtkWidget *label)
{
  GSList *old_list, *new_list;
...
/**
 * __gtk_widget_get_no_show_all:
 * @widget: a #GtkWidget
 * 
 * Returns the current value of the GtkWidget:no-show-all property, 
 * which determines whether calls to __gtk_widget_show_all() and 
 * __gtk_widget_hide_all() will affect this widget. 
 * 
 ...
 **/
gboolean
__gtk_widget_get_no_show_all (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
...
/**
 * __gtk_widget_set_no_show_all:
 * @widget: a #GtkWidget
 * @no_show_all: the new value for the "no-show-all" property
 * 
 * Sets the #GtkWidget:no-show-all property, which determines whether 
 * calls to __gtk_widget_show_all() and __gtk_widget_hide_all() will affect 
 * this widget. 
 *
 ...
 **/
void
__gtk_widget_set_no_show_all (GtkWidget *widget,
			    gboolean   no_show_all)
{
  ...
  if (no_show_all == __gtk_widget_get_no_show_all (widget))
    return;
  ...
  if (priv_has_tooltip != has_tooltip || force)
    {
	  ...
	  if (__gtk_widget_get_has_window (widget))
	      __gtk_widget_add_events (widget,
				     GDK_LEAVE_NOTIFY_MASK |
				     GDK_POINTER_MOTION_MASK |
				     GDK_POINTER_MOTION_HINT_MASK);
	}
...
/**
 * __gtk_widget_set_tooltip_window:
 * @widget: a #GtkWidget
 ...
 * have the name "gtk-tooltip", see __gtk_widget_set_name().
 *
 * Since: 2.12
 */
void
__gtk_widget_set_tooltip_window (GtkWidget *widget,
			       GtkWindow *custom_window)
{
  ...
  if (has_tooltip && __gtk_widget_get_visible (widget))
    gtk_widget_queue_tooltip_query (widget);
}

/**
 * __gtk_widget_get_tooltip_window:
 * @widget: a #GtkWidget
 *
 * Returns the #GtkWindow of the current tooltip. This can be the
 * GtkWindow created by default, or the custom tooltip window set
 * using __gtk_widget_set_tooltip_window().
 *
 ...
 */
GtkWindow *
__gtk_widget_get_tooltip_window (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);
...
/**
 * __gtk_widget_trigger_tooltip_query:
 * @widget: a #GtkWidget
 *
 * Triggers a tooltip query on the display where the toplevel of @widget
 * is located. See __gtk_tooltip_trigger_tooltip_query() for more
 * information.
 *
 * Since: 2.12
 */
void
__gtk_widget_trigger_tooltip_query (GtkWidget *widget)
{
  __gtk_tooltip_trigger_tooltip_query (__gtk_widget_get_display (widget));
}
...
{
  g_slist_foreach (tooltip_query_displays, (GFunc)__gtk_tooltip_trigger_tooltip_query, NULL);
...
gtk_widget_queue_tooltip_query (GtkWidget *widget)
{
  ...
  if (tooltip_query_id == 0)
    tooltip_query_id = __gdk_threads_add_idle (tooltip_query_idle, NULL);
}

/**
 * __gtk_widget_set_tooltip_text:
 * @widget: a #GtkWidget
 ...
 *
 * See also the GtkWidget:tooltip-text property and __gtk_tooltip_set_text().
 *
 * Since: 2.12
 */
void
__gtk_widget_set_tooltip_text (GtkWidget   *widget,
                             const gchar *text)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 */
gchar *
__gtk_widget_get_tooltip_text (GtkWidget *widget)
{
  gchar *text = NULL;
...
/**
 * __gtk_widget_set_tooltip_markup:
 * @widget: a #GtkWidget
 ...
 * __gtk_tooltip_set_markup().
 *
 * Since: 2.12
 */
void
__gtk_widget_set_tooltip_markup (GtkWidget   *widget,
                               const gchar *markup)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
/**
 * __gtk_widget_get_tooltip_markup:
 * @widget: a #GtkWidget
 *
 * Gets the contents of the tooltip for @widget.
 *
 ...
 */
gchar *
__gtk_widget_get_tooltip_markup (GtkWidget *widget)
{
  gchar *text = NULL;
 ...
 */
void
__gtk_widget_set_has_tooltip (GtkWidget *widget,
			    gboolean   has_tooltip)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
 ...
 */
gboolean
__gtk_widget_get_has_tooltip (GtkWidget *widget)
{
 ...
 */
void
__gtk_widget_get_allocation (GtkWidget     *widget,
                           GtkAllocation *allocation)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  g_return_if_fail (allocation != NULL);

  *allocation = gtk_widget_get_props (widget)->allocation;
}

/**
 * __gtk_widget_set_allocation:
 * @widget: a #GtkWidget
 ...
 */
void
__gtk_widget_set_allocation (GtkWidget           *widget,
 ...
 * __gtk_widget_queue_resize() instead of __gtk_widget_queue_draw()).
 *
 * Normally, __gtk_widget_size_request() should be used.
 *
 * Since: 2.20
 */
void
__gtk_widget_get_requisition (GtkWidget      *widget,
                            GtkRequisition *requisition)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
/**
 * __gtk_widget_set_window:
 * @widget: a #GtkWidget
 * @window: a #GdkWindow
 *
 * Sets a widget's window. This function should only be used in a
 * widget's GtkWidget::realize() implementation. The %window passed is
 * usually either new window created with __gdk_window_new(), or the
 * window of its parent widget as returned by
 * __gtk_widget_get_parent_window().
 *
 * Widgets must indicate whether they will create their own #GdkWindow
 * by calling __gtk_widget_set_has_window(). This is usually done in the
 * widget's init() function.
 *
 * Since: 2.18
 */
void
__gtk_widget_set_window (GtkWidget *widget,
                       GdkWindow *window)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
  g_return_if_fail (window == NULL || GDK_IS_WINDOW (window));

  if (gtk_widget_get_props (widget)->window != window)
    {
      gtk_widget_get_props (widget)->window = window;
      g_object_notify (G_OBJECT (widget), "window");
    }
}

/**
 * __gtk_widget_get_window:
 * @widget: a #GtkWidget
 *
 ...
 */
GdkWindow*
__gtk_widget_get_window (GtkWidget *widget)
{
  g_return_val_if_fail (GTK_IS_WIDGET (widget), NULL);

  return gtk_widget_get_props (widget)->window;
}

static void
___gtk_widget_set_has_focus (GtkWidget *widget,
                           gboolean   has_focus)
{
  if (has_focus)
...
/**
 * __gtk_widget_send_focus_change:
 * @widget: a #GtkWidget
 ...
 *   fevent->focus_change.window = __gtk_widget_get_window (widget);
 ...
 */
gboolean
__gtk_widget_send_focus_change (GtkWidget *widget,
                              GdkEvent  *event)
{
  gboolean res;
  ...
  ___gtk_widget_set_has_focus (widget, event->focus_change.in);

  res = __gtk_widget_event (widget, event);
