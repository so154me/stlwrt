...
gtk_file_chooser_entry_get_completion_text (GtkFileChooserEntry *chooser_entry)
{
  ...
  return __gtk_editable_get_chars (editable, 0, MIN (start, end));
}
                                                    ...
                                                    GParamSpec **pspecs)
{
  ...
  if (__gtk_widget_get_parent (GTK_WIDGET (object)) != NULL)
    {
...
_gtk_file_chooser_entry_init (GtkFileChooserEntry *chooser_entry)
{
  ...
  comp = __gtk_entry_completion_new ();
  __gtk_entry_completion_set_popup_single_match (comp, FALSE);
  __gtk_entry_completion_set_minimum_key_length (comp, 0);
  /* see docs for __gtk_entry_completion_set_text_column() */
  ...
  __gtk_entry_completion_set_match_func (comp,
				       (GtkEntryCompletionMatchFunc) __gtk_true,
				       chooser_entry,
				       NULL);

  cell = __gtk_cell_renderer_text_new ();
  __gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (comp),
                              cell, TRUE);
  __gtk_cell_layout_add_attribute (GTK_CELL_LAYOUT (comp),
                                 cell,
  ...
  __gtk_entry_set_completion (GTK_ENTRY (chooser_entry), comp);
  g_object_unref (comp);
                         ...
                         GtkFileChooserEntry *chooser_entry)
{
  ...
  __gtk_editable_delete_text (GTK_EDITABLE (chooser_entry),
                            0,
                            __gtk_editable_get_position (GTK_EDITABLE (chooser_entry)));
  pos = 0;
  __gtk_editable_insert_text (GTK_EDITABLE (chooser_entry),
                            path,
                            -1,
                            &pos);

  __gtk_editable_set_position (GTK_EDITABLE (chooser_entry), pos);
...
static GFile *
__gtk_file_chooser_get_file_for_text (GtkFileChooserEntry *chooser_entry,
                                    const gchar         *str)
{
  GFile *file;
                                         ...
                                         const char *         text)
{
  ...
  file = __gtk_file_chooser_get_file_for_text (chooser_entry, text);
  ...
  if (_gtk_file_chooser_entry_get_props(chooser_entry)->completion_store)
    {
      ...
      completion = _gtk_entry_completion_compute_prefix (__gtk_entry_get_completion (GTK_ENTRY (chooser_entry)), text);
      completion_len = completion ? strlen (completion) : 0;

      if (completion_len > text_len)
        {
          GtkEditable *editable = GTK_EDITABLE (chooser_entry);
          int pos = __gtk_editable_get_position (editable);

          __gtk_editable_insert_text (editable,
                                    completion + text_len,
				    ...
				    GdkEventKey *event)
{
  ...
  if (start != end)
    __gtk_editable_set_position (editable, MAX (start, end));
  else
...
update_inline_completion (GtkFileChooserEntry *chooser_entry)
{
  ...
  switch (_gtk_file_chooser_entry_get_props(chooser_entry)->action)
    ...
    case GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER:
      __gtk_entry_completion_set_inline_completion (completion, TRUE);
      break;
		     ...
		     GtkFileChooserEntry *chooser_entry)
{
  ...
  completion = __gtk_entry_get_completion (GTK_ENTRY (chooser_entry));
  update_inline_completion (chooser_entry);

  if (__gtk_widget_has_focus (GTK_WIDGET (chooser_entry)))
    {
		      ...
		      gpointer   	   user_data)
{
  ...
  if (i < new_text_length ||
      ...
      (new_text_length > 0 &&
       *position <= 1 &&
       __gtk_entry_get_text_length (GTK_ENTRY (chooser_entry)) >= 2 &&
       __gtk_entry_get_text (GTK_ENTRY (chooser_entry))[1] == ':'))
    {
      __gtk_widget_error_bell (GTK_WIDGET (chooser_entry));
  ...
  if (start_pos == 0 && end_pos == 1 &&
      ...
      __gtk_entry_get_text (GTK_ENTRY (chooser_entry))[1] == ':')
    {
      g_signal_handlers_block_by_func (chooser_entry,
				       ...
				       user_data);
      __gtk_editable_delete_text (GTK_EDITABLE (chooser_entry), 0, 1);
...
_gtk_file_chooser_entry_get_current_folder (GtkFileChooserEntry *chooser_entry)
{
  ...
  return gtk_file_chooser_get_directory_for_text (chooser_entry,
                                                  __gtk_entry_get_text (GTK_ENTRY (chooser_entry)));
}
...
_gtk_file_chooser_entry_get_file_part (GtkFileChooserEntry *chooser_entry)
{
  ...
  text = __gtk_entry_get_text (GTK_ENTRY (chooser_entry));
  last_slash = strrchr (text, G_DIR_SEPARATOR);
  if (last_slash)
    return last_slash + 1;
				    ...
				    GtkFileChooserAction action)
{
  ...
  if (_gtk_file_chooser_entry_get_props(chooser_entry)->action != action)
	...
	case GTK_FILE_CHOOSER_ACTION_SAVE:
	case GTK_FILE_CHOOSER_ACTION_CREATE_FOLDER:
	  __gtk_entry_completion_set_popup_single_match (comp, TRUE);
	  break;
	}
...
_gtk_file_chooser_entry_select_filename (GtkFileChooserEntry *chooser_entry)
{
  ...
  __gtk_editable_select_region (GTK_EDITABLE (chooser_entry), 0, (gint) len);
}
