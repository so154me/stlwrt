  ...
  for (l = priv->choosers; l; l = l->next)
    {
      ...
      if (!__gtk_recent_chooser_set_current_uri (recent_chooser, uri, error))
        return FALSE;
    }
...
gtk_recent_action_get_current_uri (GtkRecentChooser *chooser)
{
  ...
  if (priv->current_chooser)
    return __gtk_recent_chooser_get_current_uri (priv->current_chooser);
  ...
  for (l = priv->choosers; l; l = l->next)
    {
      ...
      if (!__gtk_recent_chooser_select_uri (recent_chooser, uri, error))
        return FALSE;
    }
  ...
  for (l = priv->choosers; l; l = l->next)
    {
      ...
      __gtk_recent_chooser_unselect_uri (chooser, uri);
    }
}
  ...
  for (l = priv->choosers; l; l = l->next)
    {
      ...
      __gtk_recent_chooser_set_sort_func (chooser_menu, priv->sort_func,
                                        priv->sort_data,
                                        priv->data_destroy);
    }
}
                                 ...
                                 GtkWidget *widget)
{
  ...
  if (GTK_IS_RECENT_CHOOSER (widget) &&
      !g_slist_find (priv->choosers, widget))
    {
      if (priv->sort_func)
        {
          __gtk_recent_chooser_set_sort_func (GTK_RECENT_CHOOSER (widget),
                                            priv->sort_func,
                                            priv->sort_data,
                                            priv->data_destroy);
        }
...
gtk_recent_action_create_menu_item (GtkAction *action)
{
  ...
  __gtk_menu_item_set_submenu (GTK_MENU_ITEM (menuitem), menu);
...
gtk_recent_action_create_tool_item (GtkAction *action)
{
  ...
  __gtk_menu_tool_button_set_menu (GTK_MENU_TOOL_BUTTON (toolitem), menu);
  __gtk_widget_show (menu);
 ...
 */
GtkAction *
__gtk_recent_action_new (const gchar *name,
                       const gchar *label,
                       const gchar *tooltip,
...
/**
 * __gtk_recent_action_new_for_manager:
 * @name: a unique name for the action
 * @label: (allow-none): the label displayed in menu items and on buttons, or %NULL
 * @tooltip: (allow-none): a tooltip for the action, or %NULL
 * @stock_id: the stock icon to display in widgets representing the
 ...
 */
GtkAction *
__gtk_recent_action_new_for_manager (const gchar      *name,
                                   const gchar      *label,
                                   const gchar      *tooltip,
                                   const gchar      *stock_id,
                                   GtkRecentManager *manager)
...
/**
 * __gtk_recent_action_get_show_numbers:
 ...
 */
gboolean
__gtk_recent_action_get_show_numbers (GtkRecentAction *action)
{
...
/**
 * __gtk_recent_action_set_show_numbers:
 * @action: a #GtkRecentAction
 ...
 */
void
__gtk_recent_action_set_show_numbers (GtkRecentAction *action,
