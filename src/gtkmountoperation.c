 ...
 *
 * __gtk_show_uri() is a convenient way to launch applications for URIs.
 *
                                  ...
                                  GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_PARENT:
      __gtk_mount_operation_set_parent (operation, g_value_get_object (value));
      break;

    case PROP_SCREEN:
      __gtk_mount_operation_set_screen (operation, g_value_get_object (value));
      break;
                         ...
                         GtkMountOperation *operation)
{
  ...
  if (__gtk_toggle_button_get_active (button))
    {
      gpointer data;
                        ...
                        GtkMountOperation *mount_op)
{
  ...
  if (response_id == GTK_RESPONSE_OK)
    {
      ...
      if (priv->username_entry)
        {
          text = __gtk_entry_get_text (GTK_ENTRY (priv->username_entry));
          g_mount_operation_set_username (op, text);
        }

      if (priv->domain_entry)
        {
          text = __gtk_entry_get_text (GTK_ENTRY (priv->domain_entry));
          g_mount_operation_set_domain (op, text);
        }

      if (priv->password_entry)
        {
          text = __gtk_entry_get_text (GTK_ENTRY (priv->password_entry));
  ...
  __gtk_widget_destroy (GTK_WIDGET (dialog));
  g_object_unref (op);
}
...
entry_has_input (GtkWidget *entry_widget)
{
  ...
  text = __gtk_entry_get_text (GTK_ENTRY (entry_widget));
                        ...
                        GtkMountOperation *operation)
{
  ...
  __gtk_dialog_set_response_sensitive (GTK_DIALOG (priv->dialog),
                                     GTK_RESPONSE_OK,
                                     is_valid);
}
                             ...
                             GtkMountOperation *operation)
{
  ...
  __gtk_widget_set_sensitive (priv->entry_container, priv->anonymous == FALSE);
  __gtk_dialog_set_response_sensitive (GTK_DIALOG (priv->dialog),
                                     GTK_RESPONSE_OK,
                                     is_valid);
}
                       ...
                       GtkMountOperation *operation)
{
  ...
  if (next_widget)
    __gtk_widget_grab_focus (next_widget);
  else if (pw_dialog_input_is_valid (operation))
    __gtk_window_activate_default (GTK_WINDOW (priv->dialog));
}
                 ...
                 gpointer    user_data)
{
  ...
  if (value)
    __gtk_entry_set_text (GTK_ENTRY (entry), value);

  __gtk_table_attach (GTK_TABLE (table), label,
                    0, 1, row, row + 1,
                    GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  __gtk_table_attach_defaults (GTK_TABLE (table), entry,
                             1, 2, row, row + 1);
  __gtk_label_set_mnemonic_widget (GTK_LABEL (label), entry);
                                  ...
                                  GAskPasswordFlags  flags)
{
  ...
  __gtk_window_set_icon_name (window, GTK_STOCK_DIALOG_AUTHENTICATION);

  __gtk_dialog_add_buttons (dialog,
                          GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
                          _("Co_nnect"), GTK_RESPONSE_OK,
                          NULL);
  __gtk_dialog_set_default_response (dialog, GTK_RESPONSE_OK);

  __gtk_dialog_set_alternative_button_order (dialog,
                                           GTK_RESPONSE_OK,
                                           GTK_RESPONSE_CANCEL,
                                           -1);
  ...
  __gtk_misc_set_alignment (GTK_MISC (icon), 0.5, 0.0);
  __gtk_box_pack_start (GTK_BOX (hbox), icon, FALSE, FALSE, 0);

  main_vbox = __gtk_vbox_new (FALSE, 18);
  __gtk_box_pack_start (GTK_BOX (hbox), main_vbox, TRUE, TRUE, 0);
  ...
  if (secondary != NULL)
    {
      ...
      message_label = __gtk_label_new (NULL);
      __gtk_label_set_markup (GTK_LABEL (message_label), s);
      __gtk_misc_set_alignment (GTK_MISC (message_label), 0.0, 0.5);
      __gtk_label_set_line_wrap (GTK_LABEL (message_label), TRUE);
      __gtk_box_pack_start (GTK_BOX (main_vbox), GTK_WIDGET (message_label),
                          FALSE, TRUE, 0);
    ...
    {
      message_label = __gtk_label_new (message);
      __gtk_misc_set_alignment (GTK_MISC (message_label), 0.0, 0.5);
      __gtk_label_set_line_wrap (GTK_LABEL (message_label), TRUE);
      __gtk_box_pack_start (GTK_BOX (main_vbox), GTK_WIDGET (message_label),
                          FALSE, FALSE, 0);
  ...
  __gtk_box_pack_start (GTK_BOX (main_vbox), vbox, FALSE, FALSE, 0);
  ...
  if (can_anonymous)
    {
      ...
      anon_box = __gtk_vbox_new (FALSE, 6);
      __gtk_box_pack_start (GTK_BOX (vbox), anon_box,
                          FALSE, FALSE, 0);

      choice = __gtk_radio_button_new_with_mnemonic (NULL, _("Connect _anonymously"));
      __gtk_box_pack_start (GTK_BOX (anon_box),
                          choice,
                          FALSE, FALSE, 0);
      ...
      group = __gtk_radio_button_get_group (GTK_RADIO_BUTTON (choice));
      choice = __gtk_radio_button_new_with_mnemonic (group, _("Connect as u_ser:"));
      __gtk_box_pack_start (GTK_BOX (anon_box),
                          choice,
                          FALSE, FALSE, 0);
      g_signal_connect (choice, "toggled",
  ...
  entry_container = __gtk_alignment_new (0.0, 0.0, 1.0, 1.0);

  __gtk_alignment_set_padding (GTK_ALIGNMENT (entry_container),
                             0, 0, can_anonymous ? 12 : 0, 0);

  __gtk_box_pack_start (GTK_BOX (vbox), entry_container,
                      FALSE, FALSE, 0);
  priv->entry_container = entry_container;

  table = __gtk_table_new (rows, 2, FALSE);
  __gtk_table_set_col_spacings (GTK_TABLE (table), 12);
  __gtk_table_set_row_spacings (GTK_TABLE (table), 6);
  __gtk_container_add (GTK_CONTAINER (entry_container), table);
  ...
  if (flags & G_ASK_PASSWORD_NEED_PASSWORD)
    {
      priv->password_entry = table_add_entry (table, rows++, _("_Password:"),
                                              NULL, operation);
      __gtk_entry_set_visibility (GTK_ENTRY (priv->password_entry), FALSE);
    }
   ...
   if (flags & G_ASK_PASSWORD_SAVING_SUPPORTED)
    {
      ...
      remember_box = __gtk_vbox_new (FALSE, 6);
      __gtk_box_pack_start (GTK_BOX (vbox), remember_box,
                          FALSE, FALSE, 0);

      password_save = g_mount_operation_get_password_save (mount_op);
      
      choice = __gtk_radio_button_new_with_mnemonic (NULL, _("Forget password _immediately"));
      __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (choice),
      ...
      __gtk_box_pack_start (GTK_BOX (remember_box), choice, FALSE, FALSE, 0);

      group = __gtk_radio_button_get_group (GTK_RADIO_BUTTON (choice));
      choice = __gtk_radio_button_new_with_mnemonic (group, _("Remember password until you _logout"));
      __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (choice),
      ...
      group = __gtk_radio_button_get_group (GTK_RADIO_BUTTON (choice));
      choice = __gtk_radio_button_new_with_mnemonic (group, _("Remember _forever"));
      __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (choice),
  ...
  if (can_anonymous)
    {
       ...
       */
      __gtk_toggle_button_toggled (GTK_TOGGLE_BUTTON (priv->anonymous_toggle));
    }
  else if (! pw_dialog_input_is_valid (operation))
    __gtk_dialog_set_response_sensitive (dialog, GTK_RESPONSE_OK, FALSE);

  g_object_notify (G_OBJECT (operation), "is-showing");

  if (priv->parent_window)
    {
      __gtk_window_set_transient_for (window, priv->parent_window);
      __gtk_window_set_modal (window, TRUE);
    }
  else if (priv->screen)
    __gtk_window_set_screen (GTK_WINDOW (dialog), priv->screen);

  __gtk_widget_show_all (GTK_WIDGET (dialog));
                                  ...
                                  const char      *choices[])
{
  ...
  dialog = __gtk_message_dialog_new (priv->parent_window, 0,
  ...
  for (count = len - 1; count >= 0; count--)
    __gtk_dialog_add_button (GTK_DIALOG (dialog), choices[count], count);
                               ...
                               GPid                            pid)
{
  ...
  if (pixbuf == NULL)
    {
      GtkIconTheme *theme;
      theme = __gtk_icon_theme_get_for_screen (__gtk_widget_get_screen (GTK_WIDGET (mount_operation->priv->dialog)));
      pixbuf = __gtk_icon_theme_load_icon (theme,
  ...
  __gtk_list_store_set (list_store, &iter,
                      0, pixbuf,
                      1, markup,
                      2, pid,
                      -1);
                                    ...
                                    GPid               pid)
{
  ...
  if (__gtk_tree_model_get_iter_first (GTK_TREE_MODEL (list_store), &iter))
    {
      do
        {
          __gtk_tree_model_get (GTK_TREE_MODEL (list_store),
                              &iter,
                              2, &pid_of_item,
                              -1);
      ...
      while (__gtk_tree_model_iter_next (GTK_TREE_MODEL (list_store), &iter));
    }
}
                           ...
                           GArray            *processes)
{
  ...
  if (pid_indices_to_add->len > 0)
    {
      lookup_context = _gtk_mount_operation_lookup_context_get (__gtk_widget_get_display (mount_operation->priv->process_tree_view));
      ...
      if (__gtk_tree_model_get_iter_first (GTK_TREE_MODEL (list_store), &iter))
        {
          ...
          tree_selection = __gtk_tree_view_get_selection (GTK_TREE_VIEW (mount_operation->priv->process_tree_view));
          __gtk_tree_selection_select_iter (tree_selection, &iter);
        }
    }
                          ...
                          gpointer user_data)
{
  ...
  selection = __gtk_tree_view_get_selection (GTK_TREE_VIEW (op->priv->process_tree_view));

  if (!__gtk_tree_selection_get_selected (selection,
                                        NULL,
                                        &iter))
    goto out;

  __gtk_tree_model_get (GTK_TREE_MODEL (op->priv->process_list_store),
                      &iter,
                      2, &pid_to_kill,
                      -1);
  ...
  if (!_gtk_mount_operation_kill_process (pid_to_kill, &error))
    {
       ...
       * the dialog...
       */
      dialog = __gtk_message_dialog_new (GTK_WINDOW (op->priv->dialog),
                                     ...
                                     GtkMountOperation *op)
{
  ...
  item = __gtk_image_menu_item_new_with_mnemonic (_("_End Process"));
  __gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (item),
                                 __gtk_image_new_from_stock (GTK_STOCK_CLOSE, GTK_ICON_SIZE_MENU));
  g_signal_connect (item, "activate",
                    G_CALLBACK (on_end_process_activated),
                    op);
  __gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);
  __gtk_widget_show_all (menu);
  ...
  if (event != NULL)
    {
      ...
      if (__gtk_tree_view_get_path_at_pos (GTK_TREE_VIEW (op->priv->process_tree_view),
                                         (gint) event->x,
                                         (gint) event->y,
                                         &path,
                                         NULL,
        ...
        {
          selection = __gtk_tree_view_get_selection (GTK_TREE_VIEW (op->priv->process_tree_view));
          __gtk_tree_selection_select_path (selection, path);
    ...
    {
      button = 0;
      event_time = __gtk_get_current_event_time ();
    }

  __gtk_menu_popup (GTK_MENU (menu),
                  NULL,
                  widget,
                  NULL,
                  NULL,
                              ...
                              const char      *choices[])
{
  ...
  dialog = __gtk_dialog_new ();

  if (priv->parent_window != NULL)
    __gtk_window_set_transient_for (GTK_WINDOW (dialog), priv->parent_window);
  __gtk_window_set_title (GTK_WINDOW (dialog), "");
  __gtk_dialog_set_has_separator (GTK_DIALOG (dialog), FALSE);

  content_area = __gtk_dialog_get_content_area (GTK_DIALOG (dialog));
  vbox = __gtk_vbox_new (FALSE, 12);
  __gtk_container_set_border_width (GTK_CONTAINER (vbox), 12);
  __gtk_box_pack_start (GTK_BOX (content_area), vbox, TRUE, TRUE, 0);
  ...
  g_free (primary);
  label = __gtk_label_new (NULL);
  __gtk_label_set_markup (GTK_LABEL (label), s);
  g_free (s);
  __gtk_box_pack_start (GTK_BOX (vbox), label, TRUE, TRUE, 0);
  ...
  tree_view = __gtk_tree_view_new ();
  /* TODO: should use EM's when gtk+ RI patches land */
  __gtk_widget_set_size_request (tree_view,
                               300,
                               120);

  column = __gtk_tree_view_column_new ();
  renderer = __gtk_cell_renderer_pixbuf_new ();
  __gtk_tree_view_column_pack_start (column, renderer, FALSE);
  __gtk_tree_view_column_set_attributes (column, renderer,
                                       "pixbuf", 0,
                                       NULL);
  ...
  g_object_set (renderer,
                ...
                "ellipsize-set", TRUE,
                NULL);
  __gtk_tree_view_column_pack_start (column, renderer, TRUE);
  __gtk_tree_view_column_set_attributes (column, renderer,
                                       "markup", 1,
                                       NULL);
  __gtk_tree_view_append_column (GTK_TREE_VIEW (tree_view), column);
  __gtk_tree_view_set_headers_visible (GTK_TREE_VIEW (tree_view), FALSE);


  scrolled_window = __gtk_scrolled_window_new (NULL, NULL);
  __gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrolled_window),
                                  GTK_POLICY_NEVER,
                                  GTK_POLICY_AUTOMATIC);
  __gtk_scrolled_window_set_shadow_type (GTK_SCROLLED_WINDOW (scrolled_window), GTK_SHADOW_IN);

  __gtk_container_add (GTK_CONTAINER (scrolled_window), tree_view);
  __gtk_box_pack_start (GTK_BOX (vbox), scrolled_window, TRUE, TRUE, 0);
  ...
  list_store = __gtk_list_store_new (3,
                                   GDK_TYPE_PIXBUF,
                                   G_TYPE_STRING,
                                   G_TYPE_INT);

  __gtk_tree_view_set_model (GTK_TREE_VIEW (tree_view), GTK_TREE_MODEL (list_store));
...
gtk_mount_operation_aborted (GMountOperation *op)
{
  ...
  if (priv->dialog != NULL)
    {
      __gtk_widget_destroy (GTK_WIDGET (priv->dialog));
      priv->dialog = NULL;
      g_object_notify (G_OBJECT (op), "is-showing");
      g_object_unref (op);
    }
 ...
 */
GMountOperation *
__gtk_mount_operation_new (GtkWindow *parent)
{
...
/**
 * __gtk_mount_operation_is_showing:
 ...
 */
gboolean
__gtk_mount_operation_is_showing (GtkMountOperation *op)
{
...
/**
 * __gtk_mount_operation_set_parent:
 * @op: a #GtkMountOperation
 * @parent: (allow-none): transient parent of the window, or %NULL
 *
 ...
 */
void
__gtk_mount_operation_set_parent (GtkMountOperation *op,
                                GtkWindow         *parent)
{
  GtkMountOperationPrivate *priv;
  ...
  if (priv->dialog)
    __gtk_window_set_transient_for (GTK_WINDOW (priv->dialog), priv->parent_window);

  g_object_notify (G_OBJECT (op), "parent");
}

/**
 * __gtk_mount_operation_get_parent:
 * @op: a #GtkMountOperation
 *
 ...
 */
GtkWindow *
__gtk_mount_operation_get_parent (GtkMountOperation *op)
{
...
/**
 * __gtk_mount_operation_set_screen:
 * @op: a #GtkMountOperation
 * @screen: a #GdkScreen
 *
 ...
 */
void
__gtk_mount_operation_set_screen (GtkMountOperation *op,
                                GdkScreen         *screen)
{
  GtkMountOperationPrivate *priv;
  ...
  if (priv->dialog)
    __gtk_window_set_screen (GTK_WINDOW (priv->dialog), screen);

  g_object_notify (G_OBJECT (op), "screen");
}

/**
 * __gtk_mount_operation_get_screen:
 * @op: a #GtkMountOperation
 *
 ...
 */
GdkScreen *
__gtk_mount_operation_get_screen (GtkMountOperation *op)
{
  GtkMountOperationPrivate *priv;
  ...
  if (priv->dialog)
    return __gtk_window_get_screen (GTK_WINDOW (priv->dialog));
  else if (priv->parent_window)
    return __gtk_window_get_screen (GTK_WINDOW (priv->parent_window));
  else if (priv->screen)
    return priv->screen;
  else
