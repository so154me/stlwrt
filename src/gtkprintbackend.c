  ...
  if (g_module_supported ())
    {
      ...
      module_path = ___gtk_find_module (full_name, "printbackends");
      g_free (full_name);
             ...
             gpointer   user_data)
{
  ...
  *data = g_strdup (__gtk_entry_get_text (entry));
}
                  ...
                  const gchar      *prompt)
{
  ...
  dialog = __gtk_dialog_new_with_buttons ( _("Authentication"), NULL, GTK_DIALOG_MODAL, 
                                         GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
                                         GTK_STOCK_OK, GTK_RESPONSE_OK,
                                         NULL);

  __gtk_dialog_set_default_response (GTK_DIALOG (dialog), GTK_RESPONSE_OK);
  __gtk_dialog_set_has_separator (GTK_DIALOG (dialog), FALSE);

  main_box = __gtk_hbox_new (FALSE, 0);

  /* Left */
  icon = __gtk_image_new_from_stock (GTK_STOCK_DIALOG_AUTHENTICATION, GTK_ICON_SIZE_DIALOG);
  __gtk_misc_set_alignment (GTK_MISC (icon), 0.5, 0.0);
  __gtk_misc_set_padding (GTK_MISC (icon), 6, 6);


  /* Right */
  vbox = __gtk_vbox_new (FALSE, 0);
  __gtk_widget_set_size_request (GTK_WIDGET (vbox), 320, -1);
  ...
  __gtk_label_set_line_wrap (GTK_LABEL (label), TRUE);
  __gtk_widget_set_size_request (GTK_WIDGET (label), 320, -1);
  g_free (markup);
  ...
  __gtk_box_pack_start (GTK_BOX (vbox), label, FALSE, TRUE, 6);
  ...
  for (i = 0; i < length; i++)
    {
      ...
      if (ai_display[i] != NULL)
        {
          box = __gtk_hbox_new (TRUE, 0);

          label = __gtk_label_new (ai_display[i]);
          ...
          if (ai_default[i] != NULL)
            __gtk_entry_set_text (GTK_ENTRY (entry), ai_default[i]);

          __gtk_entry_set_visibility (GTK_ENTRY (entry), ai_visible[i]);
          __gtk_entry_set_activates_default (GTK_ENTRY (entry), TRUE);

          __gtk_box_pack_start (GTK_BOX (vbox), box, FALSE, TRUE, 6);

          __gtk_box_pack_start (GTK_BOX (box), label, TRUE, TRUE, 0);
          __gtk_box_pack_start (GTK_BOX (box), entry, TRUE, TRUE, 0);
