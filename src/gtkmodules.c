...
#include <gtkdebug.h>
#include <gtkprivate.h> /* STLWRT_LIBDIR */
#include <gtkintl.h> 
...
/* In STLWRT, since we try to emulate two versions of GTK, we consider modules
 * intended for both GTK+ 2 and GTK+ 3.  However, since STLWRT is a fork of
 * GTK+ 2, STLWRT is more closely related to GTK+ 2 and as such GTK+ 2 modules
 * should be more stable on STLWRT than GTK+ 3 modules.  Nevertheless, STLWRT
 * is _supposed_ to emulate GTK+ 3 as well, to a limited extent, so don't leave
 * GTK+ 3 modules out of the picture entirely -- just prefer a GTK+ 2 module to
 * a GTK+ 3 module.  In the future we may find GTK+ 3 applications using
 * features which were only implemented in the GTK+ 3 modules (maybe Canberra),
 * so we may have to reverse this decision in the future. */
...
get_module_path (void)
{
  ...
  gchar *home_gtk_dir_gtk2 = NULL;
  gchar *home_gtk_dir_gtk3 = NULL;
  gchar *module_path;
  gchar *default_dir_gtk2;
  gchar *default_dir_gtk3;
  ...
  if (home_dir)
  {
    home_gtk_dir_gtk2 = g_build_filename (home_dir, ".gtk-2.0", NULL);
    home_gtk_dir_gtk3 = g_build_filename (home_dir, ".gtk-3.0", NULL);
  }
  ...
  if (exe_prefix)
  {
    default_dir_gtk2 = g_build_filename (exe_prefix, "lib", "gtk-2.0", NULL);
    default_dir_gtk3 = g_build_filename (exe_prefix, "lib", "gtk-3.0", NULL);
  }
  else
  {
  ...
  if (module_path_env && home_gtk_dir_gtk2)
    module_path = g_build_path (G_SEARCHPATH_SEPARATOR_S,
				module_path_env, home_gtk_dir_gtk2,
				home_gtk_dir_gtk3, default_dir_gtk2,
				default_dir_gtk3, NULL);
  else if (module_path_env)
    module_path = g_build_path (G_SEARCHPATH_SEPARATOR_S,
				module_path_env, default_dir_gtk2,
				default_dir_gtk3, NULL);
  else if (home_gtk_dir_gtk2)
    module_path = g_build_path (G_SEARCHPATH_SEPARATOR_S,
				home_gtk_dir_gtk2, home_gtk_dir_gtk3,
				default_dir_gtk2, default_dir_gtk3, NULL);
  else
    module_path = g_build_path (G_SEARCHPATH_SEPARATOR_S,
				default_dir_gtk2, default_dir_gtk3, NULL);

  g_free (home_gtk_dir_gtk2);
  g_free (home_gtk_dir_gtk3);
  g_free (default_dir_gtk2);
  g_free (default_dir_gtk3);
 ...
 **/
gchar **
___gtk_get_module_path (const gchar *type)
{
 ...
 **/
gchar *
___gtk_find_module (const gchar *name,
		  const gchar *type)
{
  gchar **paths;
  gchar **path;
  ...
  paths = ___gtk_get_module_path (type);
  for (path = paths; *path; path++)
    {
      gchar *tmp_name;
...
find_module (const gchar *name)
{
  ...
  module_name = ___gtk_find_module (name, "modules");
  if (!module_name)
    {
	      ...
	      else
		{
		  ...
		  if (!info)
		      ...
		      if (info->display_init_func) 
			{
			  ...
			  displays = __gdk_display_manager_list_displays (__gdk_display_manager_get ());
...
default_display_notify_cb (GdkDisplayManager *display_manager)
{
  ...
  if (!__gdk_display_get_default () || default_display_opened)
    return;
		   ...
		   gboolean    is_error)
{
  ...
  for (i = 0; i < __gdk_display_get_n_screens (display); i++)
    {
  ...
  for (i = 0; i < __gdk_display_get_n_screens (display); i++)
    {
      ...
      if (__gdk_screen_get_setting (screen, "gtk-modules", &value))
	{
	  settings = __gtk_settings_get_for_screen (screen);
	  ___gtk_modules_settings_changed (settings, g_value_get_string (&value));
	  g_value_unset (&value);
	}
    }
		   ...
		   const gchar *gtk_modules_args)
{
  ...
  display_manager = __gdk_display_manager_get ();
  default_display_opened = __gdk_display_get_default () != NULL;
...
void
___gtk_modules_settings_changed (GtkSettings *settings, 
			       const gchar *modules)
{
