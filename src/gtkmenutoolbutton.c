...
{
  GtkMenuToolButtonPrivate *priv = gtk_menu_tool_button_get_props (button)->priv;
  GtkWidget *box;
  GtkOrientation orientation;

  orientation = __gtk_tool_item_get_orientation (GTK_TOOL_ITEM (button));

  if (orientation == GTK_ORIENTATION_HORIZONTAL)
    {
      box = __gtk_hbox_new (FALSE, 0);
      __gtk_arrow_set (GTK_ARROW (priv->arrow), GTK_ARROW_DOWN, GTK_SHADOW_NONE);
    }
  else
    {
      box = __gtk_vbox_new (FALSE, 0);
  ...
  if (priv->arrow_button && gtk_widget_get_props (priv->arrow_button)->parent)
    {
      g_object_ref (priv->arrow_button);
      __gtk_container_remove (GTK_CONTAINER (gtk_widget_get_props (priv->arrow_button)->parent),
                            priv->arrow_button);
  ...
  if (priv->box)
    {
       ...
       */
      __gtk_widget_destroy (priv->box);
    }

  priv->box = box;

  __gtk_container_add (GTK_CONTAINER (button), priv->box);
  __gtk_widget_show_all (priv->box);

  __gtk_button_set_relief (GTK_BUTTON (priv->arrow_button),
				    ...
				    GtkStateType  previous_state)
{
  ...
  if (!__gtk_widget_is_sensitive (widget) && priv->menu)
    {
      __gtk_menu_shell_deactivate (GTK_MENU_SHELL (priv->menu));
    }
}
                                   ...
                                   GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_MENU:
      __gtk_menu_tool_button_set_menu (button, g_value_get_object (value));
      break;
                                   ...
                                   GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_MENU:
      g_value_set_object (value, gtk_menu_tool_button_get_props (button)->priv->menu);
      break;
...
gtk_menu_tool_button_class_init (GtkMenuToolButtonClass *klass)
{
   ...
   * __gtk_menu_tool_button_get_menu(). 
                    ...
                    GtkMenuToolButton *button)
{
  ...
  __gtk_widget_size_request (GTK_WIDGET (priv->menu), &menu_req);
  ...
  if (orientation == GTK_ORIENTATION_HORIZONTAL)
	...
	*x += MAX (gtk_widget_get_props (widget)->allocation.width - menu_req.width, 0);
                        ...
                        GdkEventButton    *event)
{
  ...
  __gtk_menu_popup (priv->menu, NULL, NULL,
                  (GtkMenuPositionFunc) menu_position_func,
                  button,
                  event ? event->button : 0,
                  event ? event->time : __gtk_get_current_event_time ());
}
                         ...
                         GtkMenuToolButton *button)
{
  ...
  if (__gtk_toggle_button_get_active (togglebutton) &&
      !__gtk_widget_get_visible (GTK_WIDGET (priv->menu)))
    {
      /* we get here only when the menu is activated by a key
       * press, so that we can select the first menu item */
      popup_menu_under_arrow (button, NULL);
      __gtk_menu_shell_select_first (GTK_MENU_SHELL (priv->menu), FALSE);
    }
}
  ...
  if (event->button == 1)
    {
      ...
      __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (widget), TRUE);
...
gtk_menu_tool_button_init (GtkMenuToolButton *button)
{
  ...
  arrow_button = __gtk_toggle_button_new ();
  arrow = __gtk_arrow_new (GTK_ARROW_DOWN, GTK_SHADOW_NONE);
  __gtk_container_add (GTK_CONTAINER (arrow_button), arrow);
  __gtk_box_pack_end (GTK_BOX (box), arrow_button,
                    FALSE, FALSE, 0);

  /* the arrow button is insentive until we set a menu */
  __gtk_widget_set_sensitive (arrow_button, FALSE);

  __gtk_widget_show_all (box);
					  ...
					  const gchar  *type)
{
  if (type && strcmp (type, "menu") == 0)
    __gtk_menu_tool_button_set_menu (GTK_MENU_TOOL_BUTTON (buildable),
                                   GTK_WIDGET (child));
  else
 ...
 **/
GtkToolItem *
__gtk_menu_tool_button_new (GtkWidget   *icon_widget,
                          const gchar *label)
{
  GtkMenuToolButton *button;
  ...
  if (label)
    __gtk_tool_button_set_label (GTK_TOOL_BUTTON (button), label);

  if (icon_widget)
    __gtk_tool_button_set_icon_widget (GTK_TOOL_BUTTON (button), icon_widget);

  return GTK_TOOL_ITEM (button);
}

/**
 * __gtk_menu_tool_button_new_from_stock:
 ...
 **/
GtkToolItem *
__gtk_menu_tool_button_new_from_stock (const gchar *stock_id)
{
  GtkMenuToolButton *button;
		    ...
		    GtkMenuToolButton *button)
{
  ...
  __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (priv->arrow_button), FALSE);
...
/**
 * __gtk_menu_tool_button_set_menu:
 ...
 **/
void
__gtk_menu_tool_button_set_menu (GtkMenuToolButton *button,
                               GtkWidget         *menu)
{
  ...
  priv = gtk_menu_tool_button_get_props (button)->priv;

  if (priv->menu != GTK_MENU (menu))
    {
      if (priv->menu && __gtk_widget_get_visible (GTK_WIDGET (priv->menu)))
        __gtk_menu_shell_deactivate (GTK_MENU_SHELL (priv->menu));
      ...
      if (priv->menu)
	{
      ...
      if (priv->menu)
        {
          __gtk_menu_attach_to_widget (priv->menu, GTK_WIDGET (button),
                                     menu_detacher);

          __gtk_widget_set_sensitive (priv->arrow_button, TRUE);

          g_signal_connect (priv->menu, "deactivate",
                            G_CALLBACK (menu_deactivate_cb), button);
        }
      else
       __gtk_widget_set_sensitive (priv->arrow_button, FALSE);
    }
 ...
 **/
GtkWidget *
__gtk_menu_tool_button_get_menu (GtkMenuToolButton *button)
{
  g_return_val_if_fail (GTK_IS_MENU_TOOL_BUTTON (button), NULL);

  return GTK_WIDGET (gtk_menu_tool_button_get_props (button)->priv->menu);
}

/**
 * __gtk_menu_tool_button_set_arrow_tooltip_text:
 * @button: a #GtkMenuToolButton
 * @text: text to be used as tooltip text for button's arrow button
 *
 * Sets the tooltip text to be used as tooltip for the arrow button which
 * pops up the menu.  See __gtk_tool_item_set_tooltip() for setting a tooltip
 * on the whole #GtkMenuToolButton.
 *
 * Since: 2.12
 **/
void
__gtk_menu_tool_button_set_arrow_tooltip_text (GtkMenuToolButton *button,
					     const gchar       *text)
{
  g_return_if_fail (GTK_IS_MENU_TOOL_BUTTON (button));

  __gtk_widget_set_tooltip_text (gtk_menu_tool_button_get_props (button)->priv->arrow_button, text);
}

/**
 * __gtk_menu_tool_button_set_arrow_tooltip_markup:
 * @button: a #GtkMenuToolButton
 * @markup: markup text to be used as tooltip text for button's arrow button
 *
 * Sets the tooltip markup text to be used as tooltip for the arrow button
 * which pops up the menu.  See __gtk_tool_item_set_tooltip() for setting a
 * tooltip on the whole #GtkMenuToolButton.
 *
 * Since: 2.12
 **/
void
__gtk_menu_tool_button_set_arrow_tooltip_markup (GtkMenuToolButton *button,
					       const gchar       *markup)
{
  g_return_if_fail (GTK_IS_MENU_TOOL_BUTTON (button));

  __gtk_widget_set_tooltip_markup (gtk_menu_tool_button_get_props (button)->priv->arrow_button, markup);
}
